...
#   3. Pull config bundle by hash and verify the hash byte-for-byte
#   4. Read current manifest.yaml; stash its `current` block as the new
#      `previousManifest` in memory
#   5. Write new manifest.yaml (current = new digests + bundle + sourceSha +
#      ISO-8601 timestamp; previousManifest = stashed block)
