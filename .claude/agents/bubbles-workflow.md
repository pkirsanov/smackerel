...
Unavailable, disabled, stale, or empty recall MUST NOT block the workflow: record the state and continue. Never restate absent recall as a clean history or a novel problem.
...
- **No report-and-wait on filing (Gate G095).** Every discovered finding MUST be FILED as a tracked artifact the moment it is found — a `bug-filed` bug folder, plus an immediate spec/scope/DoD status flip (`status` off `done` / `requiresRevalidation:true` / unchecked `- [ ]`) when it invalidates a completion claim. This holds even for analysis-only rounds that will not fix this pass. Reporting findings to the user for authorization, "recommending" filing, or documenting them only in `report.md` prose is forbidden — file first, then report what was filed.
