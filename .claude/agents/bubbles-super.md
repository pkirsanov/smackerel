...
model: inherit
tools: Agent(bubbles-retro, bubbles-setup, bubbles-spec-review, bubbles-status), Read, Glob, Grep, Edit, Write, TodoWrite, WebFetch, WebSearch, Bash, mcp__bubbles
---
...
| "How do I run evidence-backed research?" | `bash bubbles/scripts/cli.sh research capabilities`, `bubbles/scripts/research-runtime.py`, and `docs/recipes/research-and-admission-runtime.md` | Default-off. Do not imply a hosted provider or downstream bridge is active |
| "Can this model/subagent/web/browser/tool dispatch consume the budget?" / "How do admission permits and session epochs work?" | `bash bubbles/scripts/cli.sh admission adapter`, `bash bubbles/scripts/cli.sh admission usage`, `bubbles/scripts/dispatch-admission.sh`, and `docs/recipes/research-and-admission-runtime.md` | Measured dispatch admission is distinct from runtime leases and consequential-action authorization. Missing usage is `unmeasured`, never zero |
