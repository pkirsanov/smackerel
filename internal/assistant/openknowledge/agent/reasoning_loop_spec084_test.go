...
func TestAgent_ComparisonSalvage_HonestlyFramed_BothSides_Spec084(t *testing.T) {
	...
	sideX := "Ocean Shores: mild maritime climate, rarely below freezing."
	sideY := "Port Orchard: cooler inland nights with greater frost risk."
	counter := 0
	r := ok.NewRegistry([]string{"seq_web"})
	if err := r.Register(seqWebTool{
	...
	fl := &fakeLLM{t: t, responses: []llm.Result{
		toolUse("wx", "seq_web", `{"query":"ocean shores pomegranate climate"}`, 100),
		toolUse("wy", "seq_web", `{"query":"port orchard pomegranate climate"}`, 100),
	...
	got, err := a.Run(context.Background(), "what is a better place to grow pomegranate, ocean shores or port orchard?")
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if got.Status != StatusSuccess {
...
func TestAgent_GenuineSynthesis_ReturnedVerbatim_NoSalvageFrame_Spec084(t *testing.T) {
	synthesis := "Port Orchard edges out Ocean Shores for pomegranates due to warmer summer days."
