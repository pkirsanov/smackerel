...
// tideSnippet mirrors the live "Ocean Shores tide times" search-result
// preview that the production salvage dumped three times.
const tideSnippet = "Tide Times · Home · United States; Ocean Shores tides. " +
	"Ocean Shores Tide Times, Washington. Tide Times Today & Tomorrow. " +
	"« Thu, June 11. Ocean Shores tide today …"
...
func TestAgent_ForcedFinalEmptySalvage_NotTriplicated_BUG064002(t *testing.T) {
	...
	fl := &fakeLLM{t: t, responses: []llm.Result{
		toolUse("w1", "fake_web", `{"query":"ocean shores tide 06/11"}`, 100),
		toolUse("w2", "fake_web", `{"query":"ocean shores tide times"}`, 100),
		toolUse("w3", "fake_web", `{"query":"ocean shores highs lows ft"}`, 100),
		endTurn("", 50),
	}}
	...
	got, err := a.Run(context.Background(), "ocean shores tide schedule 06/11 highs lows ft")
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestAgent_RealSynthesisIsPreserved_NotSnippetDump_BUG064002(t *testing.T) {
	synthesis := "Ocean Shores 06/11: high 7:42am 8.9 ft, low 1:55pm 0.3 ft."
	...
	fl := &fakeLLM{t: t, responses: []llm.Result{
		toolUse("w1", "fake_web", `{"query":"ocean shores tide"}`, 100),
		endTurn(final, 80),
	}}
	r := newRegistry(t)
	...
	got, err := a.Run(context.Background(), "ocean shores tide schedule")
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if got.Status != StatusSuccess {
...
func TestAgent_SalvageSourcesCappedAndDeduped_BUG064002(t *testing.T) {
	...
	fl := &fakeLLM{t: t, responses: []llm.Result{
		toolUse("m1", "multi_web", `{"query":"ocean shores tide"}`, 100),
