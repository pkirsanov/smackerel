...
| R-010 | The e2e lane pulls the test model AFTER the block that needs it, so seven capture-fallback tests skip while the suite still reports green | residue | `smackerel.sh` line 2267 (main go-e2e block) versus line 2310 (`scripts/commands/ollama-test-pull.sh`) | open | bubbles.devops | PROVEN 2026-08-28, not inferred. The main go-e2e block is invoked at line 2267 and reports PASS/FAIL at 2271/2273. The `SMACKEREL_TEST_OLLAMA` branch only opens at 2285 and the model pull runs at 2310, so the pull happens after the block that needs it. What the flag actually enables is a separate narrow suite at line 2344 (`go test -tags e2e_ollama ./tests/e2e/agent/...`) against the freshly pulled model. Verified empirically: a lane was run with the flag set (confirmed by reading it off the live process environment) and `ollama list` against the live test stack still returned an EMPTY table. Prewarm at line 677 cannot compensate, because it issues a `num_predict=1` generate to warm a model that was never pulled. CONSEQUENCE, and the reason this is filed rather than left in one bug: `TestAssistantHTTPE2E_CaptureFallbackOpenKnowledgeNoGround` plus six siblings skip with `error_cause="provider_unavailable"`, and the lane still reports PASS for go-e2e. That is the same false-green class already filed as BUG-069-005, so the suite currently overstates what it verifies. Two candidate fixes, and the choice is a real cost decision an owner should make rather than an agent make silently: either make the SST-resolved model present before the 2267 block whenever `ollama_enabled` is true, which adds a multi-GB pull to the DEFAULT e2e path for every run, or leave the pull where it is and make the skip loud so a skipped required test cannot be absorbed into a green lane. Blocks the 11 unchecked DoD items on BUG-074-002 either way | 2026-08-28 | 2026-08-28 |
