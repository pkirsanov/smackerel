...
| R-006 | specs/031-live-stack-testing carries Gate G022 artifact-lint defects (missing gaps and harden phase records) | residue | specs/031-live-stack-testing | open | bubbles.gaps | Run the gaps then harden specialists against the spec. Record each phase name in `certification.certifiedCompletedPhases` — when that array is non-empty it is the ONLY one artifact-lint reads, so a record appended to `execution.completedPhaseClaims` is silently ignored. Do not hand-author phase records, which is the fabrication G022 exists to catch | 2026-08-03 | 2026-08-04 |
| R-007 | specs/069-assistant-http-transport has one pre-existing evidence block below the 2-signal threshold | residue | specs/069-assistant-http-transport/report.md | open | bubbles.docs | Locate the remaining fenced block that scores 1/2 terminal-output signals and either add the literal `$ `-prefixed command line to it or, if it is an illustration rather than evidence, wrap it in the `bubbles:evidence-legitimacy-skip` markers. Spec lint is at 1 issue, down from 7 | 2026-08-04 | 2026-08-04 |
