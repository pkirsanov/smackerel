...
1. **The structured-JSON extraction path MUST NOT emit a qwen3 hidden-reasoning block when SST
   ...
   extraction call on the `LLM_MODEL` (qwen3) text path MUST send a thinking-disable directive
   (`/no_think`) to Ollama, so qwen3 skips its `<think>…</think>` block and returns the JSON
   directly (~10s, inside the 30s domain budget).
...
3. **Value semantics.**
   - `false` → thinking DISABLED (inject `/no_think`) — the fix's default posture.
...
4. **The agent reasoning path is UNCHANGED.** `ml/app/agent.py::handle_invoke` MUST NOT receive a
   `/no_think` injection even when `ML_STRUCTURED_EXTRACTION_THINKING=false`; the agent path keeps
   qwen3 thinking (quality > latency). This is a hard scope boundary.

5. **Non-qwen safety.** The `/no_think` mechanism MUST be a no-op on non-ollama providers and on
   non-thinking Ollama models (e.g. `gemma3:4b`), so dev/test deployments are unaffected.
...
- [ ] `apply_structured_extraction_thinking(messages, "ollama")` with SST=`false` injects
  `/no_think` into the request messages; with SST=`true` returns messages unchanged; with a
  non-ollama provider returns messages unchanged regardless of SST.
- [ ] Each in-scope call site (domain, synthesis extract + crosssource, processor, search-rerank,
  ...
  composes its `litellm.acompletion` request with `/no_think` present in the messages.
- [ ] With `ML_STRUCTURED_EXTRACTION_THINKING=true`, the same call sites compose requests WITHOUT
  `/no_think` (adversarial: proves the switch actually gates and is not hard-wired on).
- [ ] The agent path composes requests WITHOUT `/no_think` even when SST=`false` (scope boundary).
