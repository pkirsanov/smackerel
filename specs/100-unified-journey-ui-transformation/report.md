...
| F7 | `cardrewards_dashboard.spec.ts:36` SCN-083-K01 | Failure screenshot showed a fully-rendered dashboard ("0 NEEDS VERIFY", DashFlag absent). Static analysis proves the code path is correct: `Service.Reconcile` (`service_insights.go` L172) uses the **request** threshold 0.7 over **all** observation refs; `mergeObservations` (`reconcile.go` L158) sets `needsVerify = 0.4 < 0.7 = true`; `card_rewards.enabled:false` ⇒ no background scheduler; every `reconcileAPI` call uses 0.7 (no lower-threshold re-reconcile). ~~No deterministic code bug exists. **Determination: transient/flake** in the first cold run — not an impl bug, not a test defect.~~ **⚠️ SUPERSEDED — see the "F7 supersession — build-freshness exposed a REAL bug" note directly below the table.** The static read missed the parallel-pipeline path: with `card_rewards` DISABLED the pipeline reconciler's `ConfidenceThreshold` is a degenerate `0.0`, and a parallel admin `scrape-now` → `RunDailyRefresh` re-reconciled every observation at `0.0` (`needsVerify = confidence < 0.0` is always false), **clearing** K01's flag (seeded at 0.7) over the shared test DB. The earlier green was against a **stale `smackerel-core` image**; the `web-e2e-ui.sh --build` freshness fix re-exposed it as a REAL failure (~3 of 4 fresh-core runs). | ~~No code/test change. Verified by re-run: SCN-083-K01 passes green in **2/2** consecutive re-runs.~~ **Real fix:** `reconcile.go` `mergeObservations` guards the confidence flag against an out-of-range threshold — when threshold ∉ (0,1] it **preserves** an existing `needs_verification` flag instead of clearing it (no-op in production where the threshold is always valid; aligns the pipeline reconciler with `Service.Reconcile`). Adversarial regression `TestReconcile_InvalidThresholdPreservesNeedsVerification_K01` (RED pre-fix → GREEN post-fix). K01 Playwright assertion **not** weakened. |

### F7 supersession — build-freshness exposed a REAL bug (SCN-083-K01)

> **This supersedes the F7 "transient/flake / no code change" determination in the table above.** The original reasoning is retained (struck through, not erased); this note records the corrected evidence.

**Why the verdict changed.** The F7 "flake" call rested on SCN-083-K01 passing on re-run — but those runs reused a **stale 24 h-old `smackerel-core` image** (the staleness already flagged in "Environment conditions handled" #1), so the browser never exercised current source. A build-freshness fix to `scripts/runtime/web-e2e-ui.sh` — the lane now runs `e2e_ui_compose up -d --wait --wait-timeout "$wait_timeout_s" --build`, so the disposable stack **always rebuilds core from current source** — removed that mask. Against freshly-built core, SCN-083-K01 **failed ~3 of 4 runs**: a real, order-dependent failure, not a flake.

**Real root cause (application bug).** In the e2e-ui env `card_rewards` is DISABLED, so the pipeline reconciler's `CardRewards.Extraction.ConfidenceThreshold` is a degenerate `0.0`. Playwright runs spec files in parallel over one shared test DB, so a parallel admin `scrape-now` → `RunDailyRefresh` globally re-reconciled **every** observation at `0.0`; in `mergeObservations` the single-source branch computed `needsVerify = confidence < 0.0`, which is **always false**, silently **clearing** K01's `needs_verification` flag that a web reconcile had seeded at threshold `0.7`. The original static analysis reasoned only about the web `reconcileAPI` at 0.7 and missed this cross-test pipeline path — which is why the "no deterministic bug" conclusion was wrong. Disagreement-based flags are threshold-independent, so only single-source K01 flaked while K04 survived.

**Fix (`internal/cardrewards/reconcile.go`).** `mergeObservations` now guards the confidence-based flag against an out-of-range threshold: when threshold ∉ (0,1] it **preserves** an existing `needs_verification` flag rather than evaluating `confidence < threshold` and clearing it. This is a **no-op in production**, where the threshold is always a valid (0,1] value, and it aligns the pipeline reconciler with the contract `Service.Reconcile` already enforces; a brand-new record with no prior flag stays unflagged. Adversarial regression `TestReconcile_InvalidThresholdPreservesNeedsVerification_K01` was added (RED on the pre-fix `needsVerify = confidence < threshold`, GREEN post-fix; a control asserts a valid 0.70 threshold still clears the flag for a fresh 0.95-confidence source — no over-preservation). **The K01 Playwright assertion was not weakened.**

**Verification (fresh-core).** Unit `TestReconcile_InvalidThresholdPreservesNeedsVerification_K01` RED→GREEN; full `./smackerel.sh test unit` OK (0 FAIL); `./smackerel.sh lint` clean; `./smackerel.sh test e2e-ui` against freshly-built core → **43 passed / 9 skipped / 0 failed, twice (×2)**, SCN-083-K01 passing at **retry=0** (zero flakes) both times. The "Final browser-green re-run" block below is kept as the historical F1–F6 record, but its `← F7` line reflected the stale-core image; the genuine fresh-core green for K01 is this ×2 run. Environment condition #1 (stale disposable image) is now resolved by the `web-e2e-ui.sh --build` change.
