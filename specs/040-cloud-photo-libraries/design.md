...
- Reveal-token persistence: the wire blob is `<uuid>.<secret>` (UUID id + 24-byte `crypto/rand` secret). The server persists `sha256(secret)` in `photo_reveal_tokens.secret_hash` (bytea); the plaintext secret is returned only at mint time. `ConsumeRevealToken` and `CheckRevealToken` constant-time compare the presented secret's digest against the stored hash via `crypto/subtle`. Mismatched secrets collapse to a generic "token not found" error so a tampered consume cannot distinguish a missing token id from a wrong secret. The consume transaction holds `SELECT … FOR UPDATE` and the `UPDATE` carries `WHERE consumed_at IS NULL` plus a `RowsAffected()==1` check so concurrent consumers cannot both succeed.
