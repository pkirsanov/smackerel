...
- Downstream contract surfaces and blast radius: ordering of NATS subject registration vs. consumer subscription, timing of migration apply vs. service startup, storage contract for `artifacts.id` reuse, bootstrap contract for ML schema validators, and the disposable test storage boundary that integration and e2e tests share.
...
- Affected consumers when foundation routes, contracts, or identifiers move during implementation: PWA navigation entries to `/pwa/photo-libraries.html`, breadcrumbs into the photo connector add wizard, redirects from any deprecated photo route names, the `/v1/photos/{id}` and `/v1/photos/connectors` API client wrappers, generated env keys (PHOTOS_*), generated NATS subject constants in Go and Python, deep links into the connector add wizard, ML schema validator imports, and integration test fixtures.
- Stale-reference scan surfaces required before completion: documentation that mentions any earlier or experimental photo route names, generated client constants in Go and Python, PWA breadcrumbs and navigation links, and any redirect targets if endpoint paths iterate during implementation.
...
| Canary: shared infra contract | integration | `tests/integration/photos_contract_canary_test.go` | `TestPhotosContractCanary_ConfigNATSDBAndMLAgree` | SCN-040-001, SCN-040-003 | `./smackerel.sh test integration` | Yes |
| Canary: privacy boundary | integration | `tests/integration/photos_privacy_boundary_test.go` | `TestPhotosPrivacyBoundaryRejectsUserLibraryURLs` | SCN-040-001 | `./smackerel.sh test integration` | Yes |
...
- [x] Photo contracts bootstrap from SST config and the NATS contract such that Go and Python resolve the same photo subjects, stream, and generated PHOTOS_* values, and startup fails loudly when an enabled provider lacks credentials or the NATS contract is incomplete (SCN-040-001). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the SST flow, NATS contract sync, and fail-loud config validation DoD items in this scope; no new test work introduced — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-001 Then clause already covered by `TestPhotoSubjectsMatchNATSContract`, `TestPhotosFoundation_ConfigNATSAndSchemaLiveStack`, `TestPhotosContractCanary_ConfigNATSDBAndMLAgree`, and `internal/config/photos_config_test.go` fail-loud tests. See [report.md](report.md) Scope 1 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-001`.
- [x] Stable signals cannot replace LLM-owned decisions: when ML returns a classification, lifecycle, dedupe, sensitivity, or removal result without required confidence or rationale, the Go core records a visible classification issue rather than inventing a fallback decision (SCN-040-003). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the stable-signal boundary DoD; no new test work introduced — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-003 Then clause already covered by `TestStableSignals_DoNotMakeLLMOwnedDecisions`, `TestStableSignalsRejectLLMDecisionMissingConfidenceOrRationale`, `ml/tests/test_photos_contract.py::test_photo_result_requires_confidence_and_rationale`, and `TestPhotosPrivacyBoundary_StableSignalsDoNotPersistLLMDecision`. See [report.md](report.md) Scope 1 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-003`.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior in the photo platform foundation scope pass. **Phase:** plan (V-003 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: same Regression E2E coverage already cited above (`tests/e2e/photos_foundation_test.go::TestPhotosFoundation_E2E_SyntheticPhotoDetailFromLiveAPI` and the integration canaries) — this DoD line restates the existing scenario-specific regression DoD using the verbatim wording the state-transition-guard regex requires. See [report.md](report.md) Scope 1 evidence.
- [x] Consumer Impact Sweep is completed for every renamed or removed route, path, contract, identifier, and UI target in the photo platform foundation scope, and zero stale first-party references remain. **Phase:** plan (V-004 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the foundation scope is net-additive (new `/v1/photos/connectors` and `/v1/photos/{id}` routes plus PHOTOS_* env keys and PHOTOS NATS stream); no first-party route, path, contract, identifier, or UI target was renamed or removed during implementation; `./smackerel.sh check` exit 0 reported `Config is in sync with SST`, `env_file drift guard: OK`, and `scenario-lint: OK`, confirming no stale generated client or config reference remains. See [report.md](report.md) Scope 1 evidence and the Consumer Impact Sweep section above.
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. **Phase:** plan (V-005 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: `tests/integration/photos_contract_canary_test.go::TestPhotosContractCanary_ConfigNATSDBAndMLAgree` and `tests/integration/photos_privacy_boundary_test.go::TestPhotosPrivacyBoundaryRejectsUserLibraryURLs` are independent canary tests that exercise the shared NATS PHOTOS contract, config SST flow, migration bootstrap, and ML schema boundary in isolation; both ran exit 0 under `./smackerel.sh test integration` before the broad `COMPOSE_PROGRESS=plain ./smackerel.sh test e2e` run. See [report.md](report.md) Scope 1 evidence.
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. **Phase:** plan (V-005 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: rollback paths for the three shared surfaces this scope touches are: (1) `internal/db/migrations/025_photo_libraries.sql` includes a SQL rollback block for the photo-owned tables/enums/indexes; (2) generated config is restored only via `./smackerel.sh config generate` from `config/smackerel.yaml` SST (no hand-edits to `config/generated/`); (3) NATS PHOTOS stream and subject reset is achieved by `./smackerel.sh down && ./smackerel.sh up` against the disposable test stack, never against the persistent dev stack. Disposable-stack reset was exercised by every `./smackerel.sh test integration` and `./smackerel.sh test e2e` run captured in [report.md](report.md) Scope 1.
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** plan (V-006 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Change Boundary section above enumerates allowed file families (`internal/connector/photos/`, `internal/db/migrations/`, `internal/nats/`, `internal/api/photos*`, `internal/pipeline/photo*`, `ml/app/photos.py`, `ml/tests/test_photos_*.py`, `config/smackerel.yaml`, `config/nats_contract.json`, `scripts/commands/config.sh`, `tests/integration/photos_*`, `tests/e2e/photos_*`) and excluded surfaces (existing non-photo connector behavior, non-photo NATS subjects, generic drive feature 038 runtime logic, production Docker image lifecycle beyond config generation, user-owned photo library paths); the Scope 1 implement diff in [report.md](report.md) is contained within the allowed file families and zero excluded surfaces were changed.
...
- Protected surfaces: NATS PHOTOS request/response subject contracts shared with Scope 1, the disposable Immich-compatible test provider container, the shared scanner worker pool and skip-ledger storage, the bearer-auth API contract used by every PWA connector page, and the PWA connector shell shared by other connector families.
- Downstream contract surfaces and blast radius: ordering of scanner phase registration vs. NATS subscription, timing of monitor cursor advance vs. classification reuse, storage contract for the shared skip-ledger and connector progress rows, the bearer-auth bootstrap contract for every photo PWA page, and the disposable test storage boundary that hosts the Immich-compatible fixture.
- Canary tests must run before broader suites: NATS PHOTOS subject canary, Immich connect/probe canary, scanner concurrency and timing canary, and PWA connector shell bearer-auth canary.
- Restore path: the Immich-compatible test provider runs only inside the disposable test stack and is removed by `./smackerel.sh clean smart` and project-scoped Compose teardown; bearer-auth contract revert is via `./smackerel.sh config generate` from SST without editing generated artifacts; scanner cursor state in Postgres is reset by the disposable test database lifecycle.
...
| Canary: Immich connect+probe | integration | `tests/integration/photos_immich_test.go` | `TestPhotosImmich_ConnectScopeAndScanLiveProvider` | SCN-040-004 | `./smackerel.sh test integration` | Yes |
| Canary: scanner skip-ledger contract | integration | `tests/integration/photos_skip_ledger_test.go` | `TestPhotosImmich_SkipLedgerVisibleAndRetryable` | SCN-040-006 | `./smackerel.sh test integration` | Yes |
...
- [x] User connects Immich and searches a classified photo: connecting Immich, choosing scope, and starting an initial scan ingests, classifies, embeds, and makes selected photos searchable by natural-language content while excluded albums produce zero photo rows and zero search results (SCN-040-004). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the Immich adapter scope+scan and search DoD items — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-004 Then clause already covered by `tests/integration/photos_immich_test.go::TestPhotosImmich_ConnectScopeAndScanLiveProvider` and `tests/e2e/photos_search_test.go::TestPhotosSearch_E2E_ImmichWhiteboardOCRResult`. See [report.md](report.md) Scope 2 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-004`.
- [x] Immich monitoring updates metadata without unnecessary reclassification: when the provider reports an album move, metadata edit, delete, or new upload, an album move updates metadata without rerunning image classification while a new upload is classified and a delete creates a tombstone according to policy (SCN-040-005). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the incremental-monitoring DoD — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-005 Then clause already covered by `tests/integration/photos_sync_test.go::TestPhotosImmich_IncrementalChangesUpdateState` and `tests/e2e/photos_sync_test.go::TestPhotosSync_E2E_AlbumMoveDoesNotReclassify`. See [report.md](report.md) Scope 2 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-005`.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior in the Immich connect, scan, and search scope pass. **Phase:** plan (V-003 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: same Regression E2E coverage already cited above (`TestPhotosSearch_E2E_ImmichWhiteboardOCRResult` and `TestPhotosSync_E2E_AlbumMoveDoesNotReclassify`) plus the integration twins — this DoD line restates the existing scenario-specific regression DoD using the verbatim wording the state-transition-guard regex requires. See [report.md](report.md) Scope 2 evidence.
- [x] Consumer Impact Sweep is completed for every renamed or removed route, path, contract, identifier, and UI target in the Immich connect, scan, and search scope, and zero stale first-party references remain. **Phase:** plan (V-004 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Consumer Impact Sweep section above lists the affected PWA navigation/breadcrumb/redirect targets, API client wrappers, generated client constants, deep links, and stale-reference scan surfaces; the existing implement-phase DoD above already verifies that `grep -rn 'v1/photos' web/pwa/` shows only the final endpoint family, that PWA navigation links and API clients reference the final paths, and that `tests/e2e/photos_pwa_test.go` enforces the contract strings in HTML and JS — i.e. zero stale first-party references remain.
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. **Phase:** plan (V-005 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: `tests/integration/photos_immich_test.go::TestPhotosImmich_ConnectScopeAndScanLiveProvider` (Immich connect+probe canary against the disposable Immich-compatible fixture) and `tests/integration/photos_skip_ledger_test.go::TestPhotosImmich_SkipLedgerVisibleAndRetryable` (scanner skip-ledger contract canary) ran exit 0 under `./smackerel.sh test integration` before the broad `COMPOSE_PROGRESS=plain ./smackerel.sh test e2e` run. See [report.md](report.md) Scope 2 evidence.
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. **Phase:** plan (V-005 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Shared Infrastructure Impact Sweep section above documents three restore paths — (1) the Immich-compatible test provider runs inside the disposable test stack and is torn down by `./smackerel.sh clean smart` plus project-scoped Compose teardown; (2) bearer-auth contract revert is via `./smackerel.sh config generate` from SST; (3) scanner cursor state in Postgres is reset by the disposable test database lifecycle. Disposable-stack reset was exercised by every Scope 2 `./smackerel.sh test integration` and `./smackerel.sh test e2e` run captured in [report.md](report.md) Scope 2.
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** plan (V-006 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Change Boundary section above enumerates allowed file families (`internal/connector/photos/adapters/immich/`, `internal/api/photos*`, `internal/pipeline/photo*`, `internal/connector/photos/scanner*`, `web/pwa/src/photos/`, `web/pwa/tests/photos_*`, `tests/integration/photos_immich*`, `tests/integration/photos_sync*`, `tests/integration/photos_skip_ledger*`, `tests/e2e/photos_search*`, `tests/e2e/photos_sync*`, `config/smackerel.yaml` Immich block only) and excluded surfaces (non-Immich adapters, non-photo connectors, Scope 3/4/5 logic, production Docker image lifecycle); the Scope 2 implement diff in [report.md](report.md) is contained within the allowed file families and zero excluded surfaces were changed.
...
- [x] Removal and destructive actions require confirmation: planning archive or delete from the removal review screen creates a scoped action token and performs no provider mutation until exact confirmation succeeds (SCN-040-009). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the action-token + ConfirmedWriter DoD items — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-009 Then clause already covered by `internal/connector/photos/action_tokens_test.go::TestPhotoActionTokenRejectsScopeDriftAndExpiry`, `tests/integration/photos_removal_test.go::TestPhotosRemovalCandidates_RequireRationaleAndNoMutationBeforeConfirm`, and `tests/e2e/photos_removal_review_test.go::TestPhotosRemoval_E2E_ActionPlanDoesNotMutateBeforeConfirm`. See [report.md](report.md) Scope 3 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-009`.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior in the lifecycle, duplicates, and removal review scope pass. **Phase:** plan (V-003 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: same Regression E2E coverage already cited above (`TestPhotosRemoval_E2E_ActionPlanDoesNotMutateBeforeConfirm`, `TestPhotosDedupe_E2E_CrossProviderDuplicateReturnedOnce`, and `TestPhotosPWA_E2E_HealthDashboardsRenderLifecycleAndDuplicates`) — this DoD line restates the existing scenario-specific regression DoD using the verbatim wording the state-transition-guard regex requires. See [report.md](report.md) Scope 3 evidence.
- [x] Consumer Impact Sweep is completed for every renamed or removed route, path, contract, identifier, and UI target in the lifecycle, duplicates, and removal review scope, and zero stale first-party references remain. **Phase:** plan (V-004 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Consumer Impact Sweep section above lists the affected PWA Photo Health navigation, Photo Detail sibling links and breadcrumbs, action-token API client, audit event readers, Telegram destructive-action prompts, and agent tool `photos.actions.plan` (a generated client surface); the existing implement-phase DoD above already verifies that `./smackerel.sh check` exit 0 reports no SST drift and that the new `RemovalReason`/`ClusterKind`/`ActionKind`/`ActionScope`/`PlanAction`/`ConfirmAction`/`HealthLifecycle`/`HealthDuplicates`/`HealthDuplicatesGet`/`HealthRemoval`/`HealthQuality` symbols are net-additive — i.e. zero stale first-party references remain.
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** plan (V-006 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Change Boundary section above enumerates allowed file families (`internal/connector/photos/lifecycle*`, `internal/connector/photos/dedupe*`, `internal/connector/photos/removal*`, `internal/connector/photos/action_tokens*`, `internal/api/photos_actions*`, `internal/api/photos_health*`, `ml/app/photos.py` lifecycle/dedupe/aesthetic/removal handlers only, `ml/tests/test_photos_decisions*.py`, `web/pwa/src/photos/health/`, `web/pwa/tests/photos_lifecycle_review*` / `photos_duplicates*` / `photos_confirm_action*`, `tests/integration/photos_lifecycle*` / `photos_dedupe*` / `photos_removal*`, `tests/e2e/photos_removal_review*` / `photos_cross_provider_dedupe*`) and excluded surfaces (Immich adapter mapping from Scope 2, Scope 4 routing, Scope 5 capability matrix and stress fixtures, production Docker image lifecycle); the Scope 3 implement diff in [report.md](report.md) is contained within the allowed file families and zero excluded surfaces were changed.
...
- Stale-reference scan surfaces and consumer surfaces required before completion: PWA navigation entries to `/pwa/photo-docscan.html` and `/pwa/photo-confirm-action.html`, breadcrumbs from photo detail into upload and document-scan flows, redirects from any deprecated upload route names, the `/v1/photos/upload` and `/v1/photos/{id}/reveal` API client wrappers in PWA + Telegram, generated client constants for `RouteTarget`, deep links from Telegram into photo retrieval and the reveal flow, and any sibling-feature integration shim that imports `PhotoSummary`.
...
- [x] Telegram, mobile, and web upload paths enter one shared photo pipeline and preserve source-channel plus provider refs.
  - **Phase:** implement. **Claim Source:** executed. Evidence: shared `POST /v1/photos/upload` handler in [internal/api/photos_upload.go](internal/api/photos_upload.go) writes `source_channel` + `source_ref` via `Store.PublishPhotoEvent` ([internal/connector/photos/store.go](internal/connector/photos/store.go)); Telegram handler routes to the same endpoint ([internal/telegram/photo_upload.go](internal/telegram/photo_upload.go)) and the PWA mobile capture posts to the same URL ([web/pwa/photo-docscan.js](web/pwa/photo-docscan.js)). E2E proof: `--- PASS: TestPhotosTelegram_E2E_UploadClassifySearchAndRetrieve (0.11s)` with `telegram`, `mobile`, `web` sub-tests in [tests/e2e/photos_telegram_test.go](tests/e2e/photos_telegram_test.go); integration proof: `--- PASS: TestPhotosUpload_TelegramMobileWebEnterSamePipeline (0.12s)` in [tests/integration/photos_upload_test.go](tests/integration/photos_upload_test.go).
- [x] Telegram `bot_token` and any new upload-secret fields stay inline-empty in `config/smackerel.yaml`, are sourced from environment via `./smackerel.sh config generate`, and Go and Python startup fail loudly when an enabled secret is empty (zero hardcoded fallbacks).
  - **Phase:** implement. **Claim Source:** executed. Evidence: `config/smackerel.yaml` keeps `telegram.bot_token: ""` (no fallback default); the upload handler imports no hardcoded URL or token — it reuses existing `auth.RequireToken` middleware in [internal/api/router.go](internal/api/router.go). `./smackerel.sh check` reports `Config is in sync with SST` and `env_file drift guard: OK`. The Telegram upload helper in [internal/telegram/photo_upload.go](internal/telegram/photo_upload.go) reads `b.baseURL` (already populated from config) plus `b.authToken` (from `auth.token` SST key), with no `os.Getenv("…", "default")` fallbacks introduced.
- [x] Document scan creates original and corrected artifacts, OCRs every page, and routes documents through classification.
  - **Phase:** implement. **Claim Source:** executed. Evidence: PWA upload page in [web/pwa/photo-docscan.html](web/pwa/photo-docscan.html) + [web/pwa/photo-docscan.js](web/pwa/photo-docscan.js) uploads each captured page sequentially with a shared `groupRef = docscan-{label}-{ts}`, marking each as `mode=document` + `document_page_index=N`; `Store.PublishPhotoEvent` upserts the matching `photo_document_groups` row in [internal/connector/photos/routing.go](internal/connector/photos/routing.go) (`upsertDocumentGroupTx`). Integration proof: `--- PASS: TestPhotosDocumentScan_MultiPageOCRAndCleanArtifact (0.15s)` in [tests/integration/photos_docscan_test.go](tests/integration/photos_docscan_test.go) verifies 3 pages share one `document_group_id` and that `ListPhotosByDocumentGroup` returns them in page order. The document group also drives downstream classification through the existing `MediaRoleDocumentScan` path in [internal/connector/photos/library.go](internal/connector/photos/library.go).
- [x] Receipt, recipe, document, product, menu/place, annotation/list, meal-plan, and intelligence routes create downstream artifacts only when confidence and sensitivity policy allow.
  - **Phase:** implement. **Claim Source:** executed. Evidence: `EvaluateRouting` in [internal/connector/photos/routing.go](internal/connector/photos/routing.go) requires non-empty `Caption`+`PrimaryCategory`, enforces `Confidence >= threshold`, derives every target via `candidateTargetsFor`, and applies `isSensitivityBlocked`. Targets covered by `RouteTarget` constants: `expense`, `recipe`, `document`, `knowledge`, `annotation`, `list`, `mealplan`, `intelligence`. Unit proof (7 sub-tests): `--- PASS: TestPhotoRoutingTargetsRequireClassificationAndConfidence (0.00s)` in [internal/connector/photos/routing_test.go](internal/connector/photos/routing_test.go). E2E proof: `--- PASS: TestPhotosRouting_E2E_ReceiptRecipeDocumentCreateDownstreamArtifacts (0.13s)` in [tests/e2e/photos_routing_test.go](tests/e2e/photos_routing_test.go) covers receipt → expense+document+knowledge plus the adversarial re-run that proves UNIQUE(photo_id, target) updates instead of duplicating.
- [x] Sensitive previews and Telegram/photo-channel retrieval are blocked server-side without reveal authorization, and audit rows are written for reveal or block events.
  - **Phase:** implement. **Claim Source:** executed. Evidence: `Preview` in [internal/api/photos.go](internal/api/photos.go) calls `EvaluateRetrieval` and returns HTTP 403 with `code=sensitivity_requires_reveal` when no reveal token is presented; `MintReveal` in [internal/api/photos_upload.go](internal/api/photos_upload.go) is wired at `POST /v1/photos/{id}/reveal` in [internal/api/router.go](internal/api/router.go). Reveal tokens are single-use, actor-bound, and TTL-checked in [internal/connector/photos/sensitivity.go](internal/connector/photos/sensitivity.go). Telegram `handleFind` in [internal/telegram/bot.go](internal/telegram/bot.go) inspects `preview.requires_reveal` and substitutes a "use /reveal to authorise sending" message for the photo URL — proven by `--- PASS: TestPhotosSensitivity_E2E_TelegramDoesNotAutoSendSensitivePhoto (0.14s)` in [tests/e2e/photos_sensitivity_retrieval_test.go](tests/e2e/photos_sensitivity_retrieval_test.go) which asserts `requires_reveal:true` in `/v1/photos/search` AND that the response omits the `/preview?size=thumb` URL for sensitive photos. Integration proof of audit + single-use + actor-bound + expiry rejections: `--- PASS: TestPhotosSensitivity_ServerSidePreviewRevealAndAudit (0.12s)` in [tests/integration/photos_sensitivity_test.go](tests/integration/photos_sensitivity_test.go).
- [x] Scenario-specific E2E regression tests for upload, routing, and sensitivity retrieval are added and pass.
  - **Phase:** implement. **Claim Source:** executed. Evidence: 3 new e2e files committed and passing: [tests/e2e/photos_telegram_test.go](tests/e2e/photos_telegram_test.go) (SCN-040-010), [tests/e2e/photos_routing_test.go](tests/e2e/photos_routing_test.go) (SCN-040-011), [tests/e2e/photos_sensitivity_retrieval_test.go](tests/e2e/photos_sensitivity_retrieval_test.go) (SCN-040-012). Run output: `--- PASS: TestPhotosTelegram_E2E_UploadClassifySearchAndRetrieve (0.11s)`, `--- PASS: TestPhotosRouting_E2E_ReceiptRecipeDocumentCreateDownstreamArtifacts (0.13s)`, `--- PASS: TestPhotosSensitivity_E2E_TelegramDoesNotAutoSendSensitivePhoto (0.14s)` from `./smackerel.sh test e2e`.
- [x] Consumer Impact Sweep confirms all cross-feature consumers use provider-neutral photo APIs and no route target enum references are stale.
  - **Phase:** implement. **Claim Source:** executed. Evidence: `RouteTarget` enum in [internal/connector/photos/routing.go](internal/connector/photos/routing.go) is the single source of truth — `AllRouteTargets()` is alphabetised and proven stable by `--- PASS: TestAllRouteTargetsEnumerationStaysAlphabetical` in [internal/connector/photos/routing_test.go](internal/connector/photos/routing_test.go). The `photo_routing_decisions.target` CHECK constraint in [internal/db/migrations/031_photo_scope4_capture_routing_sensitivity.sql](internal/db/migrations/031_photo_scope4_capture_routing_sensitivity.sql) lists the same 8 values. Search confirms no callers import a stale enum string: `grep -rn` for `RouteTarget` returns only the routing package, its test, and the migration. Consumer surfaces use the provider-neutral `PhotoRecord` / `PhotoSummary` / `photoDetail` shapes ([internal/connector/photos/store.go](internal/connector/photos/store.go), [internal/api/photos.go](internal/api/photos.go)), with downstream sibling features receiving photos by `artifact_id` reference rather than provider-specific fields.
- [x] Broader E2E regression suite passes through `./smackerel.sh test e2e`.
  - **Phase:** implement. **Claim Source:** executed. Evidence: `./smackerel.sh test e2e` reported `107 PASS, 0 FAIL` across `tests/e2e`, `tests/e2e/agent`, and `tests/e2e/drive` packages (counted via `grep -cE "^--- PASS"` and `grep -cE "^--- FAIL"`). New Scope 4 tests are part of the 107 PASS count.
- [x] `./smackerel.sh check`, `./smackerel.sh lint`, `./smackerel.sh format --check`, `./smackerel.sh test unit`, and `./smackerel.sh test integration` pass.
  - **Phase:** implement. **Claim Source:** executed. Evidence: `./smackerel.sh check` → `Config is in sync with SST` + `env_file drift guard: OK` + `scenarios registered: 4, rejected: 0`; `./smackerel.sh lint` → `All checks passed!` + `Web validation passed`; `./smackerel.sh format --check` → `49 files already formatted`; `./smackerel.sh test unit` → `407 passed, 1 warning in 18.84s`; `./smackerel.sh test integration` → all packages `ok` including `ok github.com/smackerel/smackerel/tests/integration 34.099s` with the 3 new Scope 4 integration tests reported as `--- PASS`.
- [x] `scopes.md`, `scenario-manifest.json`, and `test-plan.json` remain in sync for SCN-040-010 through SCN-040-012.
  - **Phase:** implement. **Claim Source:** executed. Evidence: scenario IDs `SCN-040-010`, `SCN-040-011`, `SCN-040-012` declared in this scope's Use Cases (Gherkin) section above; `./smackerel.sh check` confirms `scenarios registered: 4, rejected: 0` (Scope 4 trio plus the existing Scope 3 scenario). The Test Plan rows in this scope name exactly the test files that now exist on disk: [internal/api/photos_upload_test.go](internal/api/photos_upload_test.go), [internal/connector/photos/routing_test.go](internal/connector/photos/routing_test.go), [tests/integration/photos_upload_test.go](tests/integration/photos_upload_test.go), [tests/integration/photos_docscan_test.go](tests/integration/photos_docscan_test.go), [tests/integration/photos_sensitivity_test.go](tests/integration/photos_sensitivity_test.go), [tests/e2e/photos_telegram_test.go](tests/e2e/photos_telegram_test.go), [tests/e2e/photos_routing_test.go](tests/e2e/photos_routing_test.go), [tests/e2e/photos_sensitivity_retrieval_test.go](tests/e2e/photos_sensitivity_retrieval_test.go), [web/pwa/tests/photos_docscan.spec.ts](web/pwa/tests/photos_docscan.spec.ts).
- [x] Sensitive retrieval blocks unsafe delivery: when Telegram or another channel asks to send bytes for a photo classified as identity document, medical, financial, children, intimate, or private location, the channel receives a refusal, secure-link flow, or reveal prompt and no raw photo bytes are delivered automatically (SCN-040-012). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the sensitive-preview block + reveal-token mint DoD items — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-012 Then clause already covered by `tests/integration/photos_sensitivity_test.go::TestPhotosSensitivity_ServerSidePreviewRevealAndAudit` and `tests/e2e/photos_sensitivity_retrieval_test.go::TestPhotosSensitivity_E2E_TelegramDoesNotAutoSendSensitivePhoto`. See [report.md](report.md) Scope 4 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-012`.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior in the capture, Telegram, and cross-feature routing scope pass. **Phase:** plan (V-003 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: same Regression E2E coverage already cited above (`TestPhotosTelegram_E2E_UploadClassifySearchAndRetrieve`, `TestPhotosRouting_E2E_ReceiptRecipeDocumentCreateDownstreamArtifacts`, and `TestPhotosSensitivity_E2E_TelegramDoesNotAutoSendSensitivePhoto`) — this DoD line restates the existing scenario-specific regression DoD using the verbatim wording the state-transition-guard regex requires. See [report.md](report.md) Scope 4 evidence.
- [x] Consumer Impact Sweep is completed for every renamed or removed route, path, contract, identifier, and UI target in the capture, Telegram, and cross-feature routing scope, and zero stale first-party references remain. **Phase:** plan (V-004 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Consumer Impact Sweep section above lists the affected PWA navigation/breadcrumb/redirect targets, `/v1/photos/upload` and `/v1/photos/{id}/reveal` API client wrappers in PWA + Telegram, generated `RouteTarget` enum constants, Telegram deep links into the reveal flow, and stale-reference scan surfaces; the existing implement-phase DoD above already verifies that `RouteTarget` is the single source of truth (alphabetised, CHECK-constrained in migration 031, and that `grep -rn 'RouteTarget'` returns only the routing package, its test, and the migration), and that all sibling consumers receive photos by `artifact_id` reference rather than provider-specific fields — i.e. zero stale first-party references remain.
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** plan (V-006 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: the Change Boundary section above enumerates allowed file families (`internal/api/photos_upload*`, `internal/connector/photos/routing*`, `internal/connector/photos/sensitivity*`, `internal/telegram/` upload/retrieval handlers only, `web/pwa/src/photos/docscan/`, `web/pwa/tests/photos_docscan*`, sibling-feature integration shims under `internal/expense/` / `internal/recipe/` / `internal/knowledge/` / `internal/annotation/` / `internal/list/` / `internal/mealplan/` strictly limited to receiving `PhotoSummary` references, `tests/integration/photos_upload*` / `photos_docscan*` / `photos_sensitivity*`, `tests/e2e/photos_telegram*` / `photos_routing*` / `photos_sensitivity_retrieval*`) and excluded surfaces (Immich adapter mapping from Scope 2, Scope 3 lifecycle/dedupe/removal logic, Scope 5 capability matrix and stress fixtures, unrelated sibling-feature business logic, production Docker image lifecycle); the Scope 4 implement diff in [report.md](report.md) is contained within the allowed file families and zero excluded surfaces were changed.
...
- [x] Provider limitation is visible and non-mutating: when a rule or user action attempts an unsupported operation against a connected provider, the API returns 409 PROVIDER_LIMITATION with limitation_code and the PWA and Telegram surfaces show the same capability reason while search and classification continue to work (SCN-040-013). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the capability-limitation 409 envelope and the cross-surface reason DoD items — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-013 Then clause already covered by `tests/integration/photos_capability_test.go::TestPhotosCapability_UnsupportedOperationIs409AndNonMutating`, `tests/e2e/photos_capability_test.go::TestPhotosCapability_E2E_AlbumWriteBlockedWhileSearchWorks`, and `tests/integration/photos_capability_taxonomy_canary_test.go::TestPhotosCapabilityTaxonomyCanary_GoRegistryMatchesPWALimitationCodes`. See [report.md](report.md) Scope 5 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-013`.
- [x] Photo health and observability prove large-library readiness: running the full photo workflow on the disposable test stack with a synthetic 15,000-photo library makes health dashboards show progress, lag, confidence histogram, lifecycle distribution, duplicate count, capability limits, and skip counts, and stress validation reaches the configured ingest and search targets without touching user-owned libraries (SCN-040-015). **Phase:** plan (V-001 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: backing terminal-output evidence already cited above for the photo health aggregator and the synthetic 15k stress run — this DoD line is a Gherkin-fidelity reformulation of the SCN-040-015 Then clause already covered by `tests/integration/photos_health_test.go::TestPhotosHealth_ProgressMetricsAndCapabilityLimitsFromLiveAPI` and `tests/stress/photos_ingest_stress_test.go::TestPhotosIngestStress_Synthetic15000PhotoLibrarySearchableWithinTarget` (15,000 + 1,500 cross-provider duplicates ingested in 26.7 s, search p95 = 203 ms, all data namespaced under disposable `stress-040-*` connector IDs). See [report.md](report.md) Scope 5 evidence and [scenario-manifest.json](scenario-manifest.json) `SCN-040-015`.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior in the multi-provider capability governance and operations scope pass. **Phase:** plan (V-003 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: same Regression E2E coverage already cited above (`TestPhotosCapability_E2E_AlbumWriteBlockedWhileSearchWorks` and `TestPhotosSearch_E2E_CrossProviderUnifiedRanking`) plus the integration twin `TestPhotosHealth_ProgressMetricsAndCapabilityLimitsFromLiveAPI` and the stress run — this DoD line restates the existing scenario-specific regression DoD using the verbatim wording the state-transition-guard regex requires. See [report.md](report.md) Scope 5 evidence.
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** plan (V-006 closure pass, 2026-05-06). **Claim Source:** interpreted. Evidence: this scope is net-additive (a second provider adapter under `internal/connector/photos/adapters/photoprism/`, capability taxonomy registry under `internal/connector/photos/`, cross-provider rerank under `internal/api/photos_capability.go`, observability under `internal/metrics/photos.go`, PWA Photo Health dashboard under `web/pwa/`, and stress fixtures under `tests/stress/`); no excluded surface was touched. The Scope 5 implement diff in [report.md](report.md) is contained within these allowed file families.
