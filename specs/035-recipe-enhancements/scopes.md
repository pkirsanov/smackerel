...
- [x] Scenario SCN-035-002 (ParseQuantity normalizes Unicode fractions before parsing) — Unicode fraction normalization added to ParseQuantity — **Phase:** implement | **Evidence:** `internal/recipe/quantity.go` handles ½, ⅓, ⅔, ¼, ¾, ⅛; tested in `internal/recipe/quantity_test.go` (`TestParseQuantity_UnicodeFractions`)
