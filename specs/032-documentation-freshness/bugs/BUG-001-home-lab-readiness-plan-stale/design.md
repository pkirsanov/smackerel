# Design: BUG-001 Home-Lab Readiness Docs Belong Outside Product Repo

## Design Brief

This bug is a documentation repair for target-specific home-lab material that currently lives in the Smackerel product repo. The design is deliberately limited to documentation surfaces and planning traceability. It does not change runtime behavior, target adapters, CI workflows, compose files, generated config, or source code.

## Current Truth

| Finding | Current truth | Documentation correction |
|---------|---------------|--------------------------|
| D-001 | Target-specific home-lab adapter work and readiness checklists belong to the knb deploy-adapter overlay, not Smackerel product source. | Remove or migrate product-repo home-lab checklist content and point migration readers to knb spec `003-smackerel-home-lab-adapter-readiness`. |
...
1. Remove, retire, or migrate `docs/Home_Lab_Deployment_Plan.md` target-specific checklist content out of the Smackerel product repo.
2. Keep any replacement Smackerel text generic: product deployment contracts, not home-lab operator instructions.
3. Add the knb spec `003-smackerel-home-lab-adapter-readiness` cross-reference only as a migration pointer for target-specific readiness.
...
|---------|------|--------|-----------|
| T-DOC-001 | docs-static | Smackerel docs | Home-lab checklist content is absent from product docs or reduced to a migration pointer to knb spec `003-smackerel-home-lab-adapter-readiness`. |
