# Bug Scopes — BUG-002: Stale `deploy/self-hosted/` references and policy-violating Master Plan
...
Feature: Generic deploy-target documentation reflects the actual repo state
  Scenario: SCN-032-D06 — Stale `deploy/self-hosted/` paths removed from Deployment.md
    Given docs/Deployment.md historically referenced `deploy/self-hosted/apply.sh` and `deploy/<target>/manifest.yaml` as if they live in this repo
    And the self-hosted adapter was extracted to the knb overlay in commit 1b10dc23
    When an operator greps for `deploy/self-hosted/` in docs/Deployment.md
  ...
  Scenario: SCN-032-D08 — Self_Hosted_Master_Deployment_Plan.md reduced to migration-pointer stub
    Given docs/Self_Hosted_Master_Deployment_Plan.md was a 427-line operator-coupled multi-product plan
    And it contained real Linux user `selfhosted`, real Wi-Fi NIC `wlp195s0`, real BIOS specs, real subdomain pattern, and a `***REMOVED***` password marker
...
| File | Action |
|------|--------|
| [docs/Deployment.md](../../../../docs/Deployment.md) | Reword line 88, line 169, line 192 references away from `deploy/self-hosted/...` to either adapter-contract form or `deploy/_example/target-skeleton` |
| [docs/Self_Hosted_Master_Deployment_Plan.md](../../../../docs/Self_Hosted_Master_Deployment_Plan.md) | Replace 427-line file with a generic migration-pointer stub naming the knb overlay |
...
| `docs/Self_Hosted_Master_Deployment_Plan.md` (full-file replacement permitted) | `internal/`, `cmd/`, `ml/`, `tests/`, `cmd/core/` |
...
This scope replaces `docs/Self_Hosted_Master_Deployment_Plan.md` and reduces stale path references in `docs/Deployment.md`. The downstream consumer surface is enumerated below.

| Consumer surface | Impact | Action taken |
|------------------|--------|--------------|
| `docs/Self_Hosted_Deployment_Plan.md` (sibling docs file, BUG-001 migration-pointer stub) | Already references the knb overlay; consistency maintained by stubbing the Master Plan with the same migration target. | None — verified by `grep -n 'Self_Hosted_Master_Deployment_Plan' docs/Self_Hosted_Deployment_Plan.md` (zero matches; no breadcrumb to break). |
| `docs/Operations.md` | References `docs/Deployment.md` for production guide; reword preserves that contract. | None — verified by `grep -n 'Self_Hosted_Master_Deployment_Plan' docs/Operations.md` (zero matches). |
...
| Knb deploy-adapter overlay (sibling repo) | The knb overlay's self-hosted adapter is the new owner of the operator-coupled multi-product self-hosted plan; the migration-pointer stub correctly redirects readers there. | Adapter consumer-side update is owned by the knb overlay maintainer; outside this repo's surface per the deployment ownership boundary in `.github/copilot-instructions.md`. |
| Internal cross-spec references in `specs/` | `grep -rn 'Self_Hosted_Master_Deployment_Plan' specs/` returns zero matches (no spec referenced the master plan by file name). | None — zero stale first-party references remain. |
| Internal cross-spec references to `deploy/self-hosted/` | `grep -rn 'deploy/self-hosted' specs/` may return historical references in spec/design/report artifacts (legitimate historical references to extracted state); operator-facing reads are unaffected. | None — historical artifact references are immutable evidence and not consumer surfaces. |

### Test Plan

| ID | Test Type | Location | Scenario | Assertion |
|----|-----------|----------|----------|-----------|
| T-DOC-009 | docs-static | `docs/Deployment.md` | SCN-032-D06 | Zero matches for `\bdeploy/self-hosted/(apply\|manifest)` |
| T-DOC-010 | docs-static | `docs/Deployment.md` | SCN-032-D06 | Zero matches for `cp -R deploy/self-hosted` (replaced with `deploy/_example/target-skeleton`) |
| T-DOC-013 | docs-static | `docs/Self_Hosted_Master_Deployment_Plan.md` | SCN-032-D08 | Zero matches for `\bselfhosted\b` outside the stub-explanation context, `wlp195s0`, `\*\*\*REMOVED\*\*\*`, `Wi-Fi 7 \(MediaTek MT7925\)` |
| T-DOC-014 | docs-static | `docs/Self_Hosted_Master_Deployment_Plan.md` | SCN-032-D08 | Contains "knb deploy-adapter overlay" AND total line count `< 80` |
| T-DOC-R04 | regression-e2e (docs-static) | `docs/Self_Hosted_Deployment_Plan.md` + `docs/Deployment.md` | SCN-032-D06 / D08 | Regression E2E: scenario-first static-doc grep checks (T-DOC-009/010/013/014) are persistent — re-running the grep commands listed in report.md §"Validation Evidence" reproduces the pass condition for each scenario. Run on every workflow validate phase invocation. |
...
- [x] All stale `deploy/self-hosted/(apply\|manifest)` references in `docs/Deployment.md` are removed or reworded to adapter-contract form. [SCN-032-D06: Stale `deploy/self-hosted/` paths removed from Deployment.md]
   → Evidence: `grep -nE '\bdeploy/self-hosted/(apply\|manifest)' docs/Deployment.md` returns zero matches after the fix.
- [x] The "Adding a new deploy target" section in `docs/Deployment.md` names `deploy/_example/target-skeleton` as the copy-source. [SCN-032-D06]
   → Evidence: `grep -n 'deploy/_example/target-skeleton' docs/Deployment.md` shows the new copy-source path; `grep -n 'cp -R deploy/self-hosted' docs/Deployment.md` returns zero matches.
- [x] `docs/Self_Hosted_Master_Deployment_Plan.md` is reduced to a generic migration-pointer stub of less than 80 lines naming the knb deploy-adapter overlay. [SCN-032-D08: Self_Hosted_Master_Deployment_Plan.md reduced to migration-pointer stub]
   → Evidence: `wc -l docs/Self_Hosted_Master_Deployment_Plan.md` returns a value `< 80`; `grep -n 'knb deploy-adapter overlay' docs/Self_Hosted_Master_Deployment_Plan.md` returns the migration-pointer line.
- [x] Zero env-specific leak patterns remain in `docs/Self_Hosted_Master_Deployment_Plan.md`. [SCN-032-D08]
   → Evidence: `grep -nE '\bselfhosted\b|wlp195s0|\*\*\*REMOVED\*\*\*|Wi-Fi 7 \(MediaTek MT7925\)' docs/Self_Hosted_Master_Deployment_Plan.md` returns zero matches.
- [x] BUG-001 invariants are preserved.
   → Evidence: `wc -l docs/Self_Hosted_Deployment_Plan.md` still returns ≈60 lines; `grep -n '003-smackerel-self-hosted-adapter-readiness' docs/Self_Hosted_Deployment_Plan.md` returns the existing migration-pointer line; `grep -n 'Generic Pre-Apply Prerequisites\|Connector Live-Stack Evidence Caveat' docs/Deployment.md` returns both BUG-001 sections.
...
- [x] Consumer impact sweep complete and zero stale first-party references remain.
   → Evidence: see Consumer Impact Sweep section above; `grep -rn 'Self_Hosted_Master_Deployment_Plan' specs/ docs/Operations.md README.md` returns zero matches; the migration-pointer stub still resolves at the original path.
- [x] Change boundary respected: only the allowed surfaces above were modified; excluded surfaces are byte-identical to pre-bug HEAD.
   → Evidence: `git diff --name-only HEAD~1` lists only `docs/Deployment.md`, `docs/Self_Hosted_Master_Deployment_Plan.md`, `docs/Operations.md`, `specs/050-ml-sidecar-health-isolation/spec.md`, and bug-packet artifact files. Zero matches for any excluded surface.
...
Feature: Operator onboarding leads with the production-class flow and points at the knb overlay
  ...
  Scenario: SCN-032-D10 — docs/Deployment.md adds knb-overlay breadcrumb
    Given docs/Deployment.md correctly delegates the self-hosted adapter to the knb overlay
    ...
    Then a "knb Deploy-Adapter Overlay Dependency" subsection names spec `003-smackerel-self-hosted-adapter-readiness`
...
| [docs/Deployment.md](../../../../docs/Deployment.md) | Add a "knb Deploy-Adapter Overlay Dependency" subsection naming spec `003-smackerel-self-hosted-adapter-readiness` and the operator verification step |
...
| T-DOC-016 | docs-static | `docs/Deployment.md` | SCN-032-D10 | Contains "knb Deploy-Adapter Overlay Dependency" subsection AND names `003-smackerel-self-hosted-adapter-readiness` |
...
- [x] `docs/Deployment.md` has a "knb Deploy-Adapter Overlay Dependency" subsection naming `003-smackerel-self-hosted-adapter-readiness`. [SCN-032-D10: docs/Deployment.md adds knb-overlay breadcrumb]
   → Evidence: `grep -n 'knb Deploy-Adapter Overlay Dependency\|003-smackerel-self-hosted-adapter-readiness' docs/Deployment.md` returns the new subsection and named spec.
