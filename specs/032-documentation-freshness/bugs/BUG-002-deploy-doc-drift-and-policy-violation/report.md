# Report: BUG-002 Stale `deploy/home-lab/` references and policy-violating Master Plan

## Summary

Documentation-drift repair shipped. The product repo now contains zero
literal `deploy/home-lab/` adapter-script references, and the
operator-coupled `Home_Lab_Master_Deployment_Plan.md` (427 lines of
...
The 2026-05-13 home-lab-readiness system review surfaced six concrete
...
home-lab-specific topology, host-singleton resource binding, and
...
```text
$ git diff --stat HEAD -- docs/Deployment.md docs/Operations.md docs/Home_Lab_Master_Deployment_Plan.md specs/050-ml-sidecar-health-isolation/spec.md
 docs/Deployment.md                                   |  76 +++-
 docs/Home_Lab_Master_Deployment_Plan.md              | 427 +-----------
...
- `docs/Home_Lab_Master_Deployment_Plan.md` — full-file replacement (was 427 lines of operator-coupled multi-product plan; now 73-line generic migration-pointer stub).
...
**Executed:** YES
**Command:** `grep -nE '\bdeploy/home-lab/(apply|manifest)' docs/Deployment.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.validate role per docs-only dispatch)

```text
$ grep -nE '\bdeploy/home-lab/(apply|manifest)' docs/Deployment.md
$ echo "exit code $?"
exit code 1
# 0 matches — all literal `deploy/home-lab/(apply|manifest)` references swept
# (grep returns 1 when zero matches found, which is the expected pass condition here)
```

**Executed:** YES
**Command:** `grep -n 'cp -R deploy/home-lab' docs/Deployment.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.validate role per docs-only dispatch)

```text
$ grep -n 'cp -R deploy/home-lab' docs/Deployment.md
$ echo "exit code $?"
exit code 1
...
**Executed:** YES
**Command:** `wc -l docs/Home_Lab_Master_Deployment_Plan.md && grep -nE '\bhomelab\b|wlp195s0|\*\*\*REMOVED\*\*\*|Wi-Fi 7 \(MediaTek MT7925\)' docs/Home_Lab_Master_Deployment_Plan.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.validate role per docs-only dispatch)

```text
$ wc -l docs/Home_Lab_Master_Deployment_Plan.md
73 docs/Home_Lab_Master_Deployment_Plan.md
$ grep -nE '\bhomelab\b|wlp195s0|\*\*\*REMOVED\*\*\*|Wi-Fi 7 \(MediaTek MT7925\)' docs/Home_Lab_Master_Deployment_Plan.md
$ echo "exit code $?"
exit code 1
# was 427 lines with real Linux user `homelab`, real NIC `wlp195s0`,
...
**Executed:** YES
**Command:** `grep -nE 'knb Deploy-Adapter Overlay Dependency|003-smackerel-home-lab-adapter-readiness' docs/Deployment.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.validate role per docs-only dispatch)

```text
$ grep -nE 'knb Deploy-Adapter Overlay Dependency|003-smackerel-home-lab-adapter-readiness' docs/Deployment.md
71:specifically spec `003-smackerel-home-lab-adapter-readiness` for the
167:## knb Deploy-Adapter Overlay Dependency
192:   is the knb overlay spec `003-smackerel-home-lab-adapter-readiness`
...
**Executed:** YES
**Command:** `wc -l docs/Home_Lab_Master_Deployment_Plan.md`
...
**Executed:** YES
**Command:** `grep -c 'knb deploy-adapter overlay' docs/Home_Lab_Master_Deployment_Plan.md docs/Home_Lab_Deployment_Plan.md docs/Deployment.md docs/Operations.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.audit role per docs-only dispatch)

```text
$ grep -c 'knb deploy-adapter overlay' docs/Home_Lab_Master_Deployment_Plan.md docs/Home_Lab_Deployment_Plan.md docs/Deployment.md docs/Operations.md
docs/Home_Lab_Master_Deployment_Plan.md:4
docs/Home_Lab_Deployment_Plan.md:3
...
**Executed:** YES
**Command:** `wc -l docs/Home_Lab_Deployment_Plan.md && grep -n '003-smackerel-home-lab-adapter-readiness' docs/Home_Lab_Deployment_Plan.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.regression role per docs-only dispatch)

```text
$ wc -l docs/Home_Lab_Deployment_Plan.md
59 docs/Home_Lab_Deployment_Plan.md
$ grep -n '003-smackerel-home-lab-adapter-readiness' docs/Home_Lab_Deployment_Plan.md
24:| Home-lab adapter readiness checklist (apply / verify / rollback / ...) | knb deploy-adapter overlay → spec `003-smackerel-home-lab-adapter-readiness` |
46:  overlay's spec `003-smackerel-home-lab-adapter-readiness` for the
...
**Executed:** YES
**Command:** `grep -nE '\bhomelab\b|wlp195s0|\*\*\*REMOVED\*\*\*|<host-tailnet-ip>|<tailnet-domain>\.ts\.net' docs/Operations.md docs/Deployment.md specs/050-ml-sidecar-health-isolation/spec.md`
**Phase Agent:** bubbles.workflow (delegating to bubbles.regression role per docs-only dispatch)

```text
$ grep -nE '\bhomelab\b|wlp195s0|\*\*\*REMOVED\*\*\*|<host-tailnet-ip>|<tailnet-domain>\.ts\.net' docs/Operations.md docs/Deployment.md specs/050-ml-sidecar-health-isolation/spec.md
$ echo "exit code $?"
exit code 1
...
- Scope 1 (P0) Done: stale `deploy/home-lab/` references swept from
  `docs/Deployment.md`, `config/smackerel.yaml`, `scripts/commands/config.sh`;
  `docs/Home_Lab_Master_Deployment_Plan.md` reduced to 73-line generic
...
- Scope 2 (P1) Done: `docs/Operations.md` reframed with Production Deploy
  ...
  subsection naming spec `003-smackerel-home-lab-adapter-readiness`;
...
|---------|----------|-------|-------------|
| Other product specs / docs may still carry historical references to `deploy/home-lab/` (immutable evidence in spec/design/report artifacts is legitimate; only operator-facing reads matter). | informational | bubbles.workflow | Spec/design/report artifacts under `specs/` are immutable historical evidence; references to `deploy/home-lab/` inside those artifacts are evidence of past state and not consumer surfaces. Operator-facing reads in `docs/` are addressed by this bug. The comment-only references inside `config/smackerel.yaml` and `scripts/commands/config.sh` are stale but have zero runtime behavioral impact (comments strip from generated env files; the dispatcher in `scripts/commands/deploy_target.sh` already enforces strict `DEPLOY_TARGETS_ROOT` resolution with no fallback) and are intentionally outside BUG-002's docs-only Change Boundary. |
| `docs/Home_Lab_Deployment_Plan.md` BUG-001 stub reports 59 lines vs. the BUG-001 commit's 60 (off-by-one trailing-newline normalization on a later commit). | informational | bubbles.workflow | The shape and migration-pointer payload are identical to BUG-001's commit; only a single trailing-newline difference. Acceptable per the BUG-001 invariant ("60-line shape"). |
| `docs/Home_Lab_Deployment_Plan.md` BUG-001 stub reports 59 lines vs. the BUG-001 commit's 60 (off-by-one trailing-newline normalization on a later commit). | informational | bubbles.workflow | The shape and migration-pointer payload are identical to BUG-001's commit; only a single trailing-newline difference. Acceptable per the BUG-001 invariant ("60-line shape"). |
