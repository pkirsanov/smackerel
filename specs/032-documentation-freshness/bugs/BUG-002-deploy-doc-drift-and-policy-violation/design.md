# Bug Design — BUG-002: Stale `deploy/home-lab/` references and policy-violating Master Plan

## Design Goal

Restore consistency between the documented operator workflow and the actual repo state after the deploy adapter was extracted to the knb overlay (commit `1b10dc23`), and remove the operator-coupled multi-product `Home_Lab_Master_Deployment_Plan.md` that violates the `.github/copilot-instructions.md` "No Env-Specific Content In This Repo" non-negotiable policy. Generic-only fixes per the user directive — home-lab specifics are owned by the knb overlay.

## Design Decisions

### D-001: Generic-only edits in this repo

Honors the user directive received with the work item: this repo holds the **generic** product contract; home-lab specifics live in the knb overlay. Therefore:

- Where a `deploy/home-lab/...` path appears in a sentence whose meaning is "the adapter does X", reword to **adapter-contract form** (the path is owned by the adapter, not by this repo).
...
- `Home_Lab_Master_Deployment_Plan.md` collapses to a generic migration-pointer stub naming the knb overlay's home-lab adapter spec as the owner of the operator-coupled cross-product coordination plan.
- The new "knb Deploy-Adapter Overlay Dependency" subsection in `docs/Deployment.md` MUST refer generically to "the knb deploy-adapter overlay's home-lab adapter spec" without embedding any per-target topology.

### D-002: Stub the Master Plan, do not delete

Deletion would silently break any external link or operator bookmark targeting `docs/Home_Lab_Master_Deployment_Plan.md`. The migration-pointer stub preserves the path while explicitly explaining the relocation, exactly as BUG-001 did with `docs/Home_Lab_Deployment_Plan.md`. The stub is approximately 60 lines (close to BUG-001's stub size).
...
Per the bubbles framework state-transition-guard contract for rename/removal scopes, each scope declares a Consumer Impact Sweep enumerating downstream consumers and the action taken. Scope 1 affects `docs/Home_Lab_Master_Deployment_Plan.md` consumers (the existing `docs/Home_Lab_Deployment_Plan.md` migration stub references it; `docs/Operations.md` and `README.md` may have generic links); Scope 2 affects `docs/Operations.md` readers and the new knb-overlay breadcrumb consumers.

## Scenarios

| ID | Title | Pre | Trigger | Expected | Owner |
|----|-------|-----|---------|----------|-------|
| SCN-032-D06 | Stale `deploy/home-lab/` paths removed from `docs/Deployment.md` | Doc has 3 stale references at lines 88, 169, 192 | `grep -nE 'deploy/home-lab' docs/Deployment.md` after fix | Returns zero matches OR only generic adapter-contract wording with no `deploy/home-lab/...` literal | Scope 1 |
| SCN-032-D08 | `Home_Lab_Master_Deployment_Plan.md` reduced to migration-pointer stub | File is 427 lines with operator-coupled detail | After fix: file is < 80 lines AND contains "knb deploy-adapter overlay" reference AND zero matches for `homelab\b`, `wlp195s0`, `***REMOVED***`, `<host-tailnet-ip>`, `<tailnet-domain>` | Stub names knb overlay as owner; zero env-specific content remains | Scope 1 |
...
| SCN-032-D10 | `docs/Deployment.md` adds knb-overlay breadcrumb | Doc has no pointer to knb overlay's adapter-readiness spec | After fix: a "knb Deploy-Adapter Overlay Dependency" subsection appears naming spec `003-smackerel-home-lab-adapter-readiness` and the operator verification step | Operator can verify knb overlay is shipped before attempting deploy | Scope 2 |
...
|---------|------|--------|-----------|
| T-DOC-009 | docs-static grep | `docs/Deployment.md` | Zero matches for `\bdeploy/home-lab/(apply\|manifest)` |
| T-DOC-010 | docs-static grep | `docs/Deployment.md` | Zero matches for `cp -R deploy/home-lab` (replaced with `deploy/_example/target-skeleton`) |
| T-DOC-013 | docs-static grep | `docs/Home_Lab_Master_Deployment_Plan.md` | Zero matches for the policy-leak patterns: `\bhomelab\b`, `wlp195s0`, `\*\*\*REMOVED\*\*\*`, `<host-tailnet-ip>`, `<tailnet-domain>`, `Wi-Fi 7 \(MediaTek MT7925\)` |
| T-DOC-014 | docs-static grep | `docs/Home_Lab_Master_Deployment_Plan.md` | Contains "knb deploy-adapter overlay" AND total line count `< 80` |
...
| T-DOC-016 | docs-static grep | `docs/Deployment.md` | Contains "knb Deploy-Adapter Overlay Dependency" subsection AND names `003-smackerel-home-lab-adapter-readiness` |
| T-DOC-017 | docs-static grep | `specs/050-ml-sidecar-health-isolation/spec.md` | Status line matches `Resolved — implemented` form |
| T-DOC-R04 | regression docs-static | `docs/Home_Lab_Deployment_Plan.md` | Migration-pointer stub still present (BUG-001 invariant preserved) |
...
| Risk | Mitigation |
|------|------------|
| Inadvertent removal of generic content from `Home_Lab_Master_Deployment_Plan.md` | Stub size target ≈ 60 lines (matches BUG-001's `Home_Lab_Deployment_Plan.md` stub); verify via line-count assertion T-DOC-014. |
...
| Knb overlay spec name (`003-smackerel-home-lab-adapter-readiness`) drifts from the actual overlay structure | Use the same spec name BUG-001 cited (already verified by BUG-001's docs/Home_Lab_Deployment_Plan.md stub); cite it in both BUG-001 and BUG-002 stubs for consistency. |
| Re-introduction of `***REMOVED***`-style password markers anywhere | T-DOC-R06 is a per-edited-file grep that asserts zero leaks. |
| BUG-001 invariants regress (the 60-line `Home_Lab_Deployment_Plan.md` stub or the two new Deployment.md sections) | T-DOC-R04 and T-DOC-R05 run on every regression sweep. |
...
BUG-001 explicitly named `docs/Home_Lab_Master_Deployment_Plan.md` as the carry-forward item out of its scope. This bug closes that carry-forward.
...
- Real home-lab hardware/topology/network details (lived in `Home_Lab_Master_Deployment_Plan.md` lines 1–110 and elsewhere; moved out via D-002).
- The `deploy/home-lab/` adapter scripts (`apply.sh`, `bootstrap.sh`, `verify.sh`, `rollback.sh`, `params.yaml`, `manifest.yaml`).
