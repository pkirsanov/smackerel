# Bug: BUG-002 — Stale `deploy/home-lab/` references and policy-violating Master Plan after adapter extraction

## Classification

- **Type:** Documentation drift after deploy adapter extraction (BUG-001 carry-forward + system-review carry-forward)
- **Severity:** HIGH — operator following the documented workflow for "Adding a new deploy target" hits `cp -R deploy/home-lab` and fails at step 1 (the source directory was extracted to the knb deploy-adapter overlay in commit `1b10dc23`); plus a 427-line operator-coupled file (`docs/Home_Lab_Master_Deployment_Plan.md`) violates the `.github/copilot-instructions.md` "No Env-Specific Content In This Repo" non-negotiable policy with real Linux user, real Wi-Fi NIC name, real BIOS specs, real backup paths, real subdomain pattern, and a weak-password marker line.
...
`deploy/home-lab/` was deliberately extracted from this repo to the knb deploy-adapter overlay in commit `1b10dc23` ("deploy-target: STRICT DEPLOY_TARGETS_ROOT, extract home-lab to private repo"). The strict `DEPLOY_TARGETS_ROOT` resolution rule landed in `scripts/commands/deploy_target.sh`, but four references to the extracted path remained in operator-facing surfaces:
...
| [docs/Deployment.md](../../../../docs/Deployment.md) line 192 | `1. \`cp -R deploy/home-lab deploy/<new-target>\`` (the source directory does NOT exist in this repo) |
| [config/smackerel.yaml](../../../../config/smackerel.yaml) line 1031 | `# Self-hosted target on the operator's home-lab machine. Builds are produced by .github/workflows/build.yml and applied via deploy/home-lab/apply.sh.` |
...
### Problem B — `Home_Lab_Master_Deployment_Plan.md` policy violation

`docs/Home_Lab_Master_Deployment_Plan.md` (427 lines) is a **multi-product cross-coordination plan** (covers `qf`, `s`, `gh`, `wa`) that has no business in a single-product repo, AND violates the `.github/copilot-instructions.md` "No Env-Specific Content In This Repo" non-negotiable policy:

| Class of leak | Example from the file |
|---------------|----------------------|
| Real Linux username | `User \| \`homelab\`` (line 42); `Sudo \| NOPASSWD via /etc/sudoers.d/homelab` (line 43); home-directory backup paths under the user's `$HOME` (lines 135–138, 188, 294) |
...
| Real subdomain naming pattern | `*.home-lab.<tailnet-domain>.ts.net` (lines 228, 233–242) |
| User group memberships | `homelab in docker, render, video` (line 156) |
| Real per-host install paths | per-host install path under the operator-user `$HOME` (line 188), e.g. `<HOME>/immich/library` |

This file's existence in a generic, public-facing product repo is the same class of issue BUG-001 fixed for `Home_Lab_Deployment_Plan.md`. The BUG-001 commit message explicitly named this as a carry-forward.
...
The repo correctly delegates the home-lab adapter to the knb overlay (`deploy/README.md` "Adapter Locality", BUG-001 migration-pointer stub), but offers no breadcrumb saying *what* the operator needs from the knb overlay (which spec, which adapter scripts, what status). Without this, operators can't verify whether the knb overlay's `003-smackerel-home-lab-adapter-readiness` is shipped before attempting a deploy.
...
> "Generic deployment should be done in `s` repo, and any home lab specific settings should be configured using custom deployment in `knb` repo."

This bug:

- MUST replace stale `deploy/home-lab/` references with the **adapter-contract** wording (the path is owned by the adapter, not by this repo).
- MUST reduce `Home_Lab_Master_Deployment_Plan.md` to a migration-pointer stub (the same pattern BUG-001 applied to `Home_Lab_Deployment_Plan.md`). The detailed multi-product home-lab plan content moves to the knb overlay.
...
- MUST NOT touch any home-lab adapter implementation (those scripts live in the knb overlay; this repo no longer ships them).

## Behavior Contract

**Pre-fix (defect):**

- Operator following `docs/Deployment.md` "Adding a new deploy target" tries `cp -R deploy/home-lab deploy/<new-target>` and the command fails because the source directory does not exist in this repo.
- Operator reading `config/smackerel.yaml::environments.home-lab` comment is told builds are "applied via `deploy/home-lab/apply.sh`" — a path that does not exist in this repo.
- Operator browsing `docs/Home_Lab_Master_Deployment_Plan.md` sees real Linux users, real Wi-Fi adapter names, real backup paths, real subdomain patterns, and a `***REMOVED***` weak-password marker — a non-negotiable policy violation per `.github/copilot-instructions.md`.
...
- Operator preparing to deploy to home-lab has no breadcrumb telling them which knb-overlay spec must be ready.

**Post-fix (required behavior):**

- All five stale `deploy/home-lab/` references in `docs/Deployment.md`, `config/smackerel.yaml`, and `scripts/commands/config.sh` either reword to the adapter-contract form (the path is owned by the adapter), or replace `deploy/home-lab` with `deploy/_example/target-skeleton` where a copy-source path is meaningful in-tree.
- `docs/Home_Lab_Master_Deployment_Plan.md` reduces to a generic migration-pointer stub naming the knb deploy-adapter overlay's home-lab adapter spec as the owner of the operator-coupled cross-product coordination plan. Zero real Linux users, NIC names, BIOS specs, backup paths, subdomain patterns, password markers, or operator-coupled topology remain in the file.
...
- `docs/Deployment.md` adds a "knb Deploy-Adapter Overlay Dependency" subsection naming the adapter-readiness spec operators need from the knb overlay before attempting a home-lab deploy.

## Scope Of Fix

Two scopes — see [scopes.md](scopes.md):

1. **Scope 1**: Sweep stale `deploy/home-lab/` references out of `docs/Deployment.md`, `config/smackerel.yaml`, and `scripts/commands/config.sh`; replace `Home_Lab_Master_Deployment_Plan.md` with a migration-pointer stub.
...
- [docs/Home_Lab_Master_Deployment_Plan.md](../../../../docs/Home_Lab_Master_Deployment_Plan.md) (427 lines, target of Scope 1)
- [docs/Home_Lab_Deployment_Plan.md](../../../../docs/Home_Lab_Deployment_Plan.md) (60-line migration-pointer stub, BUG-001 reference shape)
...
- Sibling product repo: knb deploy-adapter overlay, spec `003-smackerel-home-lab-adapter-readiness`
