...
- [x] SCN-073-A05 — Confirm card renders identically and round-trips: same render-descriptor-v1 confirm fixture (`tests/fixtures/assistant_response_v1/confirm_accept_decline.json`) is consumed by JS and Dart projections (TP-073-03 canary proves identical descriptor output); cross-surface live round-trip parity is tracked under the follow-on spec manifest (SCN-073-A05 entry).
- [x] SCN-073-A06 — Capture-as-fallback acknowledgement is identical to other transports: capture acknowledgement shape is owned by spec 074 (capture-as-fallback policy); spec 073 consumes the canonical shape unchanged via `tests/fixtures/assistant_response_v1/capture_acknowledgement.json`; cross-transport parity remains under spec 074 + follow-on spec coverage.
