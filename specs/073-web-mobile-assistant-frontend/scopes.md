...
- **Mobile test-runner boundary (gaps-to-doc Round 16 clarification — 2026-06-06).** The Dart-side standalone tests under `clients/mobile/assistant/test/` (codegen drift, platform declaration, no-branch guard, core storage guard, fail-loud config, renderer canary, render-descriptor static check) run via `flutter test ./clients/mobile/assistant/`, NOT via `./smackerel.sh test unit`. The repo CLI does not wire `flutter` / `dart` because mobile packaging, on-device VoiceOver/TalkBack runs, and mobile-CLI tooling were rescoped to the follow-on mobile-delivery spec (see Rescope Decision). The Go cross-language canary at `tests/unit/clients/render_descriptor_canary_test.go` (TP-073-03) is the executable repo-CLI gate that enforces Dart shared-core behavior: it spawns `dart` (AOT-compiled per TestMain) on every fixture and deep-equals js==golden, dart==golden, js==dart. Future regressions to the Dart shared-core projection therefore still fail `./smackerel.sh test unit`. Wiring `flutter test` into the repo CLI belongs to the follow-on mobile-delivery spec alongside iPhone/iOS + Android packaging; tracking this gap here as a planning note so the test-plan Command column reads correctly: rows whose primary executable evidence is a Dart-only file should be read as "TP-073-03 cross-language canary enforces shared-core behavior under `./smackerel.sh test unit`; standalone Dart test is supplementary scaffolding executed via `flutter test`."
