...
unified surfacing controller delivered by **spec 078
(cross-surface-surfacing-prioritizer)**. It applies only to Scope 9 "Surfacing
Controller Integration". The controller shipped, so the gate is **SATISFIED**
(2026-06-23); the "spec 021 M1a" naming in earlier drafts is superseded — the
milestone was rescoped from spec 021 into spec 078 by commit `640b95d0`.

The delivered controller is a **synchronous call interface**, NOT an event bus.
There is no `SurfacingProposal` event, no `SurfacingProposalSink`, and no
`AcknowledgmentBus`. Producers call
`surfacing.Controller.Propose(ctx, SurfacingCandidate) (SurfacingDecision, error)`
and dispatch only on a permit verdict. The reference integration already on
trunk is `internal/scheduler/jobs.go` `proposeSurfacing`.

### Producer/Arbiter Split

- **Producer (spec 054):** `internal/notification/service.go` continues to
  classify, enrich, dedupe, correlate, and decide. Its terminal dispatch step
  changes from "directly queue a `DeliveryAttempt`" to "build a
  `SurfacingCandidate`, call `Controller.Propose`, and queue output only on a
  permit verdict".
- **Arbiter (spec 078):** the unified surfacing controller is consulted
  synchronously by notification, the scheduler's intelligence producers, digest,
  and any future producers. A single shared instance per process owns the GLOBAL
  per-day nudge budget, the cross-channel dedupe index, and the
  acknowledgment-driven suppression window.

### SurfacingCandidate (what spec 054 populates)

`SurfacingCandidate` and `SurfacingDecision` are owned by spec 078; spec 054
does NOT define or fork them. Spec 054 populates the candidate fields:

| Field | Source | Notes |
|-------|--------|-------|
| `Producer` | constant `ProducerNotification` | identifies spec 054 as origin (additive enum constant — see Change Boundary) |
| `Channel` | decision/output mapping | bounded `surfacing.Channel` (`telegram \| web_push \| ntfy \| email_out \| digest`) |
| `ContentKey` | incident correlation key (`incident.IncidentKey`) | cross-channel dedupe + ack-suppression identity within an incident |
| `Priority` | severity-derived | `1=high, 2=medium, 3=low` (mirrors `intelligence.Alert`) |
| `TimeCritical` | decision urgency | `true` only for urgent decisions; `Priority 1 + TimeCritical` may escalate past an exhausted budget |
| `ProposedAt` | `now` | proposal timestamp |

The controller returns `SurfacingDecision{Kind, Reason}` where `Kind` is one of
`permit`, `escalated`, `deduped`, `suppressed`, or `deferred-budget-exhausted`.
Spec 054 persists this verdict against the decision/delivery record for audit.

### Contract Edges

1. **Propose edge:** the decision engine calls `Controller.Propose` at its
   dispatch step. Spec 054 production code MUST NOT queue a delivery that
   bypasses the controller when a controller is wired. A nil controller is the
   legacy direct-dispatch fallback (mirrors scheduler `proposeSurfacing`).
2. **Arbitration edge:** the synchronous `SurfacingDecision` verdict
   (`permit | escalated | deduped | suppressed | deferred-budget-exhausted`) is
   persisted against the decision row for audit.
3. **Acknowledgment edge (pull, not push):** there is no acknowledgment bus.
   When an operator acknowledges an incident, spec 054 records it on the shared
   `surfacing.InMemoryAck` via `Acknowledge(correlationKey)`. The controller's
   `SuppressionWindow` then suppresses any same-`ContentKey` follow-up within
   the configured window, regardless of producer or channel; simultaneous
   cross-channel siblings collapse via the dedupe index. This is the spec-054
   wiring half of SCN-054-030; the suppression mechanism itself is spec 078's.
4. **Urgent bypass edge:** `Priority 1 + TimeCritical` is the only producer-side
   signal the controller honors for soft-budget bypass (when SST
   `surfacing.urgent_escalation_enabled` is true). Hard safety stops (loop
   guard, destructive refusal) remain spec 054's responsibility BEFORE a
   candidate is proposed.

### Dependency Status

The Scope 9 dependency is **SATISFIED** (2026-06-23):
- The spec 078 controller is on trunk at `internal/intelligence/surfacing/` and
  exposes the stable `Controller.Propose` seam plus the `AckLookup` /
  `InMemoryAck` / `SuppressionWindow` acknowledgment surface.
- The cross-surface arbitration (shared global budget + cross-channel dedupe) is
  shipped and already consumed by the scheduler producers.
- The shared e2e harness (`tests/e2e/surfacing_budget_test.go`) exercises the
  controller and can be reused by spec 054 e2e tests.

The one outstanding 054-side gap is the production acknowledgment feed: nothing
calls `Acknowledge` outside tests today, so Scope 9 must thread a shared ack
registry and wire the notification incident-ack path into it.

### Rollback Path

Rollback is the nil-controller fallback: if the shared controller is not wired
into the notification service, the engine's existing direct-dispatch path
remains available (mirrors scheduler `proposeSurfacing`). No new feature-gate
SST key is introduced; the existing `surfacing.*` SST keys
(`daily_nudge_budget`, `suppression_window_hours`, `dedupe_window_hours`,
`urgent_escalation_enabled`) govern the controller and already fail loud in
`NewController` per Smackerel SST policy: a missing value aborts startup; there
