...
The CI workflow `.github/workflows/build.yml` builds three configuration bundles per source SHA (`dev`, `test`, `self-hosted`) via `./smackerel.sh config generate --env <env> --bundle ...` so the Build-Once Deploy-Many invariant (bubbles G074) holds. Spec 051's `FR-051-005` correctly added a defense-in-depth gate at the SST loader (`scripts/commands/config.sh`) that REJECTS dev-default Postgres passwords for non-dev SST targets. The two policies collide for the `self-hosted` matrix leg: CI has no real production secret to inject, so `infrastructure.postgres.password=smackerel` (the local-dev value in `config/smackerel.yaml`) flows through the loader, the gate fires, and the `build-bundles (self-hosted)` job exits 1.
...
2. **Spec 051 hard constraint:** "Target-specific secret injection remains a target adapter responsibility". Also correct — the adapter (`<deployment-owner>/<product>/<target>/apply.sh`) decrypts `knb/smackerel/secrets/self-hosted.enc.env` via `sops` and supplies it as the SECOND `--env-file` to `docker compose up`, which overrides anything in the bundle's `app.env`.
...
(1) and (2) are inconsistent at the moment when the self-hosted bundle is FIRST GENERATED in CI — the SST loader fails before any bundle bytes exist, so (3) cannot complete either. The bundle's value is also semantically meaningless at apply time because the adapter overrides it; we are blocking on a value that is going to be discarded.
...
**Intent:** Define a single, machine-checkable contract for how SST-managed secret values cross the CI → bundle → adapter → runtime boundary, such that (a) CI can produce a deterministic per-env bundle for `self-hosted` (and any future production-class target) without any real production secret being known to CI, (b) the bundle published to OCI never contains a real production secret value, (c) the deploy adapter is REQUIRED to inject real secrets at apply time and FAILS LOUD if any required secret is missing or unsubstituted, and (d) spec 051's existing dev-default rejection continues to fire for actual local-dev attempts.

**Success Signal:** Pushing to `main` causes the entire CI matrix (`dev`, `test`, `self-hosted`) to go green and `publish-build-manifest` to write a `build-manifest-<sourceSha>.yaml` containing all three `configBundles[*]` entries with valid sha256 digests. An auditor can pull the published `self-hosted` bundle from `ghcr.io/pkirsanov/smackerel-config-bundles:self-hosted-<sourceSha>`, extract `app.env`, and observe that NO key declared as a "secret" in the SST manifest contains a real value. An operator can run `bash <deployment-owner>/<product>/<target>/apply.sh ...` against that bundle and the resulting containers boot with the real secrets sourced from `knb/smackerel/secrets/self-hosted.enc.env` (via `sops`). A regression that removes the placeholder substitution OR reintroduces a real value into the published bundle is caught by an adversarial test that fails the CI run BEFORE the bundle is pushed to the registry.

**Hard Constraints:**

- The Build-Once Deploy-Many invariant (bubbles G074) MUST be preserved end-to-end. The CI matrix `[ dev, test, self-hosted ]` MUST continue to build all three bundles deterministically and publish all three to OCI.
...
- Target-specific secret injection MUST remain an adapter (`<deployment-owner>/<product>/<target>/apply.sh`) responsibility. This spec MUST NOT add a real-secret-emission path to the smackerel CI workflow.
...
1. The published `self-hosted` config bundle contains a real production secret value.
2. The published `self-hosted` config bundle contains the literal local-dev Postgres password `smackerel`, or any other value from `internal/config/secrets.go::DevDBPasswords`.
3. The deploy adapter `<deployment-owner>/<product>/<target>/apply.sh` runs `docker compose up` against an unsubstituted bundle and Smackerel boots with a placeholder value as a real credential.
...
> Job 76061941351 on run 25881130287 (head_sha b14742c4) failed at step "Generate config bundle for self-hosted" with exit 1. Local reproduction:
> ```
> $ TARGET_ENV=self-hosted ./smackerel.sh config generate --env self-hosted --bundle
> ERROR: infrastructure.postgres.password is a known dev-default value —
> refusing to generate config for TARGET_ENV=self-hosted (spec 051 FR-051-005).
> ```
> Resolution requires an operator decision (define `CI_BUNDLE_POSTGRES_PASSWORD` GitHub Actions secret + env override on bundle-generate step, OR drop self-hosted from the CI matrix and have the deploy adapter generate the bundle on the target). This is squarely a spec 051 follow-up and does not block spec 047 R13 close-out.
...
| Deploy Operator | Runs `bash <deployment-owner>/<product>/<target>/apply.sh ...` from a workstation OR target host with the operator age key. Owns the encrypted secrets at rest in `knb/smackerel/secrets/self-hosted.enc.env`. | Apply a CI-published bundle to the self-hosted target with real secrets injected. Be able to rotate any secret without rebuilding bundles. | Decrypt `knb/smackerel/secrets/self-hosted.enc.env` via `sops`, run adapter scripts, restart containers. |
...
### UC-052-001: CI builds a deterministic self-hosted bundle without operator-provided secrets
...
- **Main Flow:**
  1. The `build-bundles` matrix leg for `env=self-hosted` checks out the source at the resolved `sourceSha`.
  2. The leg invokes `./smackerel.sh config generate --env self-hosted --bundle --output-dir dist/config-bundles/`.
  3. The SST loader resolves every key in `config/smackerel.yaml`. For every key declared as a "secret" in the SST secret-key manifest AND for which `TARGET_ENV` is `self-hosted`, the loader emits a placeholder marker (e.g., `__SECRET_PLACEHOLDER__POSTGRES_PASSWORD__`) in `app.env` instead of the literal yaml value.
  4. The loader validates that NO real secret value appears in the staged bundle.
  5. The bundle is packaged into `config-bundle-self-hosted-<sourceSha>.tar.gz` deterministically.
...
- **Postconditions:** `dist/config-bundles/config-bundle-self-hosted-<sourceSha>.tar.gz` exists and contains `app.env` with placeholder markers in place of every secret value. The bundle's sha256 matches a second invocation of the same command (determinism).

### UC-052-002: Adapter applies the self-hosted bundle with real secrets injected at apply time

- **Actor:** Deploy Operator
- **Preconditions:** A CI run for `<sourceSha>` has published `ghcr.io/pkirsanov/smackerel-config-bundles:self-hosted-<sourceSha>` and a `build-manifest-<sourceSha>.yaml` declaring the bundle's expected sha256. The operator has the age private key. `knb/smackerel/secrets/self-hosted.enc.env` exists, is sops-encrypted to the operator age key, and contains a real value for every key declared as a "secret" in the SST secret-key manifest.
- **Main Flow:**
  1. Operator runs `bash <deployment-owner>/<product>/<target>/apply.sh --image-core=sha256:... --image-ml=sha256:... --config-bundle=self-hosted-<sourceSha> --source-sha=<sourceSha>`.
  ...
  4. Adapter decrypts `knb/smackerel/secrets/self-hosted.enc.env` via `sops` into a `chmod 0600` tmpfile.
...
- **Main Flow:**
  1. Operator decrypts `knb/smackerel/secrets/self-hosted.enc.env` via `sops`.
...
- **Main Flow:**
  1. Auditor pulls the bundle by digest from `ghcr.io/pkirsanov/smackerel-config-bundles:self-hosted-<sourceSha>`.
...
```gherkin
Scenario: BS-052-001 self-hosted bundle generation succeeds in CI without operator-provided secrets
  Given the SST secret-key manifest declares POSTGRES_PASSWORD as a secret
  And TARGET_ENV=self-hosted
  And no real Postgres password is set in the CI environment
  When the CI build-bundles matrix leg for env=self-hosted runs ./smackerel.sh config generate --env self-hosted --bundle
  Then the loader exits 0
  And dist/config-bundles/config-bundle-self-hosted-<sourceSha>.tar.gz exists
...
Scenario: BS-052-002 published self-hosted bundle does NOT contain a real production password
  Given a CI run has published config-bundle-self-hosted-<sourceSha>.tar.gz to OCI
...
Scenario: BS-052-003 Adapter substitutes all placeholder secrets before docker compose up
  Given a published self-hosted bundle exists for sourceSha S
  And the operator has knb/smackerel/secrets/self-hosted.enc.env with real values for every secret key declared in the bundle's SST secret-key manifest
  When the operator runs `bash <deployment-owner>/<product>/<target>/apply.sh --image-core=... --image-ml=... --config-bundle=self-hosted-S --source-sha=S`
...
Scenario: BS-052-004 Adapter fails loud if any placeholder remains in app.env at apply time
  Given a published self-hosted bundle exists
...
Scenario: BS-052-005 A regression that removes placeholder substitution is caught by an adversarial test
  Given the SST loader has a bug that causes it to emit the literal yaml value for POSTGRES_PASSWORD on self-hosted
  When the contract test internal/deploy/bundle_secret_contract_test.go runs ./smackerel.sh config generate --env self-hosted --bundle and inspects the bundle's app.env
...
Scenario: BS-052-006 spec 051 dev-default DB password rejection continues to fire for actual local-dev attempts
  ...
  And TARGET_ENV=self-hosted
  And the operator is running ./smackerel.sh config generate --env self-hosted --bundle locally (NOT in CI)
...
Scenario: BS-052-007 Operator rotates a secret without rebuilding the bundle
  Given Smackerel has been deployed with config-bundle=self-hosted-S
  When the operator updates POSTGRES_PASSWORD in knb/smackerel/secrets/self-hosted.enc.env
  And the operator re-runs apply.sh with the same --config-bundle=self-hosted-S argument
...
Scenario: BS-052-008 A new secret added by a future spec automatically routes through the contract
  ...
  When CI runs `./smackerel.sh config generate --env self-hosted --bundle`
...
| Operator's secrets file shipped on wrong target | Operator runs adapter on `dev` host with `self-hosted` bundle (or vice versa) | Adapter MUST verify the bundle's environment label matches the secrets file's environment label. Mismatch = fail loud. |
...
| Dev/test bundles regressing to use placeholders | A naive implementation rewrites all bundles to use placeholders, breaking local dev | Spec 052 MUST scope the placeholder behavior to `TARGET_ENV ∈ { non-dev, non-test }` (currently just `self-hosted`; future production-class targets opt in explicitly). dev/test bundles MUST continue to ship inline values for local dev convenience. |
...
|-------------|------|----------|-----------|
| Unblock CI green for `self-hosted` matrix leg | Table Stakes | P0 | Currently blocking every push to main. Until this is fixed, the spec 047 vulnerability gate, the spec 049 monitoring stack, and every future CI-validated artifact ship under a partial-failure regime. |
...
1. **Immediate (this work cycle):** Adopt the SST secret-key manifest + placeholder substitution pattern (IP-052-001). Land the adversarial bundle-inspection test in the same change set. Update `<deployment-owner>/<product>/<target>/apply.sh` to perform the substitution and the post-substitution scan.
...
- **Sketch:**
  ...
  - SST loader change: when `TARGET_ENV ∈ { self-hosted, ...future production-class targets }` AND a key is in the secret-key manifest, emit `__SECRET_PLACEHOLDER__<KEY>__` instead of the literal yaml value.
  ...
  - Adapter change: `<deployment-owner>/<product>/<target>/apply.sh` reads `secret-keys.yaml` from the extracted bundle, validates that every declared key has a placeholder in `app.env` AND a real value in the decrypted secrets file, then performs the existing two-`--env-file` `docker compose up` (no change to that step).
  ...
  - Contract test: `internal/deploy/bundle_secret_contract_test.go` runs `./smackerel.sh config generate --env self-hosted --bundle` and asserts every key in the secret-key manifest appears as a placeholder in the bundle's `app.env` and zero secret-key values from `internal/config/secrets.go::DevDBPasswords` appear anywhere.
...
### IP-052-003: Drop self-hosted from the CI matrix; adapter generates bundle at apply time ⚠ Compromises Build-Once Deploy-Many

- **Impact:** Medium (unblocks CI green) / Negative (compromises Build-Once Deploy-Many guarantee)
- **Effort:** M (≈ 2 scopes: remove self-hosted from matrix, restructure adapter to call `./smackerel.sh config generate` on target with full smackerel CLI installed)
- **Competitive Advantage:** None — moves bundle generation OFF the trust boundary that bubbles G074 was designed to enforce.
- **Actors Affected:** Release Engineer (smaller CI matrix), Deploy Operator (now needs full smackerel CLI on target host), Auditor (cannot verify a published bundle for self-hosted because none exists).
...
- **Recommendation:** Reject. Listed for completeness because it appears in `F-047-B`'s "OR drop self-hosted from the CI matrix" alternative; spec 052 explicitly recommends against it.
...
- **Performance:** Adapter substitution + post-substitution scan SHOULD complete in under 5 seconds on the self-hosted target host (a small Ed25519 / sops decrypt + a sub-second grep over `app.env` + a sub-second startup of `docker compose up`).
- **Security:**
  - Zero real secret values in any committed file (smackerel repo OR knb repo plaintext OR any CI workflow file). Real values exist only in `knb/smackerel/secrets/self-hosted.enc.env` (sops-encrypted to operator age key) and as decrypted values in a `chmod 0600` tmpfile during apply.
...
- **Operator Ergonomics:**
  - Secret rotation: updating a value in `self-hosted.enc.env` and re-running `apply.sh` MUST be sufficient. NO CI run, NO bundle rebuild required.
...
- Building a new secret manager. The existing `sops` + `age` + `knb/smackerel/secrets/self-hosted.enc.env` workflow is the source of real secret values; this spec does not replace it.
...
- **FR-052-002 (draft):** When `TARGET_ENV` is non-dev/non-test (currently `self-hosted`; any future production-class target by explicit declaration) AND a key appears in the secret-key manifest, the SST loader MUST emit `__SECRET_PLACEHOLDER__<KEY>__` in the generated `app.env` instead of the literal yaml value.
...
- **FR-052-005 (draft):** `<deployment-owner>/<product>/<target>/apply.sh` MUST read `secret-keys.yaml` from the extracted bundle, validate that every declared key has a placeholder in `app.env` AND a real (non-empty, non-placeholder) value in the decrypted secrets file, and fail loud on any mismatch BEFORE invoking `docker compose up`.
...
- **FR-052-008 (draft):** A contract test (`internal/deploy/bundle_secret_contract_test.go` or equivalent) MUST run `./smackerel.sh config generate --env self-hosted --bundle`, extract the result, and assert: (a) every key in the secret-key manifest appears as a placeholder in `app.env`, (b) no value from `internal/config/secrets.go::DevDBPasswords` appears anywhere in the bundle, (c) the placeholder format is the canonical `__SECRET_PLACEHOLDER__<KEY>__` shape.
