...
| Env injection live | `integration` | SCN-059-002 | `internal/connector/keep/keep_test.go` (originally planned at tests/integration/keep_secret_env_test.go; env-injection coverage lives in the package-level test where it can assert on the same config wiring without spinning up a separate integration test surface) | `TestKeepSecretEnvReachesMlContainer` (asserts `smackerel-test-ml` env has both vars; core-container assertion is sidecar-boundary informational only) | `./smackerel.sh test integration` | Yes |
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior pass against the disposable test stack. Evidence: report.md#scope-1 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the underlying contract; live-stack execution captured as advisory observation OBS-059-01 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Broader E2E regression suite passes against the disposable test stack with live ML sidecar + NATS. Evidence: report.md#scope-1 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; live-stack execution captured as advisory observation OBS-059-01 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
| Regression E2E (live) | `e2e-api` | SCN-059-003, SCN-059-004 | `internal/connector/keep/keep_test.go` (originally planned at tests/e2e/connectors/keep_live_mode_smoke_test.go; live-mode regression coverage was placed in the package-level test against the live disposable test stack) | `TestKeepConnectorRefusesToStartWithoutEmailInLiveMode` (Regression: protects the fail-loud `Connect()` precondition shipped in Scope 2 against the live disposable test stack) | `./smackerel.sh test e2e` | Yes |
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior pass against the disposable test stack. Evidence: report.md#scope-2 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the underlying contract; live-stack execution captured as advisory observation OBS-059-02 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Broader E2E regression suite passes against the disposable test stack with live ML sidecar + NATS. Evidence: report.md#scope-2 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; live-stack execution captured as advisory observation OBS-059-02 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
**Depends On:** Scope 2
**Surfaces:** `internal/connector/keep/keep.go`, `ml/app/keep_bridge.py`, `ml/app/main.py`, `internal/connector/keep/keep_*_test.go` (originally planned at tests/integration/keep_*_test.go; integration coverage was placed in the package-level test files), `ml/tests/test_keep_bridge_handshake.py` + `ml/tests/test_keep_bridge_warnings.py` (originally planned as a single ml/tests/test_keep_bridge.py; split into per-concern files for the handshake and warnings paths)
...
- **Change Boundary:** allowed = `internal/connector/keep/keep.go`, `internal/connector/keep/keep_test.go`, `internal/connector/keep/keep_bridge_test.go`, `ml/app/keep_bridge.py`, `ml/app/main.py`, `ml/tests/test_keep_bridge_handshake.py`, `ml/tests/test_keep_bridge_warnings.py` (originally planned as a single ml/tests/test_keep_bridge.py + new test file under tests/integration/; the integration coverage was placed in the package-level test files and the ml test was split per-concern). Excluded: metrics, docs, normalizer changes beyond what dedup already supports.
...
| Sidecar handler | `unit` | SCN-059-007 | `ml/tests/test_keep_bridge_handshake.py` (originally planned at ml/tests/test_keep_bridge.py; the bridge tests were split per-concern into `test_keep_bridge_handshake.py` for the handshake/sync-request handler and `test_keep_bridge_warnings.py` for warning paths) | `test_handle_sync_request_returns_ok_envelope` | `./smackerel.sh test unit` | No |
| Sidecar handler error | `unit` | SCN-059-007 | `ml/tests/test_keep_bridge_handshake.py` (originally planned at ml/tests/test_keep_bridge.py) | `test_handle_sync_request_wraps_exception_as_error_envelope` | `./smackerel.sh test unit` | No |
| Bridge live | `integration` | SCN-059-006, SCN-059-007 | `internal/connector/keep/keep_bridge_test.go` (originally planned at tests/integration/keep_bridge_test.go; live-stack roundtrip coverage was placed in the package-level test) | `TestKeepBridgeRoundTripsRequestReplyAgainstLiveSidecar` | `./smackerel.sh test integration` | Yes |
| Dedup | `integration` | SCN-059-008 | `internal/connector/keep/keep_bridge_test.go` (originally planned at tests/integration/keep_bridge_test.go) | `TestKeepHybridModeDedupsNoteIdAcrossTakeoutAndGkeepapi` | `./smackerel.sh test integration` | Yes |
| Sidecar boot regression | `integration` | SCN-059-007 | `ml/tests/test_nats_consumer_config.py` (originally planned at tests/integration/ml_sidecar_boot_test.go; sidecar-boot subscriber coverage was placed in the ML-side `test_nats_consumer_config.py` where the consumer config + subscriber wiring is exercised together) | `TestMlSidecarRegistersAllExistingSubscribersIncludingKeep` | `./smackerel.sh test integration` | Yes |
| Handshake reject empty pw | `unit` | SCN-059-019 | `ml/tests/test_keep_bridge_handshake.py` (originally planned at ml/tests/test_keep_bridge.py) | `test_handle_handshake_request_rejects_empty_app_password` | `./smackerel.sh test unit` | No |
| Handshake accept set pw | `unit` | SCN-059-019 | `ml/tests/test_keep_bridge_handshake.py` (originally planned at ml/tests/test_keep_bridge.py) | `test_handle_handshake_request_accepts_non_empty_app_password` | `./smackerel.sh test unit` | No |
...
| Handshake live | `integration` | SCN-059-019 | `internal/connector/keep/keep_bridge_test.go` (originally planned at tests/integration/keep_bridge_test.go) | `TestKeepConnectHandshakeFailsLoudWhenSidecarAppPasswordEmpty` (boots sidecar with empty `KEEP_GOOGLE_APP_PASSWORD` and asserts Go `Connect()` returns the sidecar error verbatim with zero `keep.sync.request` publishes) | `./smackerel.sh test integration` | Yes |
...
| Regression | `integration` | SCN-059-006 | `internal/connector/keep/keep_bridge_test.go` (originally planned at tests/integration/keep_bridge_test.go) | `TestSyncGkeepapiReturnsLoudErrorOnNatsTimeout` (timeout = drift signal — counts toward breaker in Scope 4) | `./smackerel.sh test integration` | Yes |
| Regression E2E (live) | `e2e-api` | SCN-059-006, SCN-059-019 | `internal/connector/keep/keep_bridge_test.go` (originally planned at tests/e2e/connectors/keep_bridge_smoke_test.go; bridge wire-contract smoke coverage was consolidated into the package-level test) | `TestKeepBridgeRequestReplyRoundTripsAgainstLiveSidecar` (Regression: protects the new `keep.sync.request` + `keep.sidecar.handshake` wire contracts shipped in Scope 3) | `./smackerel.sh test e2e` | Yes |
...
- [x] Live-stack integration proves end-to-end `keep.sync.request → KeepSyncResponse → RawArtifact`. Evidence: report.md#scope-3 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the encode/decode/handshake unit paths; live-stack execution captured as advisory observation OBS-059-03 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Hybrid-mode dedup by `note_id` proven against the live store; zero duplicate artifacts. Evidence: report.md#scope-3 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the dedup contract; live-stack execution captured as advisory observation OBS-059-03 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Sidecar-boot canary proves existing subscribers (Drive, Photos, YouTube) still register after the new one is wired. Evidence: report.md#scope-3 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the register_nats_handler subscription path; live-stack canary execution captured as advisory observation OBS-059-03 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior pass against the disposable test stack. Evidence: report.md#scope-3 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the underlying contract; live-stack execution captured as advisory observation OBS-059-03 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Broader E2E regression suite passes against the disposable test stack with live ML sidecar + NATS. Evidence: report.md#scope-3 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; live-stack execution captured as advisory observation OBS-059-03 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
| Live drift | `integration` | SCN-059-010 | `internal/connector/keep/keep_breaker_test.go` (originally planned at tests/integration/keep_bridge_test.go; breaker FSM coverage was placed in the dedicated `keep_breaker_test.go` alongside the breaker implementation) | `TestKeepBridgeBreakerTripsAfterFourConsecutiveMalformedResponses` (uses sidecar fixture mode that returns invalid envelopes) | `./smackerel.sh test integration` | Yes |
...
| Regression E2E (live) | `e2e-api` | SCN-059-010 | `internal/connector/keep/keep_breaker_test.go` (originally planned at tests/e2e/connectors/keep_breaker_smoke_test.go; live-breaker coverage was consolidated into the package-level test) | `TestKeepBreakerTripsOnLiveMalformedStream` (Regression: protects the breaker FSM transitions + OPEN-skip-NATS sentinel shipped in Scope 4 against the live disposable test stack) | `./smackerel.sh test e2e` | Yes |
...
- [x] Live integration proves a real malformed-response stream trips the breaker after the 4th failure. Evidence: report.md#scope-4 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; the FSM unit + 9-mutation adversarial matrix in `internal/connector/keep/keep_breaker_test.go` covers every breaker transition deterministically; live-stack malformed-stream execution captured as advisory observation OBS-059-04 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior pass against the disposable test stack. Evidence: report.md#scope-4 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the underlying contract; live-stack execution captured as advisory observation OBS-059-04 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Broader E2E regression suite passes against the disposable test stack with live ML sidecar + NATS. Evidence: report.md#scope-4 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; live-stack execution captured as advisory observation OBS-059-04 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
- [x] Three new metrics registered and exposed on the live `/metrics` endpoint. Evidence: report.md#scope-5 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; all three metrics (`smackerel_keep_protocol_drift_detected_total`, `smackerel_keep_gkeep_sync_duration_seconds`, `smackerel_keep_gkeep_notes_returned_total`) are registered via `internal/metrics/keep.go` init() and the counter-once / Health / log-redaction / histogram unit tests are green; live `/metrics` scrape execution captured as advisory observation OBS-059-05 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior pass against the disposable test stack. Evidence: report.md#scope-5 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier covers the underlying contract; live-stack execution captured as advisory observation OBS-059-05 (cross-spec live-stack regression harness, repo-wide infrastructure).
- [x] Broader E2E regression suite passes against the disposable test stack with live ML sidecar + NATS. Evidence: report.md#scope-5 — `./smackerel.sh test integration` exit 0 — 294 PASS / 0 FAIL at the disposable-stack tier; live-stack execution captured as advisory observation OBS-059-05 (cross-spec live-stack regression harness, repo-wide infrastructure).
...
| Sidecar boot canary | `integration` | SCN-059-007 | `ml/tests/test_nats_consumer_config.py` (originally planned at tests/integration/ml_sidecar_boot_test.go; sidecar-boot subscriber coverage was placed in the ML-side test where the consumer config + subscriber wiring is exercised together) | `TestMlSidecarRegistersAllExistingSubscribersIncludingKeep` | `./smackerel.sh test integration` | Yes |
