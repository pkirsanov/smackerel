...
| Unit | `internal/connector/discord/discord_test.go` | Discord config parsing, Connect validation | SCN-019-002, SCN-019-003 |
| Unit | `internal/connector/twitter/twitter_test.go` | Twitter config parsing, archive/API mode validation | SCN-019-003, SCN-019-004 |
| Unit | `internal/connector/weather/weather_test.go` | Weather location validation | SCN-019-003 |
| Unit | `internal/connector/alerts/alerts_test.go` | Gov Alerts coordinate/radius validation | SCN-019-003 |
| Unit | `internal/connector/markets/markets_test.go` | Financial Markets API key + watchlist validation | SCN-019-003 |
...
- [x] Scenario SCN-019-002 (Enabling Discord connector makes it operational): 5 conditional-start blocks follow exact pattern of existing 10 connectors, so when `connectors.discord.enabled=true` Discord is registered, connected, and started via the supervisor — **Evidence:** report.md Audit Evidence cites `cmd/core/connectors.go:205,230,251,273,308` for Discord/Twitter/Weather/GovAlerts/FinancialMarkets `if cfg.*Enabled` blocks alongside the existing 5 (Bookmarks/BrowserHistory/Maps/Hospitable/GuestHost). **Claim Source:** executed
...
- [x] Scenario SCN-019-003 (Missing credentials produce clear startup errors): Empty credentials cause `Connect()` to fail with descriptive error, not silent fallback, so the Financial Markets connector with empty finnhub_api_key fails loudly while other connectors continue operating — **Evidence:** report.md Chaos Evidence cites `internal/connector/markets/markets.go:172` (`finnhub_api_key is required`), `markets.go:920` (`fred_enabled is true but fred_api_key is empty`), `internal/connector/discord/discord.go:254` (`discord bot_token is required`), `discord.go:261` (token-too-short rejection). **Claim Source:** executed
...
- [x] Scenario SCN-019-004 (Config entries exist for all 5 connectors in smackerel.yaml): `connectors.discord`, `connectors.twitter`, `connectors.weather`, `connectors.gov-alerts`, and `connectors.financial-markets` entries exist in `config/smackerel.yaml` with `enabled: false` as the default for each entry, satisfying the connectors-section single-source-of-truth contract — **Evidence:** `tests/integration/test_connector_wiring.sh` exercises `./smackerel.sh config generate` and asserts every one of the 5 connectors' env vars exist in the generated `dev.env` plus each defaults to `enabled=false` (32 PASS / 0 FAIL, exit 0, last line `SCN-019-004: PASS`); cross-referenced by BUG-019-002 close-out. **Claim Source:** executed
- [x] Scenario SCN-019-005 (Health endpoint shows all 15 connectors): the `/api/health` endpoint shows status for all 15 connectors via the `ConnectorHealthLister` interface and the registry holding the 15 instances, and disabled connectors show status `disconnected` per the handler contract — **Evidence:** `internal/api/health.go::ConnectorHealthLister` + `health.go` wires the registry holding 15 connector instances into the handler; `internal/api/health_test.go::TestHealthHandler_ConnectorHealth` PASS (focused run `go test -count=1 -run TestHealthHandler_ConnectorHealth ./internal/api/...` exits 0); cross-referenced by BUG-019-002 close-out. **Claim Source:** executed
