...
- [x] SCN-039-005-01 Healthy provider makes capability ready: only fresh healthy category-compatible production coverage permits the exact action. → Evidence: [report.md](../../report.md) "Runtime Wiring & Live Readiness Proof — 2026-07-26" → Live Integration Proof subtest `enabled_with_one_healthy_production_provider_is_ready` (state=available, ready=true, healthy_eligible=1 on the REAL registry) + Availability Unit — No Regression (`runtime_test.go` "one healthy production provider is ready", category-mismatch stays not-ready).
- [x] SCN-039-005-02 Enabled zero-provider state is not ready: required startup refuses and optional capability is unavailable with no ready action. → Evidence: [report.md](../../report.md) "Runtime Wiring & Live Readiness Proof — 2026-07-26" → Live Integration Proof subtests `enabled_zero_providers_is_honest_not_ready` + `enabled_with_only_a_healthy_fixture_is_still_not_ready` (state=unavailable, cause=zero_configured_providers, ready=false) prove the optional-capability verdict on the real registry; the required-mode refusal verdict is the gate's `MustRefuseStartup()` recorded under REC04-TP01. The runtime startup-abort WIRING is tracked (still `[ ]`) by the "Required failure refuses startup ..." core outcome below.
...
- [x] SCN-039-005-10 Disabled providers do not dilute readiness: disabled, unconfigured, fixture, and category-irrelevant declarations are excluded from numerator and denominator, and one eligible healthy provider suffices for first readiness. → Evidence: [report.md](../../report.md) "Runtime Wiring & Live Readiness Proof — 2026-07-26" → Live Integration Proof fixture-excluded subtest `enabled_with_only_a_healthy_fixture_is_still_not_ready` (counts.fixtures=1, eligible=0) + one-healthy-suffices subtest `enabled_with_one_healthy_production_provider_is_ready`; Availability Unit — No Regression (`runtime_test.go` fixture-only, disabled-only, wrong-category-only, and healthy+fixture cases all exclude the non-eligible declaration).
