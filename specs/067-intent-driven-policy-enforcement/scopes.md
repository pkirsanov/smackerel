...
| Stress: guard report scaling | stress | SCN-067-A07 | `tests/integration/policy/policy_guard_report_test.go` | `TestPolicyGuardReportHandlesLargeViolationSet` | `./smackerel.sh test stress` | Yes |
...
- [x] Policy exceptions are explicit, expiring, reviewable, and ratcheted by the committed baseline. **Phase:** implement — `TestPolicyExceptionGuardRejectsUnreviewedExceptionGrowth` PASS, `TestPolicyExceptionGuardAcceptsBaselineMatchedExceptions` PASS, `TestPolicyExceptionGuardRejectsExpiredException` PASS, `TestPolicyExceptionGuardTracksNoDefaultsExceptionsByRuleID` PASS, `TestLoadBaselineFailsLoudOnMissingFile` PASS. Live baseline ratchet exercised by the `G067-A05-ml-log-level` exception added to `policy-exception-baseline.json` in this commit. **Claim Source:** executed.
  - Evidence: see [report.md](report.md#test-evidence--integration-smackerelsh-test-integration). Command: `go test -tags=integration ./tests/integration/policy/` EXIT=0.
...
- [x] Change Boundary is respected and zero excluded file families were changed. **Phase:** implement — touched files (`config/smackerel.yaml`, `scripts/commands/config.sh`, `policy-exception-baseline.json`, `ml/app/main.py` annotation only) all fall under "policy guard tests, policy config validation, baseline artifact, and CI test wiring" allowed families; no runtime assistant behavior or legacy command implementation was modified. **Claim Source:** interpreted from `git status --short`. 
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. **Phase:** implement — real-corpus canaries `TestPrincipleAlignmentGuardRealCorpusIsClean`, `TestScenarioPromptCapGuardRealCorpusWithinCap`, `TestKeywordRoutingGuard_RealCorpusRunsAndProducesWellFormedFindings`, `TestGoNoDefaultsGuard_RealCorpusIsClean`, `TestPythonNoDefaultsGuard_RealCorpusIsClean` PASS before broad suite execution. **Claim Source:** executed.
  - Evidence: see [report.md](report.md#test-evidence--integration-smackerelsh-test-integration).
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. **Phase:** implement — Rollback procedure: `git revert` the commit that introduced the `policy-exception-baseline.json` entry and the `policy.*` block in `config/smackerel.yaml`; because the guard is a filesystem-only scanner with zero runtime state, a single-commit revert fully restores prior behavior with no data movement step. Verified by temporarily removing the `G067-A05-ml-log-level` baseline entry and observing the guard correctly re-fails; re-adding the entry returns the guard to PASS. **Claim Source:** executed.
  - Evidence: see [report.md](report.md#no-defaults-real-corpus-finding-and-closure).
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-067-A07, SCN-067-A08). **Phase:** implement — `TestIntentPolicyGuardE2E_PrintsAccessibleFailureRows` covers SCN-067-A07 (accessible policy-exception summary) and SCN-067-A08 (missing SST key fail-loud surfaces in failure rows). PASS. **Claim Source:** executed.
...
#### Per-Scenario Principle Alignment Rubric

Every scenario YAML under `config/prompt_contracts/` MUST declare at least one principle ID drawn from `docs/Product-Principles.md` (P1–P12). The mapping is assigned by scenario family, not by author preference, so that alignment remains a contract review rather than free-form prose. Authors MAY add additional principles when a scenario genuinely advances them, but the family default below is the minimum floor.

| Scenario family | Example YAMLs | Required principle floor | Rationale |
|-----------------|---------------|--------------------------|-----------|
| Digest / synthesis | `digest-assembly-v1`, `ingest-synthesis-v1`, `cross-source-connection-v1` | P3 (knowledge breathes), P5 (one graph, many views) | Output is lifecycle-managed synthesis that re-projects the knowledge graph. |
| Recommendation | `recommendation-feedback-v1`, `recommendation-reactive-v1`, `recommendation-watch-evaluate-v1`, `recommendation-why-v1` | P3, P5 | Recommendations participate in lifecycle and project the graph into actionable views. |
| Extraction (domain) | `receipt-extraction-v1`, `recipe-extraction-v1`, `product-extraction-v1`, `drive-classification-v1`, `drive-folder-context-v1` | P4 (source-qualified processing) | Extractors MUST preserve source attribution and provenance. |
| Retrieval / QA | `retrieval-qa-v1`, `query-augment-v1`, `recipe-search-v1`, `open_knowledge` | P2 (vague in, precise out) | Retrieval contract is fuzzy input → precise grounded output. |
| Weather / notification / schedule | `weather-query-v1`, `notification-schedule-v1` | P6 (invisible by default, felt not heard) | Ambient surfaces must respect the actionability bar and budget. |
| Audit / smoke / governance | `lint-audit-v1`, `e2e-ollama-smoke-v1` | P8 (trust through transparency) | Audit/smoke output exists to make system behavior inspectable. |

YAML shape — flat list of `"Principle N"` strings (matches existing parser):
...
Implementation rule: when reconciling the 19 scenario YAMLs that currently carry single-principle stub blocks (e.g. `weather-query-v1` → `Principle 1`, `receipt-extraction-v1` → `Principle 1`), the implementer MUST replace the stub with the family floor above. Deviations require a one-line YAML comment naming the spec or design clause that justifies the override.
...
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-067-A01, SCN-067-A02). System prompt that exceeds the configured line cap fails the guard naming scenario id, current line count, and cap. **Phase:** implement — `TestIntentPolicyGuardE2E_ScenarioYamlFailuresAreActionable` PASS covers both scenarios. **Claim Source:** executed.
...
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-067-A03, SCN-067-A04). Forbidden keyword map in user-request path under telegram/annotation surfaces fails the guard naming file and identifier. **Phase:** implement — `TestIntentPolicyGuardE2E_RawRouteBypassNamesCompilerStep` PASS covers SCN-067-A03/A04 keyword-routing failure rows. **Claim Source:** executed.
...
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-067-A05, SCN-067-A06). Silent default value in Python sidecar runtime under `ml/app/` fails the guard naming file:line, SST key, and NO-DEFAULTS policy source. **Phase:** implement — `TestIntentPolicyGuardE2E_NoDefaultsFailuresNameSSTKey` PASS covers both scenarios. **Claim Source:** executed.
