...
| **DD-5** | Cite the policy reversal commit by SHA + bug ID | `BUG-029-003 (HEAD eec1437c, 2026-05-14)` — the SHA pins the reversal to a single concrete commit; the bug ID pins it to a spec dossier; the date pins it to the home-lab readiness re-scan window. All three are required for an unambiguous citation. |
