...
**Status:** Done
**Status Note:** Implementation, unit tests, integration tests, and adversarial regression tests (BS-003/004/005/006/007/008/015/021) all green against the live test stack. Live-Ollama happy-path E2E verification gap closed by spec 043 (Ollama Test Infrastructure) on 2026-05-09 — `tests/e2e/agent/happy_path_test.go` lands `TestAgentHappyPath_PlanToolSynthesis` + `TestAgentHappyPath_DeterministicOutput` + `TestOllamaUnreachable_FailsLoudly`, and `./smackerel.sh test e2e` gates them on `SMACKEREL_TEST_OLLAMA=1` after pulling `qwen2.5:0.5b-instruct` via `scripts/commands/ollama-test-pull.sh`. MIT-037-OLLAMA-001 is closed; cross-spec impact on Scope 9 `telegram_replies_test.go` is also unblocked.
...
- [x] Live-stack happy-path E2E green against real Ollama via `./smackerel.sh test e2e` — closed 2026-05-09 by spec 043 (Ollama Test Infrastructure). `tests/e2e/agent/happy_path_test.go` (build tag `e2e_ollama`) gates on `SMACKEREL_TEST_OLLAMA=1`; the e2e runner pulls `qwen2.5:0.5b-instruct` via `scripts/commands/ollama-test-pull.sh` and then runs `TestAgentHappyPath_PlanToolSynthesis` against the live stack with the Ollama compose profile up. Determinism is sourced from the `OLLAMA_TEST_REQUEST_*` SST keys forwarded by `ml/app/agent.resolve_ollama_determinism_options()`.
- [x] No mocks in any `e2e-*` test file
- [x] `./smackerel.sh test unit integration e2e` all pass — closed 2026-05-09 by spec 043 (Ollama Test Infrastructure). The runner-orchestrated agent happy-path is now reachable via `SMACKEREL_TEST_OLLAMA=1 ./smackerel.sh test e2e`; baseline `./smackerel.sh test e2e` (without the gate) keeps the fast lane.
