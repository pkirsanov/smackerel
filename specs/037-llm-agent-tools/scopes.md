...
| 4 | Intent Router                            | `internal/agent`                        | unit, adversarial                  | Similarity + override + fallback + forbidden-pattern guard  | [ ] Not started   |
| 5 | Execution Loop (Go ↔ Python NATS)        | `internal/agent`, `ml/app/agent.py`     | unit, integration, live-stack, adv | All adversarial outcomes wired with structured envelopes    | [ ] Not started   |
| 6 | Trace Persistence & Replay               | `internal/agent`, db migrations         | integration, live-stack            | Trace queryable; replay PASS/FAIL/ERROR exit codes          | [ ] Not started   |
| 7 | Security & Concurrency Hardening         | `internal/agent`                        | unit, integration, stress, live-stack adv | Redaction; integrity hash; isolation under load; injection blocked | [ ] Not started   |
...
- [ ] Migrations applied; tables and indexes present
- [ ] Tracer writes one trace row + N tool-call rows per invocation
- [ ] `smackerel agent replay` CLI returns 0/1/2 per design §6.2
- [ ] BS-019 in-flight version isolation tested under hot-reload
- [ ] Live-stack PASS and FAIL replay tests green
- [ ] `./smackerel.sh test integration e2e` pass
...
- [ ] `x-redact` enforced at persistence boundary, never at handler boundary
- [ ] Replay integrity check active with override flag
- [ ] BS-018 stress test passes with no cross-trace leakage
- [ ] BS-020 adversarial live-stack test passes against real Ollama and forces the malicious tool call (no bailout)
- [ ] BS-022 redaction integration test passes
- [ ] `./smackerel.sh test integration stress e2e` pass
...
- [ ] Scheduler and pipeline surfaces call `Executor.Run` (no regex/switch routers)
- [ ] `cmd/scenario-lint` wired into `./smackerel.sh check`
- [ ] Forbidden-pattern guard active in CI
- [ ] BS-001 zero-Go-change live-stack test passes
- [ ] `docs/Development.md` updated with "Adding a Scenario / Tool" sections referencing this scope
- [ ] `./smackerel.sh check lint test unit integration e2e` all pass
