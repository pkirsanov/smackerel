...
> **Production-wiring status note (BUG-034-002, 2026-05-25):** A `bubbles.simplify` probe (stochastic-quality-sweep R7 / sweep-2026-05-25-r10) confirmed that several Scope 08 DoD claims are structurally met (the symbols exist in `internal/telegram/expenses.go` and are unit-test covered) but are NOT reached from the production Telegram dispatch path in `internal/telegram/bot.go`. Specifically: the eight `Format*` helpers backing T-001..T-007/T-009/T-010 (`FormatExpenseConfirmation`, `FormatOCRFailure`, `FormatPartialExtraction`, `FormatAmountMissing`, `FormatExpenseList`, `FormatExpenseCSVMessage`, `FormatFixPrompt`, `FormatFieldUpdated`); the three intent helpers `isExpenseEntry`, `isSuggestionAccept`, `isSuggestionDismiss`; the `expenseStateStore` methods `Get` / `Set` / `Clear` backing the multi-turn fix-flow state machine; and the supporting `expenseAmountReplyPattern` / `expenseEntryAmountRe` regexps and `truncateVendor` / `maxTelegramVendorLen` helpers — none of them have a production caller. The priority-7 expense router at `internal/telegram/bot.go:438-443` only invokes `isExpenseQuery` and `isExpenseExport`. The photo path goes through `handlePhotoUpload` (`internal/telegram/photo_upload.go`) posting to the unified `/v1/photos/upload` endpoint and replying with the photo id; there is no `artifacts.processed` NATS subscriber in `internal/telegram/`, contrary to design.md L690. `handleExpenseQuery` (`bot.go:1188-1245`) and `handleExpenseExport` (`bot.go:1248-1289`) build minimal inline reply text via a local `strings.Builder` instead of calling `FormatExpenseList` / `FormatExpenseCSVMessage`. The TTL goroutine started by `newExpenseStateStore(120).StartCleanup` runs against a map that no production code mutates. The DoD checkbox evidence below is therefore structurally true (helpers exist + tests pass) but operationally misleading. Final disposition (wire the helpers into a real production caller graph, or delete the orphan code) is owned by Scope 15 of this spec (currently `Not started`, depends on spec 037 `037-llm-agent-tools`). Tracked under [bugs/BUG-034-002-telegram-format-helpers-not-wired/](bugs/BUG-034-002-telegram-format-helpers-not-wired/spec.md). Scope 08 status remains `Done` because the underlying helpers and tests are in place; the wiring gap is now visible to future readers and to the Scope 15 migration.
