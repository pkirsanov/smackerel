...
| LLM gateway | `config/smackerel.yaml` `llm:` (lines ~80–89; `provider`, `model`, `ollama_url`, `ollama_model`, …) | Local Ollama extraction. On home-lab, point at evo-x2 host Ollama (`knb/docs/HomeLabServices.md`). |
...
card_rewards:
  ...
  extraction:
    ...
    endpoint: ""                        # REQUIRED when enabled — e.g. evo-x2 host Ollama URL
...
target the evo-x2 host Ollama. The generator (`scripts/commands/config.sh`)
...
evo-x2/test Ollama per spec 043. No mocks of internal components.
