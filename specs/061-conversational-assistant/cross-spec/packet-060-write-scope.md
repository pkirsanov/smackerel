...
**Routing status:** packet authored 2026-05-28 by `bubbles.implement` during spec 061 SCOPE-08 substrate landing. **Accepted 2026-05-29** by spec 060 owner (`bubbles.workflow` bugfix-fastlane) — artifact-only acceptance with contract translation. Wire form translated to spec 060's `<surface>:<capability>` regex: `assistant:notifications-write`. Code-side surface registration (`"assistant"` added to `internal/auth/scopes.go::RegisteredScopeSurfaces`) deferred to spec 061's commits per spec 060's documented "surface registration lands with introducing spec" pattern. Owner-only / default-deny enforcement: spec 060 has NO grant DB; the default-deny posture is enforced naturally by (a) spec 060's "missing scope = mismatch" invariant (design.md §4) — tokens lacking `assistant:notifications-write` receive 403 from `auth.RequireScope` middleware on spec 061's `notification_execute` route, and (b) operator discipline: `auth enroll --scope assistant:notifications-write` MUST NOT be added to bot-shared token minting; spec 061 owner must document explicit per-owner mint in design.md §14 step 3 runbook. Adversarial regression for the default-deny path is owned by spec 061's `notification_execute` route tests (token without the scope → 403). See spec 060 `report.md` → "Accepted Cross-Spec Packets (Spec 061)" for full acceptance record.
