...
- [ ] Broader E2E regression suite passes with the new test in place — `./smackerel.sh test e2e` (full suite, no `--go-run` selector) exit 0. **NOT YET RUN.** The scenario-specific lane above is green, but the full-suite run has not been executed in this session, so this item is deliberately left unchecked rather than inferred from the focused run. It is the only item standing between this packet and a terminal transition.
