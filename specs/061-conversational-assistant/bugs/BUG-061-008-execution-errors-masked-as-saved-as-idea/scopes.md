...
| Regression E2E (scenario-specific) | `e2e-api` | `tests/e2e/assistant/execution_error_honesty_e2e_test.go` | SCN-061-008-01/03 over the LIVE `POST /api/assistant/turn` route: a band-HIGH turn that terminates on an execution error surfaces `StatusUnavailable` + a typed `ErrorCause` from the closed vocabulary, `capture_route=false`, no capture acknowledgement, and a body carrying no internal implementation vocabulary | `./smackerel.sh test e2e --go-package assistant --go-run 'TestAssistantHTTPE2E_ExecutionErrorSurfacesHonestly'` | Yes |
| Regression E2E (broader suite) | `e2e-api` | `tests/e2e/**` | the full E2E suite still passes with the new scenario-specific test in place | `./smackerel.sh test e2e` | Yes |

### Definition of Done
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior — `tests/e2e/assistant/execution_error_honesty_e2e_test.go` (`TestAssistantHTTPE2E_ExecutionErrorSurfacesHonestly`) drives the LIVE HTTP ingress and binds SCN-061-008-01/03 on the wire. Every prior proof for this packet was an in-package unit test that never crossed a transport boundary. **Claim Source:** executed 2026-08-27. `--- PASS … (0.14s)`, `ok tests/e2e/assistant 0.163s`, `PASS: go-e2e`, exit 0; live envelope `status="unavailable" error_cause="provider_unavailable" capture_route=false sources=0 body="the service is unavailable right now — please try again in a moment."`. **Non-vacuity proven by mutation, not asserted:** inverting the `StatusSavedAsIdea` assertion made the run `--- FAIL … FAIL: go-e2e (exit=1)` against the same live envelope, so the assertion is reached and load-bearing; the probe was reverted and the file re-verified clean (`grep -c 'MUTATION PROBE'` → 0) with the test green again. Evidence: [report.md](report.md) → "P2 evidence".
- [x] Broader E2E regression suite passes with the new test in place — `./smackerel.sh test e2e` (full suite, no `--go-run` selector) exit 0. **RUN 2026-08-28: `E2E_V3_EXIT=0`.** This packet's own test ran inside that suite and passed: `--- PASS: TestAssistantHTTPE2E_ExecutionErrorSurfacesHonestly (0.11s)` at log line 3577. All three go phases green: `PASS: go-e2e`, `PASS: go-e2e-graph-disabled`, `PASS: go-e2e-corpus-enforce`. The single `FAIL:` string in the log (line 1231, "Services did not become healthy within 8s") is the EXPECTED output of a negative scenario that deliberately stops postgres — the next line is `PASS: SCN-002-BUG-002-001 (stopped postgres rejected, exit=1)` — not a suite failure. **Claim Source:** executed. Evidence: [report.md](report.md) → "Broader suite — full `test e2e`"
