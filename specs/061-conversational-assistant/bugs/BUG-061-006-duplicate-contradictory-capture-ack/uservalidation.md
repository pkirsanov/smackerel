...
- [ ] LIVE: on the bot deployed to `<target>`, a bare `/ask` no longer shows the contradictory `? Failed to save` + `saved as an idea` pair — the fix is **deployed + running + healthy** (fix digest verified, adapter bound); **behavioral confirmation pending a Telegram smoke test by `<operator>`** (a human turn).
