...
- [ ] LIVE: on the self-hosted bot, `/ask <question>` / `/weather <loc>` show exactly ONE acknowledgement — the fix (sourceSha `777323fa`) is **deployed + running + healthy** (running core digest matches the fix build; assistant Telegram adapter wired and bound), so the fixed code path is live; **behavioral confirmation pending the operator's Telegram smoke test** (a human turn).
...
- [ ] LIVE: on the self-hosted bot, a bare `/ask` no longer shows the contradictory `? Failed to save` + `saved as an idea` pair — the fix is **deployed + running + healthy** (fix digest verified, adapter bound); **behavioral confirmation pending the operator's Telegram smoke test** (a human turn).
