...
**Status:** Blocked-pending-design-amendment (Round 74 plan-triage — `bubbles.plan` evaluated finding `SCOPE-06C-CPU-INTERACTIVE-15S-INFEASIBLE` against the current SST surface and the warm-baseline evidence, and **selected remediation option (c) — operator-defer cpu-tier live-stack to accel-tier host**. Options (a) and (b) both require net-new agent-runtime plumbing (no per-call `num_predict` SST key exists today; `schema_retry_budget` is already 1; `assistant.skills.retrieval.top_k` is global not per-tier), and the prewarm-baseline math (`second_call_ms=7194` for a bare 64-token generation, vs real cited-JSON output that is materially longer) makes (a) optimistic at best. Option (c) parallels DoD #6's operator-defer pattern and matches the cpu-tier's scoped purpose ("dev-loop ergonomic, not production-quality"). NO budget widening. NO SCOPE-06 cascade. See [report.md#round-74-scope-06c-plan-triage](report.md#round-74-scope-06c-plan-triage) and the prior RED evidence at [report.md#round-73-scope-06c-cpu-live-stack-red](report.md#round-73-scope-06c-cpu-live-stack-red). Prior Round 71d implement edits remain GREEN — DoD #1–#4 + Build Quality Gate are still `[x]`; #5 stays `[ ]` with the option-(c) annotation pending design amendment; #6 stays `[ ]` (operator-deferred); #7 stays `[ ]` (depends on #5's operator-defer landing via design + implement).)
...
- [ ] **#6 — Live-stack RE-RUN PASS on accel tier — DEFERRED TO OPERATOR VALIDATION ON ACCELERATED-RUNNER HARDWARE** — Same fixtures, run on accelerated-runner hardware with `SMACKEREL_HARDWARE_TIER=accel`; `latency_ms < 5000`. No agent in this session has access to such hardware; checkbox remains `[ ]` with the explicit `(deferred to operator validation on accelerated-runner hardware)` annotation. Operator records PASS evidence in `report.md#scope-06c-accel-live-stack-operator-pass` when the deploy adapter overlay for that target ships AND BS-002/BS-007 are run against it. → Evidence: `report.md#scope-06c-accel-live-stack-operator-pass` (operator-owned anchor; not produced this session)
