...
**Status:** In Progress (Round 12 — BQG closed; 8/11 prior-core DoD items met; 3 DoD items reopened by Round-14 design extension (design §17 Option A — Telegram webhook mode): DoD #13 (handler unit tests), #14 (SST + wiring), #15 (BS-001 webhook e2e). Round-14 design unblocks `SCOPE-05-E2E-INJECTION-MECHANISM` by design (implementation pending). Carry-forward open: (a) DoD #8 BS-001 live-stack e2e — superseded by DoD #15 once landed; (b) DoD #9 BS-010 — paired with SCOPE-06 retrieval-skill landing; (c) DoD #11 cross-spec packet to spec 060 — `cross-spec/packet-060-read-scopes.md` routed (status DRAFT), awaiting spec 060 owner acceptance. DoD #12 Build Quality Gate `[x]` from Round 12 unchanged.)
...
13. **(NEW — design §17.5)** Rewrite `tests/e2e/telegram_assistant_bs001_test.sh` to drive the live BS-001 fixture via `POST $WEBHOOK_PATH` with `X-Telegram-Bot-Api-Secret-Token: $ASSISTANT_TELEGRAM_WEBHOOK_SECRET`; assert 200; poll PostgreSQL for the resulting `idea` artifact with the verbatim content. Add corresponding `tests/e2e/telegram_assistant_bs010_test.sh` shape (real retrieval skill round-trip) — gating closure pairs with SCOPE-06 retrieval landing per existing DoD #9. Test stack `config/generated/test.env` (regenerated via `./smackerel.sh config generate`) sets `ASSISTANT_TRANSPORT_TELEGRAM_MODE=webhook` and a known `ASSISTANT_TELEGRAM_WEBHOOK_SECRET`. This DoD row resolves `SCOPE-05-E2E-INJECTION-MECHANISM`.
...
- [ ] **(NEW — design §17.3)** Webhook handler unit tests pass for the full §17.3 behavior matrix (missing/mismatched/valid secret; malformed/empty/message/callback updates; oversize body; non-POST method; no body content in logs) → Evidence: [report.md#scope-05-webhook-handler]
- [ ] **(NEW — design §17.4 + §7.2 rules 7–10)** SST keys `assistant.transports.telegram.{mode,webhook_secret_ref,webhook_path}` added with `${VAR:?...}` fail-loud; `validateAssistantConfig` rules 7–10 implemented and unit-tested; `cmd/core/wiring.go::startTelegramBotIfConfigured` implements the exclusive mode branch; `internal/api/router.go` registers the webhook route via `RegisterTelegramWebhookRoute` OUTSIDE bearer-auth ONLY when mode=webhook → Evidence: [report.md#scope-05-webhook-sst-wiring]
- [ ] **(NEW — design §17.5)** `tests/e2e/telegram_assistant_bs001_test.sh` rewritten to drive a live BS-001 webhook POST (valid secret → 200 + idea artifact; adversarial wrong-secret → 401 + zero rows); test stack runs `mode=webhook`. Resolves `SCOPE-05-E2E-INJECTION-MECHANISM`. → Evidence: [report.md#scope-05-bs-001-webhook]
...
- [ ] **BS-002 (4b — adapter-composition leg)** — Telegram-fixture-driven full-stack POST → reply contains synthesized answer + trailing `sources:` block. BLOCKED on `SCOPE-05-E2E-INJECTION-MECHANISM` (paired with SCOPE-05 DoD #8). → Evidence: [report.md#scope-06-bs-002-round-11]
...
- [ ] **BS-007 (5b — adapter-composition leg)** — Force-empty-sources case driven through real Telegram-fixture POST → counter `assistant_provenance_violations_total{skill="retrieval"}` incremented; `idea` artifact created. BLOCKED on `SCOPE-05-E2E-INJECTION-MECHANISM`. → Evidence: [report.md#scope-06-bs-007-round-11]
- [ ] Telegram trailing `sources:` block rendering verified end-to-end (paired with DoD #4b adapter-composition leg). BLOCKED on `SCOPE-05-E2E-INJECTION-MECHANISM`. → Evidence: [report.md#scope-06-render-sources]
...
**Status:** In Progress (Round 14 — capability-layer landing complete. 6/8 core DoD items met with live evidence; 2 honestly open with named foreign owners: (a) DoD #6 BS-003 SHELL e2e — the in-process `TestFacadeWeatherIntegration_BS003_*` capability-layer leg is GREEN against real PostgreSQL, but the literal `tests/e2e/assistant_bs003_test.sh` row is blocked by the same SCOPE-05 `SCOPE-05-E2E-INJECTION-MECHANISM` finding that blocks SCOPE-05 BS-001 and SCOPE-06 BS-002 shell e2e (`tgbotapi.GetUpdatesChan` long-poll; no shell-driveable webhook injection path); (b) DoD #7 BS-006 SHELL e2e — same shared injection-mechanism block; the in-process `TestFacadeWeatherIntegration_BS006_*` capability-layer leg is GREEN. Per Gate G024 SCOPE-07 cannot promote to Done while these 2 remain `[ ]`. Per Gate G026 the stress test passes the 3s manifest budget with margin and proves the cache invariant (p95=8.7ms, hit_ratio=0.948).
Per the established SCOPE-05/06 pattern, the shell-e2e leg is the only remaining gap; it shares the same root cause and the same external owner. Same plan-correction split recommended (capability-layer leg green + adapter-composition leg blocked) should be applied if/when that finding lands.)
**Depends On:** SCOPE-03, SCOPE-04, SCOPE-05 (catalog packet 060 read scopes)
...
- [ ] **BS-003** — Weather query e2e returns provider-attributed answer → Evidence: [report.md#scope-07-bs-003] — **BLOCKED on `SCOPE-05-E2E-INJECTION-MECHANISM` (shared with SCOPE-05 BS-001 / SCOPE-06 BS-002 shell e2e). Capability-layer leg `TestFacadeWeatherIntegration_BS003_HappyPathEmitsExternalProviderSource` GREEN against real PostgreSQL (Round 14 evidence).**
- [ ] **BS-006** — Provider outage e2e renders error + offer-to-capture; user confirm → capture → Evidence: [report.md#scope-07-bs-006] — **BLOCKED on same `SCOPE-05-E2E-INJECTION-MECHANISM` finding. Capability-layer leg `TestFacadeWeatherIntegration_BS006_ProviderUnavailableTriggersRefusal` GREEN against real PostgreSQL (Round 14 evidence).**
