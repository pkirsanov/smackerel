...
- [ ] **BS-003** — Weather query e2e returns provider-attributed answer → Evidence: [report.md#scope-07-bs-003] — **Shell e2e fixture `tests/e2e/assistant_bs003_test.sh` not yet authored. The original `SCOPE-05-E2E-INJECTION-MECHANISM` block was RESOLVED by commit `a8ac4f8b`'s landing of `internal/telegram/webhook_handler.go`; remaining gap is NEW WORK deferred to a follow-up scope round per the commit narrative. Capability-layer leg `TestFacadeWeatherIntegration_BS003_HappyPathEmitsExternalProviderSource` GREEN against real PostgreSQL (committed in `a8ac4f8b`; re-validated this session via wrapper PASS).**
- [ ] **BS-006** — Provider outage e2e renders error + offer-to-capture; user confirm → capture → Evidence: [report.md#scope-07-bs-006] — **Same situation as BS-003: shell e2e fixture not yet authored; capability-layer leg `TestFacadeWeatherIntegration_BS006_ProviderUnavailableTriggersRefusal` GREEN against real PostgreSQL.**
...
- [ ] Manifest matches design §5.3 exactly → Evidence: [report.md#scope-08-manifest] — **HONESTLY OPEN: design.md §10 relocated `internal/assistant/skills/notifications/` to `config/prompt_contracts/notification-schedule-v1.yaml` + `internal/agent/tools/notification/` (both shipped). Whether that relocation satisfies "manifest matches §5.3 exactly" requires bubbles.plan reconciliation between this DoD's wording and the relocated layout. See report.md#round-15-scope-07-08-combined-pass DoD #1 Uncertainty Declaration.**
...
- [ ] **BS-004** — Notification e2e proves confirm-card flow with real scheduler row creation → Evidence: [report.md#scope-08-bs-004] — **BLOCKED on packet 054 (real scheduler binding) + packet 060 (write-scope authorization). Substrate (PgConfirmStore, confirm.Machine, audit writer, cmd/core wiring with SST gate) shipped this round; the only missing pieces are foreign-owned. See Round 15 honest gap inventory.**
...
- [x] All 11 Prometheus metrics from design §8.1 emitted with correct types + closed-vocabulary labels → Evidence: [report.md#scope-09-metrics]
