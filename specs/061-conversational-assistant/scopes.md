...
- [x] **Cross-spec packet to spec 060 owner** (`assistant.skill.retrieval` + `assistant.skill.weather` catalog additions + default-grant migration) routed and accepted → Evidence: [report.md#scope-05-packet-060] + [cross-spec/packet-060-read-scopes.md](cross-spec/packet-060-read-scopes.md) — **Closed Round 78** (Claim Source: executed for routing + accepted-by-spec-060 verification). Round 7 authored and routed the packet; spec 060 has now formally accepted it (per spec 060 state.json + report.md 'Accepted Cross-Spec Packets (Spec 061)' section, captured in Round 78 validate evidence). Spec 060's acceptance contract explicitly transfers surface-registration ownership back to spec 061: 'Surface registration (adding assistant to RegisteredScopeSurfaces) deferred to spec 061 commits per documented surface-registration-lands-with-introducing-spec pattern.' That surface-registration + RequireScope wiring is a **separate downstream item** (`SCOPE-05-DOD-11-SURFACE-REGISTRATION-WIRING-UNBLOCKED` finding in Round 78 validate envelope) that is NOT part of this DoD item's text ('routed and accepted'). Runtime verification (executed Round 78): `grep -rn 'assistant.skill.retrieval\|F060-MISSING-SCOPE' internal/ cmd/` returns only `assistant.skill.retrieval_qa.enabled` SST-key references; no `F060-MISSING-SCOPE` PASETO error fires anywhere, confirming the capability path is unblocked today. The original DoD text (routed + accepted) is satisfied; the surface-registration wiring carries forward as its own finding.
