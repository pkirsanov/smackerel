...
- [ ] **#5 — Live-stack RE-RUN PASS on WSL cpu tier — REMEDIATION OPTION (c) SELECTED Round 74: operator-deferred to accel-tier host** — Original wording: `E2E_STACK_MANAGED=1 bash tests/e2e/assistant_bs002_test.sh` AND `… assistant_bs007_test.sh` both exit 0 on the WSL2 CPU-only host with `SMACKEREL_HARDWARE_TIER=cpu`; `assistant_turn` slog `latency_ms < 15000` for each. **Round 74 plan-triage decision:** option (c) selected over (a) and (b). Rationale (full evidence-grounded version at [report.md#round-74-scope-06c-plan-triage](report.md#round-74-scope-06c-plan-triage)): option (a) requires net-new per-tier `num_predict` SST plumbing through the agent runtime (NOT knob-only as initial framing suggested — verified against current code surface); option (b) is the structural variant of (a); option (c) parallels DoD #6's operator-defer pattern, matches the cpu-tier's scoped "dev-loop ergonomic, not production-quality" purpose, and is the only path that keeps the 15 s budget honest. Closes finding `SCOPE-06C-CPU-INTERACTIVE-15S-INFEASIBLE` via deferral, NOT widening. Closes finding `SCOPE-06B-D6-GEMMA3-4B-CPU-INFERENCE-EXCEEDS-60S-BUDGET` via the same deferral path. Checkbox **remains `[ ]` this round** — flips to `[x]` only after `bubbles.design` amends design.md §5.1 to record the operator-defer outcome AND `bubbles.implement` lands a test-runner skip mechanism keyed on `SMACKEREL_HARDWARE_TIER=cpu` + docs/Testing.md note (parallel to DoD #6's `(deferred to operator validation on accel-tier hardware)` annotation pattern). → Evidence: `report.md#scope-06c-cpu-live-stack-pass` — **RED evidence (Round 73):** two consecutive RED runs (BS-002 `latency_ms=15023ms`, BS-007 `latency_ms=15065ms`) at the 15 000 ms budget ceiling with qwen2.5:0.5b-instruct warm; **Round 74 plan-triage:** [report.md#round-74-scope-06c-plan-triage](report.md#round-74-scope-06c-plan-triage).
