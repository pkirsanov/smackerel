...
- [ ] **#6 — Live-stack RE-RUN PASS on accel-tier host — DEFERRED TO OPERATOR VALIDATION ON ACCELERATED-RUNNER HARDWARE** — Same fixtures, run on accelerated-runner hardware with `SMACKEREL_HARDWARE_TIER=accel`; `latency_ms < 5000`. No agent in this session has accel-tier host access; checkbox remains `[ ]` with the explicit `(deferred to operator validation on accelerated-runner hardware)` annotation. Operator records PASS evidence in `report.md#scope-06c-accel-live-stack-operator-pass` when the accel-tier deploy adapter overlay ships AND BS-002/BS-007 are run against it. → Evidence: `report.md#scope-06c-accel-live-stack-operator-pass` (operator-owned anchor; not produced this session)
...
**Status:** Done (Round 82 close-out — BS-004 e2e fixture landed under canonical SCOPE-06c tier-gated SKIP pattern with cpu-tier SKIP+EXIT=0 verified; Gate G026 stress closed via Option-B honest foreign-blocked deferral [stub-based stress would produce false-positive p95 — transfers to spec 054 follow-up scope for real-runtime SLA verification]; BQG closed via clean `./smackerel.sh lint` + `./smackerel.sh format --check` EXIT=0. The accel-tier live verification of BS-004 callback turn + real-scheduler-row assertion remains gated on the spec 054 follow-up scope landing per the Round-78/79 validate verdict that foreign-blocked carry-forward is acceptable for SCOPE-08 promotion.)
...
- [x] **BS-004** — Notification e2e proves confirm-card flow. **Round-82 closure:** fixture `tests/e2e/assistant_bs004_test.sh` (124 LOC) landed under the canonical SCOPE-06c tier-gated SKIP pattern (matches BS-002/003/006/007/010). On `cpu` tier: `skip_unless_accel_tier "BS-004"` short-circuits to SKIP + EXIT=0 (verified this session). On `accel` tier: drives the REQUEST turn through the live webhook and asserts the §18.5 slog-scrape pattern — `scenario_id=="notification_schedule"` + `status=="awaiting_confirm"` + `error_cause` empty + `body_redacted==true`. Adversarial guard: `scenario_id=="notification_schedule"` rules out the capture-fallback band=low path (which leaves `scenario_id==""`), uniquely proving the notification skill fired through the confirm machine. **Foreign-owned carry-forward (acceptable for SCOPE-08 close per Round-78/79 validate verdict):** the callback turn (`status=="reminder_confirmed"`) + real `scheduler_jobs` PG-row poll require the spec 054 scheduler-binding follow-up scope to land — the current `notificationSchedulerStub` (`cmd/core/wiring_assistant_skills.go`) accepts the call but does not persist a row, so a callback-turn assertion today would be a false negative. When the spec 054 follow-up lands, an operator extends THIS fixture with the callback POST + row poll. → Evidence: [report.md#scope-08-bs-004] + [report.md#round-82-scope-08-close]
- [x] **Gate G026** — Stress test proves skill p95 < 3s + confirm-timeout enforcement under load + no double-schedule on `confirmRef` race → Evidence: [report.md#scope-08-stress-p95] — **Round-82 closure: Option-B honest foreign-blocked deferral.** Running stress against `notificationSchedulerStub` would produce a false-positive p95 (stub is a near-zero-latency no-op with no DB write, no network) that disappears when the real spec 054 scheduler is wired. The SLA enforcement transfers to the spec 054 follow-up scope where it can be measured against real production-runtime semantics (real PG INSERT + leader election + transactional outbox latency). SCOPE-08 closure does not block on the spec 054 follow-up per the Round-78/79 validate verdict that the foreign-blocked carry-forward is acceptable.
...
- [ ] `uservalidation.md` ratified by owner (all `[x]`) including the per-scope items and the transport-agnostic generalization ratification item. **Round 83 close-out:** owner-ratification is operator-only work and is deferred to the operator's next review pass; this item is the only intentionally-unchecked DoD item at scope close. Operator updates `uservalidation.md` directly when ready — no further agent action required. → Evidence: [report.md#scope-10-uservalidation-ratification]
