...
**Status:** In Progress (Round 71d — `bubbles.implement` landed the hardware-tier × model-role matrix in SST: `models:` block in `config/smackerel.yaml`, fail-loud `SMACKEREL_HARDWARE_TIER` resolution in `scripts/commands/config.sh` with tier-orthogonal per-env override, retrieval-qa-v1 placeholder form + loader env-expansion, Go `ValidateHardwareTier` + adversarial unit tests, `.smackerel.local.env` gitignored overlay with `.example` template, prewarm threshold derived from tier-resolved budget, scenario-lint env propagation. DoD #1–#4 + Build Quality Gate flipped `[x]` with executed evidence. DoD #5 (cpu-tier live-stack RE-RUN PASS) deferred to `bubbles.test` next round. DoD #6 (accel-tier live-stack PASS) deferred to operator on Evo-X2 hardware. DoD #7 (SCOPE-06 unblock note) deferred until #5 lands.)
