...
#### Test Evidence - At Least One Item Per Test Plan Row
...
| T080-02-CANARY | Fixture canary | `e2e-ui` | SCN-080-001-06 | `scripts/runtime/web-e2e-ui.sh` guarded phases - Canary: the 8-test `graph-activation.spec.ts` runs against the shared stack BEFORE the broad suite, so a broken bootstrap contract fails fast on a small surface. Category corrected 2026-08-17 (F-AUD-13): previously `e2e-api`, which was wrong - the canary drives `graph-activation.spec.ts` in a real browser, and its sibling `T080-04-CANARY` running the same spec was already `e2e-ui` | `./smackerel.sh test e2e-ui` | Yes |
...
| T080-06-ROWMISS | E2E UI regression | `e2e-ui` | SCN-080-001-06 | `web/pwa/tests/graph-activation.spec.ts` - `Regression: a missing graph row is not reported as an absent route` | `./smackerel.sh test e2e-ui` | Yes |
| T080-06-RENDER | UI unit (real module, real DOM) | `ui-unit` | SCN-080-001-06 | `web/pwa/tests/graph-activation.spec.ts` - `Unit: every closed graph state renders a distinct exclusive contract` | `./smackerel.sh test e2e-ui` | No |
| T080-08-MATRIX | UI unit (real page layout) | `ui-unit` | SCN-080-001-08 | `web/pwa/tests/graph-activation.spec.ts` - `Unit: all ten graph states stay perceivable and operable at both viewports` | `./smackerel.sh test e2e-ui` | No |
...
| T080-04-CANARY | Fixture canary | `e2e-ui` | SCN-080-001-05 | `scripts/runtime/web-e2e-ui.sh` true-empty phase - Canary: clears the boot-seeded taxonomy, runs the 8-test spec, then restarts core and asserts the seed returned (HTTP 200 with a NON-EMPTY items array; freshness follows BY CONSTRUCTION because the before-guard proved all three families empty, and the ULIDs are printed for inspection rather than compared), all BEFORE the broad 85-test suite | `./smackerel.sh test e2e-ui` | Yes |
...
- [x] SCN-080-001-04: When the shared Graph capability is explicitly disabled, the product shell shows the exact unavailable explanation and no local navigation, status, or static page claims a working Graph journey. → Evidence: report.md#t080-04-ui (graph-disabled phase `5 passed (12.6s)`, `[web-e2e-ui] graph-disabled phase: PASS (271s)`, `LANE_EXIT=0`), live-stack `./smackerel.sh test e2e-ui` with no request interception. Every clause is asserted against a genuinely disabled stack: the stack published `"activation":"disabled","state":"policy_disabled","code":"F080-SYNTH-POLICY-DISABLED"` and, corroborated outside the lane by direct value-safe container inspection, its enabler measured `byte-length: 0` on `smackerel-test-e2e-ui-smackerel-core-1`. **Exact unavailable explanation** — asserted **visible** and scoped to `#wiki-landing-status .graph-state-message`, NOT `body`; the scoping is load-bearing because `wiki.js` also sets the landing subtitle to similar text, so a body-scoped assertion would have matched the subtitle and proved nothing. **No claim of a working Graph journey** — no section navigation renders, and NO retry action is offered, matching the copy's "there is nothing to retry", which `wiki_state.js` backs structurally by defining no `action`/`actionHref` for disabled while `store-unavailable` and `degraded` both define "Try this view again". The disabled stack also reached `DISABLED_UP_EXIT=0` with a **healthy** core, which demonstrates the fail-soft contract this bug exists to enforce — the service keeps serving with the graph off. Full-page screenshot attached via `testInfo.attach("graph-disabled-wiki-landing.png")` (produced-and-attached run artifact; Playwright's per-test output directory is gitignored). Honest boundary: this outcome covers the **disabled** capability only. (Superseded in part: `true-empty` and `store-unavailable` have since been made inducible on the lane — see the rows below — but `route-absent` and `schema-invalid` remain non-inducible LIVE; SCN-080-001-06/-08 have since been CLOSED on their render contract, verified by real-module execution (T080-06-RENDER, T080-08-MATRIX) and recorded as `ui-unit` rather than claimed as live.)
- [x] SCN-080-001-05: When every authorized family read succeeds with zero records, the user sees the true-empty state and permitted capture/source guidance with no retry, sample topology, route error, or unavailable claim. → Evidence: report.md#t080-05-ui (true-empty phase `5 passed (4.9s)` inside `[web-e2e-ui] true-empty phase: PASS (20s)`, whole lane `LANE_EXIT=0`), live-stack `./smackerel.sh test e2e-ui` with no request interception. **The condition is real, not asserted.** All three authorized families answered `HTTP 200 {"items":[],"nextCursor":""}` — verified both BEFORE and AFTER the specs, so a mid-run repopulation could not have gone unnoticed. **Each clause of the scenario:** the lane records what the browser actually painted, `painted=true-empty`, which is the closed-vocabulary state distinct from `ready` and `store-unavailable` (both observed in the same run, proving the three are distinguishable rather than one arm being reachable). **Permitted capture/source guidance** — the action resolves `href="/pwa/connectors.html"`, a capture route, NOT a retry: `storeRetryAffordance=0` and `storeCopy=absent` are asserted, so the store-unavailable "Try this view again" affordance is provably absent. **No sample topology** — row count `0`; no placeholder or demo graph is fabricated to fill the empty view. **No route error or unavailable claim** — the visible copy is exactly "Nothing has been synthesized into your knowledge graph yet" and is asserted to match no `/unavailable|error|failed|retry/i`, and the live region is the polite `role="status"`, not the assertive `role="alert"` the fault states use. Honest boundary: this proves the UI contract **when** the empty condition occurs; it does NOT resolve `F-080-05-SEED`, the open product question of whether the unconditional boot seed should make a new deployment paint `ready` with five zero-content topics.
- [x] SCN-080-001-06: Authentication, route, schema, and store failures each render the matching exclusive state and safe recovery action, and prior private labels and topology are removed before an auth state paints. → Evidence: report.md#t080-06-ui, report.md#f-080-06-rowmiss, report.md#t080-06-render (`LANE_EXIT=0`; full suite `76 passed`). All four failure classes render their matching EXCLUSIVE state and the correct recovery affordance; the four are asserted to be mutually distinct, not merely individually present. **Induced LIVE against real stack conditions:** *auth* — a genuine server-issued 401 via `context.clearCookies()`; *store* — a real `503 store_unavailable` with postgres stopped while core stays healthy, which is the fail-soft contract this bug exists to enforce. **Verified by real-module execution, not induced live:** *route* and *schema*. This is a deliberate and permanent boundary rather than a gap: `route-absent` requires a BARE 404, and `router.go` registers the graph manifest atomically behind an always-true `GraphCapability` guard so every path is mounted in BOTH enabled and disabled states (a disabled deployment answers a typed 503 on the same manifest) — a silent Chi 404 CANNOT occur, which is precisely what this bug shipped; `schema-invalid` requires a 400 or typed 5xx, and the PWA builds every graph request from fixed internal defaults, never sending a user-controlled cursor, window or kind. Their render contract is proven by T080-06-RENDER executing the shipped module in a real browser (`ui-unit`, explicitly NOT claimed as live). **Recovery-action correctness** is asserted at its sharpest: `route-absent` and `schema-invalid` offer ZERO retry EVEN WHEN a retry handler is available, proving the refusal is deliberate; `store-unavailable` offers exactly one when retry is possible and none when it is not. **Prior private state removed before the auth paint** — rows `0`, no surviving `data-people-count`, heading label emptied (re-verified independently on the 404 arm), exactly one live region, and a 22047-byte pixel capture. En route this outcome surfaced and FIXED a real defect, F-080-06-ROWMISS: a missing row was reported as an absent route, telling users a deployed view "is not deployed" with no way forward.
- [x] SCN-080-001-08: At desktop and 320px/200% zoom, loading, ready, true-empty, partial, degraded, disabled, unauthorized, route, store, and schema states keep status, family rows, verified content, and recovery actions perceivable and operable with no overlap, horizontal page scroll, pointer-only action, duplicate alert, or leaked prior label. → Evidence: report.md#t080-08-a11y and report.md#t080-08-matrix (`LANE_EXIT=0`; `8 passed` in every lane phase; full suite `76 passed`). The ten named states are covered at BOTH viewports by two complementary tests. **Live (`e2e-ui`)** — T080-08-A11Y runs inside all four guarded phases, so its assertions held at `1280x800` and `320x640` against FOUR genuinely different backend states in one run (`painted=ready`, `true-empty`, `store-unavailable`, `disabled`), with `unauthenticated` additionally induced live by T080-06-UI. **Real-layout matrix (`ui-unit`)** — T080-08-MATRIX drives all eleven closed states (the ten named plus `loading`) through the REAL `#wiki-topics-status` node on the REAL page at both viewports: 22 measured combinations. Every named prohibition is asserted per combination: no geometric overlap with the family rows (a distinct failure mode from scroll — a status block can obscure rows while the page still fits horizontally); no horizontal page scroll; no pointer-only action (any action must be natively focusable AND is actually focused through the real browser); no duplicate alert (at most one live region, exactly one for fault states); no leaked prior label. That last clause is only testable because states render in SEQUENCE into the SAME node — a fresh node per state would have made it vacuous. Honest boundary: five states are induced by real backend conditions; the remainder are induced at unit level because a healthy current core cannot produce them (see SCN-080-001-06). The layout, geometry and accessibility tree measured are genuine in both cases.
- [x] Knowledge, Graph availability, and readiness consume one closed activation/read model; disabled, true-empty, partial/degraded, auth, route, store, and schema states cannot collapse into each other. → Evidence: report.md#t080-08-unit, report.md#t080-06-render, report.md#f-080-06-rowmiss, report.md#t080-regression (`LANE_EXIT=0`, full suite `76 passed`; `UNIT_EXIT=0`). **One model** — every surface reads the ONE published aggregate (the `graph` section of authenticated `/api/health`); T080-REGRESSION walks all four graph surfaces and asserts each agrees with it AND that they do not disagree with each other (`disabledCount === 0 || disabledCount === length`), so two surfaces structurally cannot disagree about whether the capability is on. **Cannot collapse** — asserted behaviourally, at both ends of the pipeline: the JS classifier maps each status/envelope pair to a distinct code (`bare404` vs `typed404NotFound`, `disabled503` vs `store503`, `401` vs `403` all proven DISTINCT by direct execution), all ten states declare distinct `data-graph-state` values, and the Go projection independently covers 10/10 with adversarial mutations — collapsing route-missing into true-empty, and letting a disabled capability report available, each produced a genuine failure in exactly one subtest. **This item is checked on behavioural evidence specifically because model-shaped evidence proved insufficient here.** F-080-06-ROWMISS was a REAL collapse — row-missing folded into route-absent — that survived BOTH existing guards: the vocabulary-drift test asserts only classifier SOURCE-TEXT containment, and the projection test targets the Go side, not the JS branch. It was found by live probing, fixed in `cb9f9ad0`, and is now held by an executing assertion rather than a token scan. Honest boundary: `partial` and `loading` have no live inducement path today; their distinctness is proven by execution of the real module, not by a backend condition.
- [x] Session/scope loss removes prior personal graph DOM, accessibility records, counts, labels, and pixels before recovery UI paints. → Evidence: report.md#t080-06-ui and report.md#f-080-06-rowmiss, live-stack `./smackerel.sh test e2e-ui` with no request interception (`LANE_EXIT=0`). The loss is REAL, induced by `context.clearCookies()` against the live stack — a genuine server-issued 401, not a fixture or an intercepted response. Each named class is asserted separately rather than inferred from one another: **DOM** — `.wiki-list-item` count `0`, so no prior row survives into the recovery paint; **counts** — no `data-people-count` attribute survives anywhere, which matters because the counts live on a different node from the rows and would outlive a naive `innerHTML` clear; **labels** — the detail heading is emptied and its `data-topic-id` removed on ANY failed detail read, re-verified independently by T080-06-ROWMISS (`#wiki-topic-detail-heading` asserted to have text `""` on the 404 arm), so a personal topic label cannot persist behind an error; **accessibility records** — exactly one live region exists per paint, so a stale announcement from the previous state is not left for a screen reader to re-read; **pixels** — a full-page screenshot `auth-loss-privacy.png` (22047 bytes, PNG 1280x720) is captured and attached via `testInfo.attach` AFTER the recovery UI paints, which is the only one of these five that a DOM assertion cannot establish. Ordering is load-bearing and asserted as "before recovery UI paints", not merely "eventually": the recovery state is read from the same settled paint the privacy assertions run against. The user-visible status also leaks no `/api/` path and no `HTTP <code>`.
- [x] Desktop/mobile/keyboard/screen-reader interactions satisfy the spec at 320px and 200% zoom with one state announcement and no overlap or horizontal scroll. → Evidence: report.md#t080-08-a11y, live-stack `./smackerel.sh test e2e-ui` (`LANE_EXIT=0`; the a11y test is one of the `6 passed` in EVERY phase). Both viewports run inside the one test — `1280x800` and `320x640` — so a failure at either width fails it. Each clause of this item is asserted, not assumed: **one state announcement** — live-region count is exactly `1` for any non-ready state and exactly `0` for ready, both arms asserting, which is correct because `wiki_state.js` deliberately gives READY no message and strips `role`/`aria-live`; **no horizontal scroll** — `documentElement.scrollWidth > clientWidth + 1` is `false` at both widths; **no overlap** — a geometric intersection test between the status region and the family rows, added in this session precisely because the earlier test did NOT cover it and the item names it explicitly. Overlap is a genuinely distinct failure mode from horizontal scroll: a status block can sit on top of verified rows at 320px while the page still fits horizontally, silently obscuring content that the no-scroll assertion would pass. **keyboard** — any offered recovery action must be natively focusable (`<a>`/`<button>`, a `<div>` fails) and is actually `.focus()`ed and asserted focused, so a pointer-only affordance cannot pass. Breadth: because the a11y test runs in all four guarded phases, these assertions held at both viewports against FOUR genuinely different backend states in this run — `painted=ready`, `painted=true-empty`, `painted=store-unavailable`, `painted=disabled`. Honest boundary: this item is closed on FOUR live states at both viewports. The full ten-state matrix named by SCN-080-001-08 is closed separately by T080-08-MATRIX, which drives all eleven closed states through the real status node at both viewports as `ui-unit`, not as a live claim.
- [x] The consumer sweep is complete, existing Wiki journeys remain usable, and spec 105 remains blocked until this bug is certified. → Evidence: report.md#consumer-sweep. All three clauses verified this session. **Sweep** — the SCOPE-04 plan names three stale-reference classes and each was scanned first-hand. (a) *nil-handler*: no `TopicsHandlers`/`PeopleHandlers`/`PlacesHandlers`/`TimeHandlers`/`EdgesHandlers` nil-branch exists anywhere in non-test code; the graph manifest registers atomically behind one `deps.GraphCapability != nil` guard, and `NewGraphCapability` returns `&GraphCapability{...}` unconditionally while every wiring branch assigns it — so the routes are ALWAYS mounted and a silent Chi 404 is unreachable, which is this bug's core fix. The `!= nil` hits that remain belong to unrelated subsystems (Annotation, List, Recommendation, Drive, Photos, Notification, AuthAdmin) and are outside this packet. (b) *route-missing-as-empty*: every remaining 404 site in `web/pwa/*.js` was read, not just counted. `wiki_lib.js:109` maps a 404 on the capability probe to `{available:false, reason:"not-deployed"}` — route-absent semantics, never "empty" — which is correct for a capability probe as opposed to a row read. `wiki_state.js` now discriminates typed `not_found` from a bare 404 (F-080-06-ROWMISS). No site treats a 404 as an empty graph. (c) *static Wiki-ready*: `data-wiki-ready` is set in exactly ONE place, `wiki.js:30`, the landing page — no static or duplicated readiness claim exists. That single-writer fact is load-bearing and was confirmed the hard way: a test of mine wrongly waited on it from a DETAIL page, which never sets it. **Wiki journeys usable** — T080-REGRESSION passes inside the `74 passed` suite, walking all four graph surfaces plus the pre-existing Wiki landing journey. **Spec 105 gated** — its `state.json` records verbatim "BUG-080-001 must be certified done before SCOPE-01 pickup"; 105 sits at planning depth (`in_progress`, `completedScopes` empty) with no implementation scope picked up, so the gate is recorded AND holding. Correction worth recording: 105 also carries a stale note claiming this bug has an unresolved design-staleness gap; this packet's own `notes` show that gap was RESOLVED by `bubbles.design` (2026-07-24T18:25Z) with the '## Corpus Ownership And Authorization Model' section grounding SCN-080-001-09 and the operator-owned global-corpus grant model, and plan coverage added by `bubbles.plan` (2026-07-24T18:49Z). The gate itself stands; only 105's stated reason is out of date.
...
- [x] T080-05-UI passes with current-session raw evidence and screenshot references in `report.md#t080-05-ui`. → Evidence: report.md#t080-05-ui (true-empty phase `5 passed (4.9s)` inside `[web-e2e-ui] true-empty phase: PASS (20s)`; same lane run also `73 passed (25.3s)`, store-unavailable `PASS (17s)`, graph-disabled `PASS (130s)`, `LANE_EXIT=0`), live-stack `./smackerel.sh test e2e-ui` with no request interception. Commit `23396c5c` (`gitleaks`-clean) gave the lane a fourth phase that induces the state this row needs. Until it landed, the lane could only present a seeded backend, so this spec's three-way branch **always took the ready arm** — the true-empty arm had never executed against a real stack. **It cannot be faked:** the phase carries a precondition guard that REFUSES to run the specs unless all three families (topics, people, places) answer `200` with an empty array, checked both before and after; a non-empty or non-`200` family fails the lane loudly rather than silently re-running the ready arm, and the failure propagates **non-zero** (first-failure-wins, no advisory bypass). **The state is restored, not left mutated:** the phase restarts `smackerel-core` so the idempotent boot seed re-runs, waits on the health probe (`healthy again (after 4 probe(s))`), and asserts the seed is back — `assert_seeded_taxonomy_restored` requires HTTP 200 with a NON-EMPTY `items` array. **Precision, corrected by the independent `bubbles.test` pass:** it does NOT compare returned ULIDs against the pre-delete ids, so calling it a fresh-ULID assertion overstated the mechanism. The freshness conclusion still holds, but BY CONSTRUCTION rather than by comparison: the before-specs guard proved all three families were `"items":[]`, so every row observed afterwards must have been newly inserted. The ULIDs (`01M05Y1DTK…`) are printed for inspection, not asserted against. The 82-test suite that follows therefore still sees the seeded fixture it expects, and held at `73 passed` with no regression. Screenshot requirement met by the same unconditional `testInfo.attach` captures cited in `T080-04-UI`/`T080-06-UI` (Playwright's per-test output directory is gitignored, so these are cited as produced-and-attached run artifacts rather than committed files).
- [x] T080-06-ROWMISS passes with current-session raw evidence in `report.md#f-080-06-rowmiss`. → Evidence: report.md#f-080-06-rowmiss (fix run: true-empty phase `6 passed (6.2s)`, full suite `74 passed (26.6s)`, store-unavailable `6 passed (7.0s)`, graph-disabled `6 passed (5.2s)`, `LANE_EXIT=0`), live-stack `./smackerel.sh test e2e-ui` with no request interception. This row exists because SCOPE-04 verification surfaced a REAL defect in this bug's own deliverable: `wiki_state.js` classified EVERY 404 as `CODE_ROUTE_ABSENT`, ignoring the typed envelope, so a stale/deleted topic link claimed "it is not deployed" — false, and dead-ending because route-absent carries no recovery action. **Reproduction gate satisfied with the same probe on both sides:** BEFORE `probeStatus=404 probeCode=not_found painted=route-absent` → FAIL; AFTER `probeStatus=404 probeCode=not_found painted=degraded` → PASS. The only variable was commit `cb9f9ad0`. **Provably non-tautological:** in the very same run the other arms pass — store-down `probeCode=store_unavailable painted=store-unavailable`, disabled `probeCode=capability_disabled painted=disabled` — because every assertion is anchored to what the API actually answered rather than a hardcoded expectation, so the test fails ONLY in the buggy condition. **Non-regression:** full suite `73 → 74` passing, exactly +1 (this test), zero failures. `UNIT_EXIT=0` (the `web/pwa/tests` drift suite still `ok`), `LINT_EXIT=0`, `FMT_EXIT=0`, `regression-quality-guard` `0 violation(s), 0 warning(s)`. Honest boundary: an early red run on this test was an INVALID measurement — it timed out on `data-wiki-ready`, a landing-page-only signal the detail view never sets — and was corrected to `#wiki-topic-detail[aria-busy="false"]` before any fix was written, so no fix was validated against a broken harness.
- [x] T080-06-UI passes with current-session raw evidence, DOM/accessibility/pixel privacy checks, and screenshots in `report.md#t080-06-ui`. → Evidence: report.md#t080-06-ui (phase-1 `73 passed (32.2s)`; store-unavailable phase `5 passed (7.0s)` inside `[web-e2e-ui] store-unavailable phase: PASS (24s)`; graph-disabled phase `5 passed (5.1s)` inside `[web-e2e-ui] graph-disabled phase: PASS (168s)`; whole lane `LANE_EXIT=0`), live-stack `./smackerel.sh test e2e-ui` with no request interception. Commits `9e3f82ac` (live store-unavailable coverage) and `49202e07` (pixel-privacy screenshot), both `gitleaks`-clean, make the five `graph-activation.spec.ts` specs run against **three genuinely different backend states** — enabled, store-down, and disabled — each induced-fault phase gated by a precondition guard that refuses to run the specs unless the stack actually reports the target state; the guard is load-bearing because these specs are **state-adaptive** and would otherwise pass by silently taking their healthy arm. **Auth:** a REAL server-issued 401 induced by `context.clearCookies()` (no interception, no fixture), with exclusivity asserted against `true-empty`, `route-absent`, and `store-unavailable`. **Store:** the store-unavailable contract is now asserted live — the lane records what the browser painted, `painted=store-unavailable | rows=0 | action=<button>`: no fabricated rows, an offered recovery action that is natively focusable, an assertive `role="alert"`, and no `/api/` or `HTTP <code>` leak in user-visible status. Deliberate CONTRAST with `disabled`, which offers NO retry because retrying a capability the operator switched off can never help. **DOM privacy:** prior rows removed (`.wiki-list-item` count `0`) with no `data-people-count` survivor. **Accessibility:** exactly one live region per paint, assertive announcement for faults. **Pixel privacy:** full-page screenshot `auth-loss-privacy.png`, **22047 bytes**, PNG 1280x720, attached via `testInfo.attach` AFTER every privacy and accessibility assertion had already held (produced-and-attached run artifact; Playwright's per-test output directory is gitignored, so it is not a committed file). The store phase's guard requires BOTH HTTP `503` AND a body carrying `"code":"store_unavailable"`, and runs BEFORE **and** AFTER the specs, proving the state held for the whole run rather than only at entry; it reuses the already-running phase-1 stack and costs ~24s instead of another rebuild/boot cycle. Guards executed this session: `regression-quality-guard.sh` on the spec → `0 violation(s), 0 warning(s)`; `lint` exit `0`; `format --check` exit `0`. **This closes the TEST-EVIDENCE row only** — every element the row names (current-session raw evidence, DOM/accessibility/pixel privacy checks, screenshots) is satisfied. The **SCN-080-001-06 Core Outcome stays unchecked** because `route-absent` and `schema-invalid` were NOT induced — they would need build variants that do not exist — so the scenario's four-class claim remains unproven, as does the full ten-state matrix of SCN-080-001-08.
- [x] T080-08-A11Y passes on desktop and narrow viewport with current-session evidence in `report.md#t080-08-a11y`. → Evidence: report.md#t080-08-a11y (`✓  77 graph-activation.spec.ts:311:3 › ... Knowledge Graph activation states remain keyboard and screen-reader operable at desktop and 320px 200 percent zoom (2.2s)`, in a run ending `73 passed (57.1s)`), live-stack `./smackerel.sh test e2e-ui` with no request interception. **Line citations corrected 2026-08-17 (F-AUD-12).** The `:311:3` figure above is retained deliberately: it is verbatim historical Playwright output from the run that produced this evidence, and rewriting it would falsify the transcript. The PRESENT-TENSE structural citations below had drifted and now point at the code they actually describe — the test is at `:501`, and the old `:314-318` / `:341-361` referred to positions the file no longer holds. Both required viewports ran inside the one test: `graph-activation.spec.ts:505-508` iterates `{1280x800 desktop}` then `{320x640 "320px/200%"}`, so a failure at either width would have failed it. Asserted unconditionally at BOTH widths: painted state within the closed vocabulary, no horizontal page scroll (`scrollWidth > clientWidth + 1` is `false`), and any offered recovery action natively focusable (`<a>`/`<button>`) and actually focused. The status arm at `:531-551` is state-dependent and BOTH arms assert — not a bailout; the `STATE_READY` arm executed here (status not visible, live-region count `0`), which is the correct contract because `wiki_state.js` gives `READY` no message. Honest boundary: this asserted the state-appropriate contract for the states actually painted, NOT all ten states named by SCN-080-001-08 — the ten-state claim belongs to SCN-080-001-08, which has since been CLOSED by T080-08-MATRIX driving all eleven closed states through the real status node at both viewports. Unlike T080-04/05/06-UI this row's text requires no screenshot references, so the `only-on-failure` policy leaves no stated requirement unmet.
- [x] T080-06-RENDER passes with current-session raw evidence in `report.md#t080-06-render`. → Evidence: report.md#t080-06-render (`8 passed` in EVERY lane phase; full suite `76 passed`; `LANE_EXIT=0`). Executes the REAL `wiki_state.js` module in a REAL browser against a REAL DOM node — the shipped functions with real inputs, nothing mocked — and is classified `ui-unit`, NOT `e2e-ui`, because it induces states the server cannot produce. It therefore satisfies no live-stack row. Observed classifier matrix, printed by the test itself — **all ELEVEN probes, transcribed verbatim (corrected 2026-08-17, F-AUD-08: `bare503` was previously omitted)**: `bare404=ROUTE-ABSENT`, `typed404NotFound=ROW-MISSING`, `plain400=SCHEMA-INVALID`, `cursor400=CURSOR-INVALID`, `schema500=SCHEMA-INVALID`, `plain500=SERVER-ERROR`, `disabled503=CAPABILITY-DISABLED`, `store503=STORE-UNAVAILABLE`, `bare503=SERVER-ERROR`, `unauth401=UNAUTHENTICATED`, `forbidden403=FORBIDDEN`. Eleven probes resolve to NINE distinct state values, and that collapse is intended, not a defect: an untyped 400 and a schema-500 are both `SCHEMA-INVALID`, and an untyped 500 and a bare 503 are both `SERVER-ERROR`, because in each pair the server has given the client no basis to distinguish them. The earlier wording here said "all ten states declare distinct values", which was wrong on both counts — ten should be eleven, and the values are deliberately NOT all distinct. What IS asserted, and is the load-bearing claim, is that every MEANING-CARRYING pair is distinct: bare-404 vs typed-404 (the F-080-06-ROWMISS defect), disabled vs store-down, and 401 vs 403. **The affordance contract is the sharp part.** A first draft failed on "store-unavailable must offer a retry" — my assertion was wrong, not the product: `renderReadState` paints a retry button only when the caller supplies a real `onRetry`, because a dead retry button is worse than none. Rather than relax it, each state is now rendered TWICE: `route-absent`/`schema-invalid`/`disabled` offer ZERO retry EVEN WHEN a handler is available (proving the refusal is deliberate, not incidental); `store-unavailable` offers none without a handler and exactly one with; `true-empty` offers an unconditional LINK to `/pwa/connectors.html`. The weaker assertion would have passed even if route-absent had silently begun offering a useless retry.
- [x] T080-08-MATRIX passes with current-session raw evidence in `report.md#t080-08-matrix`. → Evidence: report.md#t080-08-matrix (`8 passed` in EVERY lane phase; full suite `76 passed`; `LANE_EXIT=0`). Drives all ELEVEN closed states (the ten named by SCN-080-001-08 plus `loading`) through the REAL `#wiki-topics-status` node on the REAL page at BOTH viewports — `1280x800` and `320x640` — for 22 measured combinations. The induction is unit-level; the layout, geometry and accessibility tree are genuine. Each combination asserts: declares its own state; no horizontal page scroll; no geometric overlap with the family rows; no surviving prior label; at most one live region (a duplicate alert is a defect); and for any recovery action, that it is natively focusable AND actually takes focus through the real browser (the "no pointer-only action" clause). States render in sequence into the SAME node deliberately — that is what makes the leaked-prior-label clause testable at all, since a fresh node per state would make it vacuous. Classified `ui-unit`, not `e2e-ui`.
