...
- [ ] SCN-080-001-03: The product-owned synthetic executes its fixed family sequence and emits one value-safe row per required family plus one aggregate result, failing acceptance for any 401, 403, 404, 5xx, schema, cursor, or missing-row outcome. → OPEN — first half proven, second half not. The fixed sequence and the one-row-per-family-plus-aggregate contract are proven by report.md#t080-03-stress (160/160 runs, 1280 family reads, canonical order enforced by `Aggregate.Validate()`) and report.md#t080-03-synth. Of the acceptance-failure list, only `401`/`403` has executed evidence (the `T080-03-SYNTH` rejection arm swaps ONLY the credential and asserts `unavailable` with a real 401/403 family row). **Unproven clause: acceptance failure for `404`, `5xx`, `schema`, `cursor`, and `missing-row`.** report.md#t080-03-trace exercises those codes only as telemetry INPUTS to prove the observability vocabulary is closed — that proves the codes are reportable, not that an aggregate carrying one is refused. See report.md#t080-03-stress → "Unproven clause". Claim text deliberately NOT narrowed.
