...
| 2026-08-17 | The SCOPE-04 `[hidden]` reset does not cover the sibling `.hidden` CLASS mechanism. `.hidden { display: none }` (`style.css:97`) and `.btn { display: inline-flex }` (`style.css:231`) are both specificity 0-1-0, so the later `.btn` wins the cascade tie — and `assistant.js` toggles only the class, never the attribute, so `#assistant-retry-btn` (`assistant.html:62`, `class="btn btn-secondary hidden"`) stays painted while marked hidden | **ROUTED, THEN RESOLVED OFF-PACKET (updated 2026-08-17 per AUD-080-001-REGISTER-001).** Originally routed — correctly — because correcting it changes observable assistant behaviour and needed its own reproduction and regression test rather than riding along inside a simplify pass. That routing was HONOURED, not bypassed: the fix landed as a STANDALONE commit `4bd2ade8`, outside this packet's boundary, with its own adversarial guard (`web/pwa/tests/pwa_hidden_cascade_guard_test.go`) proven by mutation — reverting the fix yields `FAIL: TestHiddenUtilityBeatsLaterEqualSpecificityDisplay`, restoring it yields `ok`. The original "NOT browser-verified" caveat was also discharged: the full `e2e-ui` lane ran green (`LANE_EXIT=0`, 76 passed) with the fix in place. This row previously still read "deliberately not fixed here" after the fix had shipped, which under-reported the outcome. | `web/pwa/style.css:97,231`; `web/pwa/assistant.html:62`; `web/pwa/assistant.js`; commit `4bd2ade8` |
...
**Measured at HEAD `8b9baf42`, and stable across every commit since `44c9ae36`:** `state-transition-guard.sh` reports
