...
| SCN-071-A09 | stress | (planned: tests/stress/intenttrace/retention_sweep_stress_test.go — 100k-rows stress harness was deferred to bubbles.test follow-up per state.json `discoveredIssues` 2026-06-02; integration-tier proof via `tests/integration/assistant/intent_trace_retention_test.go::TestIntentTraceRetentionSweepRemovesExpiredAndKeepsFresh` is the active SLA coverage today) | `Stress: retention sweep keeps p95 sweep duration < retention_sweep_interval under 100k expired rows` | `./smackerel.sh test stress` | Yes | SLA proof for retention TTL enforcement (sweep must complete inside the configured interval). |
