...
| SCN-071-A09 | stress | `tests/stress/intenttrace/retention_sweep_stress_test.go` | `Stress: retention sweep keeps p95 sweep duration < retention_sweep_interval under 100k expired rows` | `./smackerel.sh test stress` | Yes | SLA proof for retention TTL enforcement (sweep must complete inside the configured interval). |
...
- [x] SLA stress coverage: `tests/stress/intenttrace/retention_sweep_stress_test.go` proves the retention sweep stays within the configured `retention_sweep_interval` at 100k expired rows.
