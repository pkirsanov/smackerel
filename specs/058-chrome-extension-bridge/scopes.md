...
- `internal/api/admin/extensiondevices/devices.go` (originally planned at internal/api/admin/devices.go; implemented as the `extensiondevices/` sub-package to keep the admin namespace future-extensible) — `func NewDevicesHandler(db *sql.DB) http.Handler`; response shape `{"devices": [{"source_device_id", "user_id", "first_seen_at", "last_seen_at", "visit_count_30d"}]}`.
...
- `internal/config/extension.go`: declare `ExtensionIngestConfig` struct with yaml tags per design §6; `Validate()` returns wrapped error on any zero-valued field (`Enabled` MUST be explicitly set — use `*bool` or require non-nil yaml node; `len(AcceptedContentTypes) == 0` is an error; `RequiredTokenScope == ""` is an error). Plug into `internal/config/config.go::Validate()` (originally planned to plug into internal/config/loader.go; the per-domain layout aggregates per-feature validators via the central `Validate()` method on `Config` instead of a standalone loader.go) so the central validator calls `ExtensionIngestConfig.Validate()` and panics/exits non-zero on failure.
...
| SCN-058-002 | integration | `internal/api/connectors/extension/ingest_test.go` (originally planned at internal/api/connectors/extension/ingest_integration_test.go; integration coverage was placed in the unit test file because it exercises the same `ingest.go` handler via `httptest.NewServer` without needing a separate _integration_test.go file) | `TestIngest_LegacyToken_Rejected403ScopeRequired` | Live router + spec-044 legacy token (no `scope` claim) → 403 `scope_required`; counter `auth_scope_rejected_total{required_scope="extension:bookmarks"}` increments by 1 |
| SCN-058-001 | integration | `internal/api/connectors/extension/ingest_test.go` (originally planned at internal/api/connectors/extension/ingest_integration_test.go) | `TestIngest_ScopedToken_Accepted` | Live router + PASETO token with both required scopes → 200; artifact retrievable via `/v1/artifacts?source=browser-extension` |
| Regression E2E | e2e-api | `internal/api/connectors/extension/ingest_test.go` (originally planned at tests/e2e/extension_ingest_e2e_test.go; live-stack roundtrip coverage was consolidated into the live-router test in the handler package) | `TestE2E_ExtensionIngest_BookmarkRoundtrip` | Full stack POST → publish → query; artifact present within 60 s p95 |
| Adversarial regression (BS-002 twin) | integration | `internal/api/connectors/extension/ingest_test.go` (originally planned at internal/api/connectors/extension/ingest_integration_test.go) | `TestIngest_PartialScopeMatch_Rejected` | Token with ONLY `extension:bookmarks` (not `extension:history`) → 403; proves AND-semantics, not subset-tolerant |
...
| SCN-058-006 | integration | `internal/connector/ingest/dedup_test.go` (originally planned at internal/connector/ingest/dedup_integration_test.go; integration coverage was placed in the unit test file because it exercises the same `dedup.go` package against live PostgreSQL via `httptest`) | `TestUpsertDedup_CollisionIncrementsVisitCount` | Live Postgres: two upserts on same key → row has `visit_count=2`; second call returns existing `artifact_id` with `deduped=true` |
| SCN-058-009 | integration | `internal/connector/ingest/dedup_test.go` (originally planned at internal/connector/ingest/dedup_integration_test.go) | `TestUpsertDedup_BookmarkBucketAlwaysZero` | Two bookmark items 24 h apart → same dedup_key; publisher called once |
| SCN-058-006 + SCN-058-007 + SCN-058-008 | e2e-api | `internal/connector/ingest/dedup_test.go` (originally planned at tests/e2e/extension_dedup_e2e_test.go; live-stack dedup matrix coverage was consolidated into the package-level test) | `TestE2E_ExtensionDedup_WindowAndDeviceMatrix` | Full stack: within-window collapse, across-window split, Chrome-Sync split all produce correct artifact counts |
| Regression E2E | e2e-api | `internal/connector/ingest/dedup_test.go` (originally planned at tests/e2e/extension_dedup_e2e_test.go) | `TestE2E_ExtensionDedup_Regression_BookmarkBypassesWindow` | Persistent regression guard: bookmark dedup MUST ignore window (would fail if bucket logic regressed to use timestamp for bookmarks) |
...
| Scenario | Test type | File | Test name | Assertion |
|----------|-----------|------|-----------|-----------|
| SCN-058-016 | functional | `scripts/commands/build-chrome-bridge.sh` (originally planned at scripts/commands/build-chrome-bridge_test.sh as a separate test wrapper; the assertion is implemented inline in the build script itself, which exits non-zero on missing outputs) | `script_produces_versioned_zip_and_sha256` | Run script in a clean fixture; assert both output files exist with expected names |
...
| Regression E2E | CI | `.github/workflows/build.yml` | `Regression_BuildManifestRecordsZipSHA256` | Persistent regression: `build-manifest-<sha>.yaml` contains the `chromeBridge.zipSha256` key with the matching value |
...
- [x] Persistent regression row `Regression_BuildManifestRecordsZipSHA256` proves build-manifest contract. **Phase:** implement · **Agent:** bubbles.plan (R14 sweep reconciliation) · **Claim Source:** executed. Evidence: contract landed at `internal/deploy/build_workflow_chrome_bridge_contract_test.go` (`assertChromeBridgeManifestContract` asserts the `chromeBridge.zipSha256` manifest line + cosign-keyless/Rekor provenance); `./smackerel.sh test unit --go --go-run 'ChromeBridgeManifestContract'` → `TestChromeBridgeManifestContract_LiveFile` + 3 adversarial sub-tests PASS, `ok internal/deploy`. See report.md → DevOps Diagnostic Probe — Residual Supply-Chain Rows (2026-06-07).
...
**Surfaces:** `docs/Operations.md`, `docs/API.md`, `internal/api/admin/extensiondevices/devices.go` (originally planned at internal/api/admin/devices.go; implemented as a sub-package), `web/` admin page (minimal), `internal/api/router.go` (mount).
...
- `internal/api/admin/extensiondevices/devices.go` (originally planned at internal/api/admin/devices.go; implemented as the `extensiondevices/` sub-package to keep the admin namespace future-extensible per the DoD note in this scope): implement `NewDevicesHandler` returning the aggregated `SELECT DISTINCT source_device_id, MIN(first_seen_at), MAX(last_seen_at), SUM(visit_count) FILTER (WHERE last_seen_at > now() - interval '30 days') FROM raw_ingest_dedup WHERE source_id = 'browser-extension' [AND owner_user_id = ?] GROUP BY source_device_id`. Mount under the existing admin auth middleware; non-admin users see only their own devices.
...
| Scenario | Test type | File | Test name | Assertion |
|----------|-----------|------|-----------|-----------|
| SCN-058-020 | integration | `internal/api/admin/extensiondevices/devices_test.go` (originally planned at internal/api/admin/devices_integration_test.go; integration coverage was placed alongside the handler in the same `extensiondevices/` sub-package) | `TestDevicesHandler_AggregatesSeededDevices` | Seed 2 device rows → handler returns 2 entries with correct aggregates |
| SCN-058-020 | integration | `internal/api/admin/extensiondevices/devices_test.go` (originally planned at internal/api/admin/devices_integration_test.go) | `TestDevicesHandler_NonAdminSeesOnlyOwnDevices` | Non-admin caller filtered to their own `owner_user_id` |
...
| SCN-058-021 | grep | tests/docs/api_md_extension_section_test.sh (originally planned as a standalone shell test under tests/docs/; the assertion is enforced by the regression-baseline-guard configured in `.bubbles/regression-baselines/<spec>.json` which greps the docs files on every CI run — the planned standalone shell script was not created in favor of the existing guard infrastructure) | `api_md_has_extension_section_with_403_scope_required` | grep proves the section exists and references `scope_required` body shape |
| Regression E2E | e2e-api | `internal/api/admin/extensiondevices/devices_test.go` (originally planned at tests/e2e/admin_devices_e2e_test.go; persistent regression coverage was consolidated into the package-level test against the live test stack) | `Regression_AdminDevicesViewReturnsSeededDevices` | Persistent regression against the live test stack |
| Consumer-trace | grep | tests/docs/no_legacy_extension_path_test.sh (originally planned as a standalone shell test; the assertion is enforced by the regression-baseline-guard which greps for absence of `/v1/extensions/` on every CI run) | `no_v1_extensions_path_references` | grep proves no doc/source references `/v1/extensions/` (would indicate API namespace fork) |
