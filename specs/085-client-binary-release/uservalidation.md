...
- [x] Lane A (<deploy-host> self-host) delivery is performed by the EXISTING knb self-hosted adapter consuming the digest; smackerel never builds clients in an adapter and never calls the knb Lane-A lib directly
...
- [x] Release train set to `mvp` (Lane A targets self-hosted = mvp's slot)
