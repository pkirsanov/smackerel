...
A holistic MVP → evo-x2 home-lab readiness review surfaced nine **in-repo**,
...
the physical evo-x2 host, the knb deploy adapter, or production secrets to be
...
2. **Reliability** — the Ollama model-envelope validator
   ...
   **concurrent sum** of keep-alive-resident interactive models. On home-lab,
...
7. **Operability** — `scripts/deploy/promote.sh` parses the CI build-manifest
   ...
   `scripts/commands/build-home-lab.sh` emits a different shape (`images:`
...
|---|------------------|-------|-----------------|-----------|
| a | On-GPU accel inference validation on physical evo-x2 | operator + hardware | needs the real Strix Halo iGPU | risk R-082-A |
...
| d | Actual deploy to evo-x2 | operator | adapter-driven apply | risk R-082-D |
...
- **R-082-A** — Scope 2 raises the home-lab ollama envelope to fit the
  interactive concurrent set; the true GPU residency of `gemma4:26b` under
  ROCm is only verifiable on the evo-x2 (blocker a). The guard uses published
...
- **FR-082-002** (Scope 2) — `validateModelEnvelopes` MUST additionally reject
  ...
  home-lab envelope MUST be sized to fit its interactive sum; the `gemma`-class
...
- **FR-082-004** (Scope 4) — the `nats-data` volume MUST be labelled to
  ...
  remove `nats-data` on a running home-lab stack.
...
Scenario: SCN-082-D01 — clean cannot wipe queued capture events
  ...
  Then nats-data is excluded from removal on a running home-lab stack
