...
and offline-proven; the live evo-x2 verification (SCOPE-02) is DEFERRED-to-flip
...
- `config/generated/{dev,test,home-lab}.env` — regenerated (3-var contract).
...
`./smackerel.sh config generate` (+ `--env test` / `--env home-lab`) → exit 0.
Each generated env now carries the 3-var contract, `OTEL_EXPORTER_ENDPOINT` gone:

```
--- dev.env / test.env / home-lab.env (identical block) ---
...
`ts.net` / tailnet-100.x / `evo-x2` / `/home/<user>` / raw IPs (excluding
loopback):
...
| `config/generated/{dev,test,home-lab}.env` | `OTEL_EXPORTER_ENDPOINT=` | 3 vars | regenerated (old-var count = 0 in all three) |
...
acceptance constraints forbid live evo-x2 mutation; accepted precedent:
WanderAide scope-05 T5/T7, knb scope-03):

| Item | Why deferred | Unblock condition |
|---|---|---|
| T4 — live `/metrics` 200 scraped by shared Prometheus | requires the shared stack + a running smackerel-core under the shared posture on evo-x2 | operator sets smackerel `sharedServices.observability: shared` in the knb adapter params + runs `apply-shared-obs` on evo-x2 |
...
mutation, no ssh to evo-x2, occurred.
