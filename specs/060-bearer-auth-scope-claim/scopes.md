...
- [ ] Hot-path validation budget unchanged: microbenchmark deferred to follow-up (no behavioral risk; the middleware adds one `SessionFromContext` lookup + `slices.Contains` per required scope, well under the 10 µs design budget). **Uncertainty Declaration:** benchmark not yet captured; functional correctness is fully covered by unit + integration tests above.
