...
Scopes 9 (Calendar-Triggered Briefs) and 10 (Reminder & Promise Engine) are intentionally **post-release deferrals** approved at portfolio-planning level. Both scopes are `SurfacingProposal` producers that hard-depend on the M1a unified surfacing controller (`SurfacingProposalSink` + `AcknowledgmentBus` contracts). That architectural blocker is now **satisfied**: the M1a controller was rescoped out of spec 021 and **delivered as [spec 078 — Cross-Surface Surfacing Prioritizer](../078-cross-surface-surfacing-prioritizer/)** (`state.json` status `done`, `releaseTrain: mvp`), landing on trunk at commit `640b95d0` (`internal/intelligence/surfacing/*`, consumed from `internal/scheduler`). The deferral decision is **unchanged**: both producers **remain intentionally scheduled for the `release-v1` train** per the portfolio-approved exception `DI-025-05` — they are not in the `mvp` producer set and are not yet implemented. Scope 10 additionally sequences after the scope 9 producer pattern (its reference implementation), which is itself `release-v1`-scheduled. They are mirrored in `state.json` under `certification.postReleaseExceptions` and referenced by `discoveredIssues.DI-025-05`. They remain excluded from the spec-level promotion gate by design — not by oversight — and will be implemented on the `release-v1` train; the only prior upstream blocker (the M1a controller) is no longer outstanding.

Unblock gates:

- **Scope 9** — **Satisfied.** The M1a unified surfacing controller (`SurfacingProposalSink` + `AcknowledgmentBus` contracts) was delivered via [spec 078](../078-cross-surface-surfacing-prioritizer/) (commit `640b95d0`) and is consumable from `internal/scheduler`. Remaining work is the `release-v1`-scheduled implementation of the scope 9 producer per `DI-025-05`; no outstanding upstream gate.
- **Scope 10** — M1a unified surfacing controller prerequisite **satisfied** via [spec 078](../078-cross-surface-surfacing-prioritizer/). Remaining prerequisite is the scope 9 calendar-brief producer pattern, which is itself `release-v1`-scheduled per `DI-025-05`; no outstanding upstream contract gate.

Guard behavior (expected until the deferred scopes are implemented on the `release-v1` train): because Scopes 9/10 are deferred, their planned test files (internal/scheduler/calendar_briefs_test.go, internal/knowledge/promises_test.go, internal/scheduler/promises_test.go — paths left un-backticked here because they are intentionally not-yet-authored, matching the Test Plan table convention) do not exist yet. `state-transition-guard.sh` PERMITS the spec-level `done` transition (it accepts the "Done (post-release-deferred)" scope status and its G057 check only verifies that `scenario-manifest.json` records `linkedTests`), but the stricter `traceability-guard.sh` G057/G059 manifest cross-check reports 6 `references missing linked test file` failures for SCN-025-24..29. Those 6 failures are the expected manifestation of the documented deferral, not a regression; they resolve when the scopes are implemented on the `release-v1` train (the M1a controller blocker is already cleared via spec 078). See `discoveredIssues.DI-025-05` in `state.json`.
<!-- bubbles:g040-skip-end -->
