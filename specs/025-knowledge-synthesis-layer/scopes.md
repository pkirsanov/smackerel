...
Guard behavior (expected until the unblock gates land): because Scopes 9/10 are deferred, their planned test files (`internal/scheduler/calendar_briefs_test.go`, `internal/knowledge/promises_test.go`, `internal/scheduler/promises_test.go`) do not exist yet. `state-transition-guard.sh` PERMITS the spec-level `done` transition (it accepts the "Done (post-release-deferred)" scope status and its G057 check only verifies that `scenario-manifest.json` records `linkedTests`), but the stricter `traceability-guard.sh` G057/G059 manifest cross-check reports 6 `references missing linked test file` failures for SCN-025-24..29. Those 6 failures are the expected manifestation of the documented deferral, not a regression; they resolve when the scopes are implemented post-M1a. See `discoveredIssues.DI-025-05` in `state.json`.
<!-- bubbles:g040-skip-end -->
