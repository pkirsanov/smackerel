# Design — 082 MVP / <deploy-host> Readiness Hardening
...
- `environments.self-hosted` overrides: `ollama_memory_limit: "20G"`,
  `llm_model/ollama_model/ollama_vision_model = gemma4:26b`,
  `agent_provider_default_model/fast_model = llama3.1:8b`,
  `agent_provider_vision_model = gemma4:26b`. self-hosted interactive distinct set
...
- `scripts/deploy/promote.sh` awk-parses `images:` LIST + `configBundles:`
  LIST (`- name:`, `- env:`). `scripts/commands/build-self-hosted.sh` emits
...
- self-hosted interactive sum 24576 > 20480 ❌ → caught.
...
dev/test/self-hosted all use `24h`/`-1` → resident → enforced.
...
**Config fix.** Raise `environments.self-hosted.ollama_memory_limit` from `"20G"`
to `"28G"` (24576 interactive sum + ~4 GiB headroom for KV-cache growth; the
<deploy-host> host has ~109 GiB). Document the new floor.
...
documenting the new concurrent-envelope guard + the self-hosted `28G` floor.
...
- `deploy/compose.deploy.yml`: relabel `nats-data`
  ...
  `clean`/`down` never target `nats-data` for removal on a running self-hosted
  stack.
...
- `docs/Deployment.md`: add a "Go-Live Readiness Checklist" section tying
  ...
  (`./smackerel.sh build --target self-hosted`); `--profile ollama` /
...
- **Assessment (recorded here):** `HSA_OVERRIDE_GFX_VERSION=11.5.1` +
  ...
  8060S, gfx1151) that the self-hosted profile targets generically — they are a
  reasonable generic default for the known <deploy-host>-class accel tier and are NOT
