...
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. [Shared Infrastructure Impact Sweep]
   → Evidence: rollback is a single git revert of the BUG-044-001 commit. The change is purely additive at one wiring site (new helper + replaced inline block); home-lab today does not enable revocation broadcast on `cfg.Auth.RevocationNATSSubject != ""`, so the live behavior is unchanged for the canonical home-lab deployment — no live-config mismatch could result from a revert. Restore is the same git revert. Verified by the RED proof step which temporarily reverts the helper body to silent-fallback form, confirms expected FAIL output, then restores. See report.md > Code Diff Evidence + Test Evidence > Red→Green proof (scenario-first TDD).
...
- **Operator-side fan-out:** the home-lab deployment does not enable revocation broadcast today (`cfg.Auth.RevocationNATSSubject` is unset in the canonical config), so the broadcaster wiring block does not even run in production. The fix is risk-free to ship — the runtime behavior is unchanged for the canonical home-lab deployment.
...
- **Rollback path:** see the corresponding DoD item — single `git revert`; no live-config mismatch possible because home-lab today does not enable revocation broadcast.
