...
**Status:** In Progress (adapter bind proven live 2026-06-02; HTTP-200 + finding closure blocked by foreign F-069-USERID-BINDING — SCOPE-2 / route to `bubbles.plan`)  
...
- [ ] `POST /api/assistant/turn` returns HTTP 200 (not 503) for a valid bearer schema-v1 turn against the live wiring, proven by `TestAssistantHTTPAdapterIsBoundInProductionWiring_ReturnsHTTP200NotHTTP503`. **Uncertainty Declaration:** Live-wiring test authored in this turn (`tests/integration/assistant/http_adapter_bind_test.go`). The "not 503" half is proven by the executed e2e `TestAssistantHTTPE2E_TextTurnReturnsSchemaValidResponse` run on 2026-06-02 — the live response is a schema-v1 `TurnResponse` envelope (`schema_version=v1`, `transport=web`, echoed `transport_message_id`) produced by the bound `*HTTPAdapter`, not a 503 with no body (see `report.md#scope-1d` live-stack evidence block). The "HTTP 200" half is blocked by F-069-USERID-BINDING (foreign): `internal/api/router.go::bearerAuthMiddleware` populates `Session.UserID=""` for shared-token bearer auth and `internal/assistant/httpadapter/adapter.go:342` rejects with HTTP 401 `auth_required`. That binding source is owned by SCOPE-2 (middleware implementations — explicitly excluded by SCOPE-1d's Change Boundary). Item stays unchecked pending SCOPE-2 / route to `bubbles.plan`. **Claim Source:** executed (bind proof) / blocked-foreign (HTTP 200).
- [ ] Telegram adapter binding and facade behavior remain unchanged (canary row passes). **Uncertainty Declaration:** Telegram bind code refactored into `wireAssistantTelegramAdapter` helper; structural equivalence verified by `go build ./...` RC=0. The in-process Telegram-shaped canary `TestHTTPAdapterCanary_TelegramAdapterAndFacadeUnchanged` lives in `tests/integration/assistant/http_adapter_canary_test.go` (SCOPE-1a) and already asserts the Telegram facade contract is intact. The dedicated live-stack `TestAssistantHTTPAdapterBindLeavesTelegramAdapterAndFacadeUnchanged` row from this scope's Test Plan was not authored or executed in this turn; left unchecked pending a follow-up bubbles.implement turn or planning reconciliation that moves the canary obligation to the existing SCOPE-1a in-process canary. **Claim Source:** not-run.
...
- [ ] Finding F-069-ADAPTER-NOT-BOUND closed with linked evidence; spec 074 finding F074-04B-ASSISTANT-HTTP-LATE-BIND is recorded as auto-resolved by this scope in `report.md#scope-1d`. **Uncertainty Declaration:** Partial — the literal "LateBoundHandler has no backing adapter → HTTP 503" symptom is resolved (live-stack response is a schema-v1 `TurnResponse` envelope produced by the bound `*HTTPAdapter`, see `report.md#scope-1d` 2026-06-02 evidence block); the same applies to F074-04B. Full closure of F-069 (the version that names the HTTP-200 success path) is blocked by the newly raised F-069-USERID-BINDING (SCOPE-2 territory) — see `report.md#scope-1d` findings list. Item stays unchecked pending route to `bubbles.plan` to either (a) split F-069 into bind-resolved + userid-binding-pending, or (b) move full closure to the post-SCOPE-2 turn. **Claim Source:** executed (bind half) / route-required (full closure).
- [ ] `./smackerel.sh test integration` and `./smackerel.sh test e2e` for the linked tests pass; artifact lint passes for this spec. **Uncertainty Declaration:** Executed in this turn — both suites currently fail GREEN: integration test failed only because the in-process Go runner reads `CORE_EXTERNAL_URL=http://127.0.0.1:45001` (host loopback, unreachable from the test container); test corrected in-place to read `CORE_API_URL=http://smackerel-core:8080` (in-network) and rerun was queued but did not reach the front of the parallel test-suite-lock queue before turn end. E2E executed end-to-end and proved the adapter is bound (schema-v1 envelope, no 503) but returned HTTP 401 `auth_required` due to F-069-USERID-BINDING (foreign blocker, SCOPE-2). See `report.md#scope-1d` 2026-06-02 evidence block. **Claim Source:** executed (e2e) / queued (integration rerun).
