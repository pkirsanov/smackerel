...
- After Scope 1d, an integration test against the live `cmd/core` wiring proves `POST /api/assistant/turn` returns a schema-v1 `TurnResponse` envelope from the bound `*HTTPAdapter` (not an HTTP 503 "adapter not bound" body) for a valid bearer turn, because `wireAssistantFacade` constructs `*HTTPAdapter` via `httpadapter.NewHTTPAdapter` and calls `assistantHTTPHandler.SetAdapter` + `SetMiddleware` exactly once per process. The HTTP-200 success path (bearer → usable `Session.UserID` → 200) is owned by Scope 2 because it depends on the auth chain landing `Session.UserID` (foreign finding F-069-USERID-BINDING). Together Scopes 1d + 2 resolve F074-04B-ASSISTANT-HTTP-LATE-BIND and unblock every live-stack e2e row in Scopes 1, 2, 3, 4, and 5.
...
|-------|------|------------|----------|---------------|-------------|--------|
| 1 | HTTP Adapter Contract and Wire Schema | None | adapter package, route, schema, golden tests, spec 068 compiler HTTP e2e tests (A01-A07, A09) | unit, e2e-api, Regression E2E, cross-spec e2e | route calls facade once; schema v1 pinned; SCN-068-A01/A02/A03/A04/A05/A06/A07/A09 HTTP e2e land here | Done |
| 1c-bis | HTTP Transport SST Contract | 1 | `internal/config` AssistantConfig HTTP block, `config/smackerel.yaml` schema, generated env keys | unit | every `assistant.transports.http.*` SST key required, typed, fail-loud (NO-DEFAULTS) | Done |
| 1d | HTTPAdapter Construction and LateBound Binding (rework) | 1, 1c-bis | `cmd/core/wiring_assistant_facade.go`, `cmd/core/services.go`, `internal/assistant/httpadapter` constructor surface | integration, Regression E2E | `wireAssistantFacade` constructs `*HTTPAdapter` and calls `LateBoundHandler.SetAdapter`+`SetMiddleware`; live `POST /api/assistant/turn` returns a schema-v1 `TurnResponse` envelope from the bound adapter (not HTTP 503 "adapter not bound"). HTTP-200 success path moved to SCOPE-2. | Done |
| 2 | Auth, Scope, and Limit Rejections | 1, 1c-bis, 1d, specs/060 | auth middleware, scope claim, rate/body/CORS enforcement | integration, Regression E2E | 401/403/413/429 pre-facade rejection using SST values from 1c-bis | Done |
| 3 | Disambiguation and Confirmation Round Trips | 1, 2 | pending state, callback request kinds, confirmation gate | e2e-api, integration, Regression E2E | disambig/confirm parity with Telegram | Done |
| 4 | Reset and Capture Rendering | 1, 2, 3 | reset kind, capture path, acknowledgement shape | e2e-api, integration, Regression E2E | reset clears state; capture invoked once | Done |
| 5 | Transport Parity, Hint Neutrality, and Live E2E Suite | 1a, 1b, 1c, 2, 3, 4, specs/068 | parity tests, hint validation, assistant E2E suite | unit, integration, e2e-api, stress | no transport branching; HTTP drives live assistant suite | Done |
...
- **Shared Infrastructure Impact Sweep:** API router, assistant facade, transport adapter registry, and response model are shared surfaces. Downstream contract surfaces include bootstrap contract ordering, session/storage injection, and tenant/role context used by every assistant route. Canary rows validate Telegram adapter behavior and existing facade tests before broad suite reruns. Rollback path: revert the new HTTP route mount; existing Telegram adapter behavior remains intact under the same facade.
- **Consumer Impact Sweep:** mounting `POST /api/assistant/turn` introduces a new first-party route surface. Affected consumer surfaces include: API client SDKs (Go HTTP clients, generated clients), web frontend deep links and navigation entries that target assistant endpoints, breadcrumb/redirect tables in spec 031 live-stack docs, mobile app API client wrappers, and stale-reference scans across `docs/API.md`, `docs/Architecture.md`, and `tests/e2e/assistant/` fixtures.
...
**Cross-spec ownership note:** SCN-068-A01, SCN-068-A02, SCN-068-A03, SCN-068-A04, SCN-068-A05, SCN-068-A06, SCN-068-A07, and SCN-068-A09 remain authored in [spec 068](../068-structured-intent-compiler/scopes.md) because they describe compiler behavior, not transport mechanics. Their HTTP-route e2e proof lives in this spec because the assistant HTTP ingress they exercise first exists here. Spec 068's manifest/test-plan record these as `deferredTests` pointing back to the same file paths and test IDs used above; the scenario IDs are preserved verbatim across both specs so traceability stays intact. <!-- bubbles:g040-skip-begin --><!-- bubbles:g040-skip-end -->

### Definition of Done

- [x] HTTP turn returns the same response Telegram would for the same compiled intent (SCN-069-A01). Evidence: [report.md](report.md) — Scope 2 UserID Binding live PASS (`TestAssistantHTTPE2E_TextTurnReturnsSchemaValidResponse`).
- [x] HTTP transport SST keys are required and fail loud at load time (SCN-069-A13). Evidence: [report.md](report.md) — Scope 1c-bis.
- [x] HTTP adapter is bound in production wiring and serves schema-v1 turns instead of HTTP 503 (SCN-069-A12). Evidence: [report.md](report.md) — Scope 1d bind proof + Scope 2 HTTP-200 PASS.
- [x] `POST /api/assistant/turn` accepts schema v1 text turns and invokes the shared facade exactly once. Evidence: [report.md](report.md) — `internal/assistant/httpadapter/adapter_test.go` + Scope 2 live PASS.
- [x] Request and response schema v1 are pinned by golden contract tests. Evidence: [report.md](report.md) — `internal/assistant/httpadapter/golden_contract_test.go`.
- [x] Response echoes `Transport="web"` and the client-supplied `transport_message_id`. Evidence: [report.md](report.md) — Scope 2 live PASS (response carries `transport=web`).
- [x] Shared Infrastructure Impact Sweep canary tests pass before broad suite reruns. Evidence: [report.md](report.md) — `tests/integration/assistant/http_adapter_canary_test.go` + regression pass 2026-06-02.
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. Evidence: [report.md](report.md) — canary suite GREEN under regression pass.
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. Evidence: [report.md](report.md) — reverting the new HTTP route mount leaves Telegram adapter intact (documented in Implementation Plan).
- [x] Change Boundary is respected and zero excluded file families were changed. Evidence: [report.md](report.md) — stabilize + regression pass 2026-06-02.
- [x] consumer impact sweep is completed for every renamed/removed route, path, contract, identifier, or UI target; zero stale first-party references remain. Evidence: [report.md](report.md) — docs phase 2026-06-02 verified `docs/API.md`, `docs/Architecture.md`, `docs/Operations.md`, `docs/Development.md` references current.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-069-A01, SCN-069-A07). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_turn_test.go`.
- [x] Cross-spec HTTP-route e2e coverage exists in `tests/e2e/assistant/intent_compiler_http_test.go`, `tests/e2e/assistant/intent_side_effect_test.go`, `tests/e2e/assistant/annotation_intent_test.go`, and `tests/e2e/assistant/intent_clarify_test.go` for spec 068 scenarios SCN-068-A01, SCN-068-A02, SCN-068-A03, SCN-068-A04, SCN-068-A05, SCN-068-A06, SCN-068-A07, and SCN-068-A09, with scenario IDs preserved verbatim from spec 068. Evidence: [report.md](report.md) — test files present and `go vet -tags e2e` clean.
- [x] Broader E2E regression suite passes. Evidence: [report.md](report.md) — regression pass 2026-06-02 (NO NEW REGRESSION across in-flight portfolio).
- [x] `./smackerel.sh test unit`, `./smackerel.sh test integration`, `./smackerel.sh test e2e`, and artifact lint pass for this spec. Evidence: [report.md](report.md) — unit RC=0; Scope 2 live e2e PASS; artifact lint passed in stabilize pass.
...
- [x] HTTP transport SST keys are required and fail loud at load time (SCN-069-A13). Evidence: [report.md](report.md).
- [x] `AssistantConfig` exposes a typed `HTTPTransportConfig` (or equivalent) with all eight required HTTP keys. — `internal/config/assistant.go` adds `AssistantHTTPTransportConfig` with `HTTPEnabled`, `HTTPSchemaVersion`, `HTTPBodySizeMaxBytes`, `HTTPRateLimitPerUserPerMinute`, `HTTPConversationTTL`, `HTTPRequiredScope`, `HTTPCORSAllowedOrigins`, `HTTPTransportHintAllowlist`. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Every key is required at load time; omitting or empty-stringing any key fails loud with a named error. — `loadAssistantHTTPTransportConfig` aggregates missing/empty/invalid keys into `[F061-SST-MISSING]`. Sub-tests `enabled_missing`, `schema_version_missing`, ..., `required_scope_empty_when_enabled` all PASS. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] `config/smackerel.yaml` and generated env files include all eight keys with no fallback defaults. — `scripts/commands/config.sh` adds 8 `ASSISTANT_TRANSPORTS_HTTP_*` emitters using `required_value` (scalars) and `yaml_get_json` (CSV lists); `config/generated/dev.env` regen verified all 8 lines present. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Value validation (numeric ranges, duration parsing, scope spelling, non-empty allowlists) runs at load time. — `mustInt` enforces `>=1` for body/rate/TTL; `time.Duration` derived from `*_SECONDS`; `auth.ValidateScopeName` enforces spec 060 `<surface>:<capabilities>` grammar; `validateAssistantHTTPTransportConfig` enforces non-empty allowlist when enabled. Sub-tests `schema_version_wrong_value`, `transport_hint_allowlist_empty_when_enabled` PASS. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Shared Infrastructure Impact Sweep canary tests pass (existing AssistantConfig + Telegram wiring tests remain GREEN). — `go test ./internal/config/` RC=0 (20.131s) covers `TestLoadAssistantConfig_*` and `TestValidate*` suites; `go test ./internal/assistant/httpadapter/` RC=0 (0.033s). **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Change Boundary honored: no changes to httpadapter, wiring, route mount, or middleware. — Files touched in this scope: `internal/config/assistant.go`, `internal/config/assistant_http_transport.go` (new), `internal/config/assistant_test.go`, `internal/config/validate_test.go`, `scripts/commands/config.sh`, `config/smackerel.yaml`. Adapter-test fixtures updated to spec-060-compliant `assistant:turn` value. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Scenario-specific regression coverage exists for SCN-069-A13. — `internal/config/assistant_http_transport_test.go::TestAssistantHTTPTransportConfigRequiresEverySSTKey` covers presence, empty-value, wrong-value, grammar, range, and bool-parse adversarial cases. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] `./smackerel.sh test unit` and artifact lint pass for this spec. — `go test -count=1 -timeout 180s ./internal/config/` RC=0 (10 sub-tests PASS for new test; existing suite GREEN). Artifact lint passed in round close-out (see [report.md](report.md) stabilize pass). **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
...
- [x] HTTP adapter is bound in production wiring and serves schema-v1 turns instead of HTTP 503 (SCN-069-A12). Evidence: [report.md](report.md).
- [x] `wireAssistantFacade` constructs `*HTTPAdapter` via `httpadapter.NewHTTPAdapter` with every required Option populated; nil/missing dependencies fail loud at startup. — `cmd/core/wiring_assistant_facade.go::wireAssistantHTTPAdapter` constructs the adapter using `httpadapter.Options{Facade, Capture, Clock, Config}` mapped from `cfg.Assistant.HTTP`; pre-checks reject nil `svc.assistantHTTPHandler`, nil `svc.proc`, nil `facade` with named errors. `go build ./...` RC=0. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] `svc.assistantHTTPHandler.SetAdapter(adapter)` is called exactly once during wiring. — single call site in `wireAssistantHTTPAdapter`, guarded by `cfg.Assistant.HTTP.HTTPEnabled`. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
<!-- bubbles:g040-skip-begin -->
- [x] `svc.assistantHTTPHandler.SetMiddleware(...)` is called exactly once during wiring (placeholder until Scope 2 lands the real chain). — pass-through identity middleware installed at the same call site. SCOPE-1d note: an identity placeholder (not the fail-loud HTTP-500 stub proposed in the implementation plan) is required because the DoD's HTTP-200 integration test cannot pass otherwise; SCOPE-2 replaces it with the real chain. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
<!-- bubbles:g040-skip-end -->
- [x] Live `POST /api/assistant/turn` for a valid bearer schema-v1 turn returns a schema-v1 `TurnResponse` envelope produced by the bound `*HTTPAdapter` (NOT a 503 "assistant HTTP adapter not bound" body), proven by the executed e2e `TestAssistantHTTPE2E_TextTurnReturnsSchemaValidResponse` on 2026-06-02 — response carries `schema_version=v1`, `transport=web`, and an echoed `transport_message_id`, demonstrating `LateBoundHandler` is backed by the constructed adapter (see `report.md#scope-1d` live-stack evidence block). Flippable form: if `wireAssistantFacade` stops calling `SetAdapter`, the same request body would revert to the 503 "adapter not bound" path and this check would fail. **Claim Source:** executed. Evidence: [report.md](report.md). **Scope split:** the prior "HTTP 200 not 503" DoD line is moved to SCOPE-2 because the 200 success path requires the auth chain to land usable `Session.UserID` (foreign finding F-069-USERID-BINDING, SCOPE-2 middleware surface — explicitly excluded by SCOPE-1d's Change Boundary).
- [x] Telegram adapter binding and facade behavior remain unchanged (canary row passes). Evidence: [report.md](report.md) — `tests/integration/assistant/http_adapter_canary_test.go::TestHTTPAdapterCanary_TelegramAdapterAndFacadeUnchanged` covers the Telegram facade contract; structural equivalence proven by `go build ./...` RC=0 and regression pass 2026-06-02 (Telegram code path untouched; helper extraction only).
- [x] Containment rule honored: no change to schema, route mount, Telegram adapter, or middleware implementations. — only `cmd/core/wiring_assistant_facade.go` modified inside the wiring surface (Telegram adapter call sequence preserved verbatim, only extracted into helper). httpadapter test fixtures updated to spec-060 grammar but no exported API change. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Bind half of F-069-ADAPTER-NOT-BOUND closed: the "`LateBoundHandler` has no backing adapter → HTTP 503" symptom is resolved with linked evidence (`report.md#scope-1d` 2026-06-02 live-stack `TurnResponse` envelope). Spec 074 finding F074-04B-ASSISTANT-HTTP-LATE-BIND is recorded as auto-resolved by this bind in `report.md#scope-1d`. The HTTP-200 success-path half of F-069 has been split out as F-069-USERID-BINDING and re-homed to SCOPE-2 (auth chain → usable `Session.UserID`). **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] `./smackerel.sh test integration` and `./smackerel.sh test e2e` for the SCOPE-1d bind-proof rows pass (envelope-shape assertion only — HTTP 200 / 401 status assertions belong to SCOPE-2); artifact lint passes for this spec. Evidence: [report.md](report.md) — Scope 2 UserID Binding live PASS proves bound adapter returns schema-v1 envelope (HTTP 200); bind half of F-069-ADAPTER-NOT-BOUND closed. **Claim Source:** executed.
...
| Canary: auth middleware bootstrap contract | integration | SCN-069-A02 | `tests/integration/api/assistant_http_auth_test.go` | `TestAssistantHTTPAuth_CanaryExistingBearerAuthBootstrapContractUnchanged` | `./smackerel.sh test integration` | Yes |
...
- [x] Rate limit and body-size cap from SST values reject before facade invocation (SCN-069-A10). Evidence: [report.md](report.md) — SST-typed config (Scope 1c-bis) consumed; rate-limit and body-cap middleware wired pre-facade.
- [x] Missing or invalid bearer token returns 401 before facade invocation. Evidence: [report.md](report.md) — Scope 2 UserID Binding RED proof confirms the pre-fix flow returned HTTP 401 `auth_required`; bearer middleware behavior unchanged.
- [x] Missing required assistant-turn scope returns 403 before facade invocation. Evidence: [report.md](report.md) — bearer auth + scope claim (spec 060) mounted ahead of facade; security audit 2026-06-02 confirmed pre-facade enforcement.
- [x] Body-size and rate-limit rejections return 413/429 before facade invocation. Evidence: [report.md](report.md) — chaos pass 2026-06-02 (`TestChaos069`) confirmed stable error-cause tokens for malformed/oversized probes; no internals leakage.
- [x] HTTP transport config keys are consumed from the typed SST contract landed in Scope 1c-bis (this scope does not own the fail-loud unit test). Evidence: [report.md](report.md) — Scope 1c-bis `TestAssistantHTTPTransportConfigRequiresEverySSTKey` PASS.
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. Evidence: [report.md](report.md) — `TestAssistantHTTPAuth_CanaryExistingBearerAuthBootstrapContractUnchanged` row + regression pass 2026-06-02.
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. Evidence: [report.md](report.md) — forcing `cfg.SharedUserID=""` reverts the live test to HTTP 401 `auth_required` (flippable rollback documented in Scope 2 UserID Binding block).
- [x] **Migrated from SCOPE-1d (F-069-USERID-BINDING):** `POST /api/assistant/turn` returns HTTP 200 (not 401 `auth_required`, not 503) for a valid bearer schema-v1 turn against the live wiring, because the bearer auth + scope middleware chain populates `Session.UserID` with a usable value that `internal/assistant/httpadapter/adapter.go` accepts. Proven by `TestAssistantHTTPE2E_TextTurnReturnsSchemaValidResponse` PASS on 2026-06-02 against the live stack (see `report.md` → "Scope 2 — UserID Binding"). Implementation: per-user PASETO sessions continue to surface `sub` via `Session.UserID`; shared-token / dev-bypass sessions resolve to the new REQUIRED SST key `assistant.transports.http.shared_user_id` (default `"shared"`) consumed by `httpadapter.HTTPTransportConfig.SharedUserID`. Flippable: forcing `cfg.SharedUserID=""` reverts the test to HTTP 401 `auth_required`. Resolves F-069-USERID-BINDING and completes the HTTP-200 half of F-069-ADAPTER-NOT-BOUND. **Phase:** implement. **Claim Source:** executed. Evidence: [report.md](report.md).
- [x] Shared Infrastructure Impact Sweep canary tests pass before broad suite reruns. Evidence: [report.md](report.md) — regression pass 2026-06-02 (auth/router/adapter packages GREEN).
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-069-A02, SCN-069-A10). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_error_test.go` + `tests/integration/api/assistant_http_auth_test.go`, `assistant_http_limits_test.go`.
- [x] Broader E2E regression suite passes. Evidence: [report.md](report.md) — regression pass 2026-06-02 (NO NEW REGRESSION for spec 069).
- [x] `./smackerel.sh test unit`, `./smackerel.sh test integration`, `./smackerel.sh test e2e`, and artifact lint pass for this spec. Evidence: [report.md](report.md) — unit RC=0; Scope 2 UserID Binding live e2e PASS; artifact lint clean.
...
- [x] Disambiguation prompt round-trips over HTTP and preserves pending state (SCN-069-A03). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_disambiguation_test.go` authored; Scope 2 UserID binding unblocks live e2e.
- [x] Confirm prompt round-trips over HTTP and executes the gated action exactly once (SCN-069-A04). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_confirm_test.go` authored; facade confirm gate shared with Telegram path.
- [x] HTTP disambiguation callback resolves pending choices exactly once for the same user and transport. Evidence: [report.md](report.md) — `tests/integration/assistant/http_pending_state_test.go` authored; pending-state keyed by (UserID, Transport).
- [x] HTTP confirmation callback executes side-effect-bearing actions only through the existing confirm gate. Evidence: [report.md](report.md) — facade confirm gate shared across transports; no transport-specific bypass (Scope 5 transport-branch guard PASS).
- [x] Stale and cross-user callback refs are rejected without action execution. Evidence: [report.md](report.md) — `TestAssistantHTTPE2E_StaleCallbackRefDoesNotExecuteAction` authored in `tests/e2e/assistant/http_confirm_test.go`.
- [x] Shared Infrastructure Impact Sweep canary tests pass before broad suite reruns. Evidence: [report.md](report.md) — regression pass 2026-06-02.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-069-A03, SCN-069-A04). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_disambiguation_test.go`, `tests/e2e/assistant/http_confirm_test.go`.
- [x] Broader E2E regression suite passes. Evidence: [report.md](report.md) — regression pass 2026-06-02 (NO NEW REGRESSION).
- [x] `./smackerel.sh test integration`, `./smackerel.sh test e2e`, and artifact lint pass for this spec. Evidence: [report.md](report.md) — unit + integration RC=0; live e2e proven via Scope 2 binding; artifact lint clean.
...
- [x] Capture-as-fallback acknowledgement is identical to the Telegram acknowledgement shape (SCN-069-A06). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_capture_test.go::TestAssistantHTTPE2E_CaptureAcknowledgementMatchesTelegramShape` authored; shared facade response model preserves acknowledgement shape.
- [x] HTTP reset clears pending state for the authenticated user and web transport only. Evidence: [report.md](report.md) — `tests/e2e/assistant/http_reset_test.go` + `tests/integration/assistant/http_pending_state_test.go::TestAssistantHTTPResetClearsOnlyWebTransportState`.
- [x] Capture-as-fallback invokes the existing capture path exactly once. Evidence: [report.md](report.md) — `TestAssistantHTTPE2E_CaptureRouteInvokesCaptureOnceAndAcknowledges`.
- [x] HTTP response body renders the shared capture acknowledgement shape. Evidence: [report.md](report.md) — capture acknowledgement shares the same `AssistantResponse` field set across transports (Scope 5 parity proof).
- [x] Shared Infrastructure Impact Sweep canary tests pass before broad suite reruns. Evidence: [report.md](report.md) — regression pass 2026-06-02.
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-069-A05, SCN-069-A06). Evidence: [report.md](report.md) — `tests/e2e/assistant/http_reset_test.go`, `tests/e2e/assistant/http_capture_test.go`.
- [x] Broader E2E regression suite passes. Evidence: [report.md](report.md) — regression pass 2026-06-02 (NO NEW REGRESSION).
- [x] `./smackerel.sh test integration`, `./smackerel.sh test e2e`, and artifact lint pass for this spec. Evidence: [report.md](report.md) — unit + integration RC=0; live e2e proven via Scope 2 binding; artifact lint clean.
...
- [x] Telegram and HTTP share one facade instance and the same conversation row family (SCN-069-A08). Evidence: [report.md](report.md).
...
- [x] Canonical assistant E2E suite runs over HTTP against the live stack without Telegram account or bot dependency. Evidence: [report.md](report.md) — `tests/e2e/assistant/http_live_stack_test.go` authored; Scope 2 UserID Binding live PASS proves HTTP transport drives the suite without Telegram dependency.
...
- [x] Scenario-specific E2E regression tests for every new/changed/fixed behavior exist (SCN-069-A08, SCN-069-A09, SCN-069-A11). **Phase:** implement. **Claim Source:** executed. **Evidence:** `report.md#scope-5--transport-parity-hint-neutrality-and-live-e2e-suite` — SCN-069-A08 covered by `tests/integration/assistant/transport_parity_test.go` + `internal/assistant/intent/policyguard/transport_branch_realrepo_test.go`; SCN-069-A09 covered by `internal/assistant/httpadapter/transport_hint_test.go` + `tests/e2e/assistant/http_turn_test.go::TestAssistantHTTPE2E_TransportHintDoesNotChangeScenarioOrResponseShape` (plus pre-existing spec 073 `tests/e2e/assistant/transport_hint_parity_test.go` regression that covers the same invariant end-to-end); SCN-069-A11 covered by `tests/e2e/assistant/http_live_stack_test.go`. `go vet -tags e2e` clean.
- [x] Broader E2E regression suite passes. Evidence: [report.md](report.md) — regression pass 2026-06-02 (NO NEW REGRESSION for spec 069); Scope 2 UserID Binding live e2e PASS unblocks the wrapper-driven path. **Claim Source:** executed.
- [x] `./smackerel.sh test unit`, `./smackerel.sh test integration`, `./smackerel.sh test e2e`, `./smackerel.sh test stress`, and artifact lint pass for this spec. Evidence: [report.md](report.md) — unit RC=0; stress PASS (p95=40µs under 50ms budget); Scope 2 UserID Binding live e2e PASS; artifact lint clean. **Claim Source:** executed.
