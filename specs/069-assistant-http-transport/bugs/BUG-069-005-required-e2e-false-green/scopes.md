...
|-------|------|------------|--------------|--------|
| 1 | Compiler SST, provider transport, and core wiring | none | SCN-BUG069005-001, SCN-BUG069005-002 | Not Started |
| 2 | Persistent disambiguation and confirmation state | Scope 1 | SCN-BUG069005-003, SCN-BUG069005-004, SCN-BUG069005-005 | Not Started |
...
- [ ] TP-BUG069005-01 proves an enabled HTTP assistant cannot start with the compiler disabled or unwired; no fallback raw-text path is selected.
- [ ] TP-BUG069005-02 proves the real ML compiler route validates the request and returns schema-bound deterministic provider output without executing tools.
- [ ] TP-BUG069005-03 proves live core constructs one compiler transport/client and attaches it to the shared facade before HTTP adapter binding.
- [ ] TP-BUG069005-04 proves annotation behavior is driven by compiled slots using adversarial input that omits legacy keywords.
- [ ] TP-BUG069005-05 proves Springfield ambiguity reaches the compiler and returns at least two canonical choices before any weather lookup.
- [ ] SCN-BUG069005-001 - Annotation slots come from the live compiled intent: a persistent ConfirmCard is built from compiled state-mutation slots and acceptance applies those values exactly once.
- [ ] SCN-BUG069005-002 - Springfield ambiguity creates a persistent choice: the live response carries at least two persistent choices and no weather lookup occurs before selection.
...
- [ ] TP-BUG069005-06 proves compiled interactive controls use the existing durable row family, are isolated by user/transport, and clear exactly once.
- [ ] TP-BUG069005-07 proves an issued disambiguation choice resolves the same pending state and uses the selected candidate; stale/cross-user refs do not resolve.
- [ ] TP-BUG069005-08 proves no list row changes before confirmation and the accepted persisted proposal changes it exactly once.
- [ ] TP-BUG069005-09 proves confirm accept executes the gated action once and replay executes zero additional actions.
- [ ] Valid confirm-required writes return `ConfirmCard` with `capture_route=false`; capture fallback remains reserved for its policy-owned failure/abandonment cases.
- [ ] Change Boundary is respected and zero excluded file families are changed.
