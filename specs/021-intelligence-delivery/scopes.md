...
- [x] Scenario SCN-021-004 (Bill alert created for upcoming subscription charge): `ProduceBillAlerts()` added — queries subscriptions ≤3 days to billing, deduplicates
  - Evidence: `internal/intelligence/alert_producers.go` `ProduceBillAlerts(ctx)` with `clampDay(time.Local)` and dedup `NOT EXISTS` clause; nil-pool and cancelled-context guard paths covered by the consolidated table-driven `TestAllProducers_NilPoolErrors` and `TestAllProducers_CancelledContext` (both `internal/intelligence/engine_test.go`, exercising all four producers including `ProduceBillAlerts`), and billing-date logic covered by `TestBillingDate_LocalMidnightNotUTCTruncate`. _(Citation reconciled 2026-06-16: the former per-producer `TestProduceBillAlerts_*` names — including `_NilPool` / `_CancelledContext` variants — were consolidated into the table-driven `TestAllProducers_*` tests in a later simplify round; producer behavior coverage is unchanged and green.)_
...
- [x] Scenario SCN-021-007 (Return window alert for expiring purchase): `ProduceReturnWindowAlerts()` added — queries artifacts with return_deadline ≤5 days, deduplicates, priority 1
  - Evidence: `internal/intelligence/alert_producers.go` `ProduceReturnWindowAlerts(ctx)` uses regex guard `metadata->>'return_deadline' ~ '^\d{4}-\d{2}-\d{2}$'` before `::date` cast; nil-pool and cancelled-context guard paths covered by the consolidated table-driven `TestAllProducers_NilPoolErrors` and `TestAllProducers_CancelledContext` (both `internal/intelligence/engine_test.go`, exercising all four producers including `ProduceReturnWindowAlerts`), and the regex-guard validation covered by `TestReturnWindowDateRegex_Validation`. _(Citation reconciled 2026-06-16: the former per-producer `TestProduceReturnWindowAlerts_*` names — including the `_CancelledContext` variant — were consolidated into the table-driven `TestAllProducers_*` tests in a later simplify round; producer behavior coverage is unchanged and green.)_
- [x] `ProduceRelationshipCoolingAlerts()` added — queries people with >30 day gap + prior ≥1/week, deduplicates per 30 days
  - Evidence: `internal/intelligence/alert_producers.go` `ProduceRelationshipCoolingAlerts(ctx)` with 30-day dedup window; nil-pool and cancelled-context guard paths covered by the consolidated table-driven `TestAllProducers_NilPoolErrors` and `TestAllProducers_CancelledContext` (both `internal/intelligence/engine_test.go`, exercising all four producers including `ProduceRelationshipCoolingAlerts`), and mutex isolation covered by `TestRelationshipCoolingUsesOwnMutex`. _(Citation reconciled 2026-06-16: the former per-producer `TestProduceRelationshipCoolingAlerts_*` names were consolidated into the table-driven `TestAllProducers_*` tests in a later simplify round; producer behavior coverage is unchanged and green.)_
