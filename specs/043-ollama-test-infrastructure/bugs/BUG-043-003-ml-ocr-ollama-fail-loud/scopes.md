...
- **Operator-side fan-out:** the home-lab deployment env file (`config/generated/home-lab.env`) MUST contain `ENABLE_OLLAMA=false` (the spec 043 default per `design.md` line 91 and line 166). The SST loader `scripts/commands/config.sh` already emits this value correctly per spec 043 BUG-001 + BUG-002 close-outs. No operator action required for the fix.
...
- **Rollback path:** see the corresponding DoD item — single `git revert`; no live-config mismatch possible because the home-lab `ENABLE_OLLAMA=false` value is already in the generated env file.
