...
### Definition of Done - Reconciled Against Current-Session Execution

- [x] SCN-004-004-C11: every scheduled/operator trigger has one run-linked, attempt-numbered `attempt_started` event and exactly one terminal event. History is immutable and content-free. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] SCN-004-004-C12: required attempt or terminal-event persistence errors propagate and prevent delivery, persisted, recovered, and healthy claims. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] SCN-004-004-C13: changed-source or policy reruns supersede before insert under the actor/cadence/window lock. Exactly one current output and both histories remain. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] SCN-004-004-C14: same-source races converge idempotently while actors, cadences, runs, and attempts cannot cross-pair. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] SCN-004-004-C15: read-back failure appends a durable failure state. Only a later coherent aggregate read appends `recovered`. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] SCN-004-004-C21: exact local source `7c3838e3b2de9ecba2e6a7764493a0412c4ed268` builds in a detached worktree, runs by final local digest without runtime fetch, proves authenticated prior-source reads and daily/weekly write-or-fail-closed behavior through 067, then yields to a current-candidate causal write with every owned resource removed. No deployment pointer or restore claim is inferred. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
...
- [x] T004-C11-IMMUTABLE passes and proves UPDATE/DELETE rejection plus event-content redaction. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C11-CAUSAL passes and proves start/terminal linkage to one run attempt. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C11-WEEKLY passes and proves both scheduler cadences persist and read back before delivery or success logging. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C11-COMPOSE-STDIN passes through the canonical unit lane and directly proves that piped stdin reaches `docker compose exec`, terminal-backed `exec` sees closed stdin, and non-`exec` Compose behavior and argv remain unchanged. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C12-AUDIT-RED fails against swallowed event-write errors and passes only when the typed error reaches the real trigger boundary. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C13-REPLACE passes for changed-source replacement under the window lock. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C13-ROLLBACK passes and proves a failed replacement restores the prior current output. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C14-RACE passes under simultaneous same-source and changed-source triggers. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C14-ACTOR-CADENCE passes with interleaved actors, cadences, run IDs, attempt numbers, and timestamps. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C14-CONFIG-UNIT passes with missing, empty, zero, negative, malformed, and unknown values refused without fallback. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C14-CONFIG-RUNTIME passes with distinct configured actor, retry, lease, and policy values observed in both cadence paths. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] T004-C15-READBACK passes with a forced production-reader failure followed by a newly verified recovery. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
...
- [x] T004-C21-HARNESS enforces the full pinned SHA, local-object-only/no-fetch rule, old-revision CLI build, final digest and resolved-input recording, runtime-network denial, lifecycle/required registration, unique resource references, and cleanup traps. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
...
- [x] T004-C03-BROAD passes with zero required skips and no internal request interception. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] Scope 7 completion evidence remains limited to local migration and source compatibility. Operator pre-deploy pointer rollback, write freeze, backup, and exceptional restore remain Scope 8/SCOPE-04A acceptance contracts and are not Scope 7 prerequisites. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] Change Boundary is respected and zero excluded file families change. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
- [x] Build Quality Gate passes: unit, integration, E2E, stress, security, check, lint, format, migration integrity, content-free observability, artifact lint, traceability, documentation alignment, zero warnings, and zero required skips. → Evidence: [child report](bugs/BUG-004-004-synthesis-persistence-and-health-truth/report.md#corrective-scope-03a-evidence)
