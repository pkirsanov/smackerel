# Ops Packet: [OPS-003] Self-Hosted Deploy Handoff — GAP-06 + BUG-067-001
...
> live self-hosted apply is deferred to the operator and intentionally NOT run
...
`self-hosted` target via the Build-Once Deploy-Many pipeline (Bubbles gate
**G081 / G074**).
...
> ⚠️ **The single most important pre-deploy fact:** the self-hosted config
...
| `ded6a2a8` | docs(ops): self-hosted deployment handoff for GAP-06 + BUG-067-001 (OPS-003) | No — this packet's doc only |
...
2. The `ML_LOG_LEVEL` requirement and its bundle-staleness caveat are called
   out prominently.
3. The packet is handed off; the live self-hosted activation is the operator's
...
- Running the push (`git push`), CI build, `promote.sh`, `apply`, or `verify`
  against the real self-hosted host. The host is currently saturated with
  concurrent heavy builds; activation waits for quiescence.
- Any edit to the operator-private self-hosted deploy adapter (lives in the `knb`
