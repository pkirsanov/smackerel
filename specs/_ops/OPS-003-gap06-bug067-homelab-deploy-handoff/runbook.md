# Runbook: [OPS-003] Home-Lab Deploy Handoff — GAP-06 + BUG-067-001

> **Owner:** `bubbles.devops` · **Target:** `home-lab` · **Deploy source SHA:** `78b293cc`
...
placeholders) are filled by the operator-private home-lab deploy adapter (knb
...
>
> - Use the `home-lab-<sourceSha>` bundle whose `<sourceSha>` matches the
>   `ml` image you are deploying (both come from the same CI build of
>   `78b293cc` or later).
> - Do **not** reuse an older `home-lab-<olderSha>` bundle with the new `ml`
...
# 2) Promote to home-lab (resolves core+ml digests + bundle ref from the
...
- `<sourceSha>` is the source SHA of the CI build (the `78b293cc` tip or a
  descendant). The `ml` image digest and the `home-lab-<sourceSha>` bundle MUST
...
- `--config-bundle-sha=<sha256-hex>` is **required** for direct `apply`; copy
  `configBundles[*].sha256` for the `home-lab` environment out of the build
...
Run `./smackerel.sh deploy-target home-lab verify` first (adapter-driven health
...
home-lab deploy handoff** and may be bundled with — or sequenced after — this
packet. This packet (GAP-06 + BUG-067-001) is independent of them and can ship
first; the operator decides ordering.

| Spec | Theme | Note |
|------|-------|------|
| `specs/084-open-knowledge-reasoning-loop/` | Open-knowledge reasoning loop | Awaiting home-lab activation |
| `specs/087-open-knowledge-genuine-synthesis/` | Open-knowledge genuine synthesis | Awaiting home-lab activation |
| `specs/088-runtime-switchable-models/` | Runtime-switchable models + GPU A/B | Awaiting home-lab activation |
...
> / runtime-switchable-models specs are activated on home-lab, confirm the
