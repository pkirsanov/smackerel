# Runbook: [OPS-004] Self-Hosted Activation Handoff (Consolidated)

> **Owner:** `bubbles.devops` · **Target:** `self-hosted` · **Deploy source SHA:** `e0e7bc4f`
> Read [`objective.md`](objective.md) first for what is shipping and why.
> **Supersedes [`OPS-003`](../OPS-003-gap06-bug067-selfhosted-deploy-handoff/) for the live deploy.**
...
operator-private self-hosted deploy adapter (the `knb` overlay, resolved via
`DEPLOY_TARGETS_ROOT` → `<deployment-owner>/<product>/<target>/`), never by
this repo. The deploy host is referred to ONLY as `self-hosted` / `<deploy-host>`
/ `<host-bind-address>`.
...
> using the `self-hosted-<sourceSha>` bundle whose `<sourceSha>` matches the `ml`
> image you deploy — both come from the **same** local-operator build of
> `e0e7bc4f`. Never pair the new `ml` image with an older `self-hosted-<olderSha>`
> bundle.

### 🔴 2. Synthesis model selection MUST be `qwen3:30b-a3b` (NOT the retired `deepseek-r1:7b`)

The self-hosted open-knowledge **synthesis** default is `qwen3:30b-a3b` — the
...
| **Envelope** | `20480` MiB; co-resident with `gemma4:26b` gather (`18432`) = `38912` MiB inside the `49152` MiB / 48G self-hosted envelope |
...
The adapter also supplies the fail-loud deploy-compose env the self-hosted stack
...
# scripts/commands/build-self-hosted.sh requires OPERATOR_COSIGN_KEY /
# OPERATOR_COSIGN_PUBKEY (default <operator-key-path>,pub})
...
./smackerel.sh build --target self-hosted
```
...
| Config bundle | `ghcr.io/pkirsanov/smackerel-config-bundles:self-hosted-<sourceSha>` (carries `ML_LOG_LEVEL` in `app.env`) |
...
adapter resolver finds `<deployment-owner>/<product>/<target>/params.yaml`.
...
# 1) Promote (PREFERRED). Resolves core+ml digests, the self-hosted bundle ref,
#    its sha256, and trustModel=local-operator from the local manifest, then
#    execs `smackerel.sh deploy-target self-hosted apply ...`.
...
- `<sourceSha>` is `e0e7bc4f` (the full 40-char SHA in the manifest filename).
  The `ml` image digest and the `self-hosted-<sourceSha>` bundle MUST come from
...
- self-hosted host ports are `41001` (`smackerel-core`) and `41002`
  ...
  self-hosted runbook. Infra (`postgres` / `nats`) publish **no** host port —
...
Run `./smackerel.sh deploy-target self-hosted verify` first, then confirm each item
...
- **088** runtime-switchable-models (per-request `--model=` / API `model`
  against the self-hosted synthesis-switch allowlist).
...
## Day-1 Ops (first live self-hosted operational cycle)

Today only DEV-stack SLO evidence exists; the live self-hosted needs its first
real cycle. Treat these as **activation gates**, not assumed-covered:

1. **First post-apply backup** of the live self-hosted stack (T1/T2). See
...
3. **Operate-plane SLO capture** on the live stack (latency / availability) so
   the self-hosted has real operate-plane evidence, not just DEV. See
...
**BUG-058 (chrome-extension bridge)** is NOT a self-hosted deploy and does NOT flow
...
- **Why separate:** the chrome-bridge zip is signed via **CI keyless cosign**
  ...
  different trust boundary from the local-operator self-hosted deploy. Sequence it
  independently of this self-hosted activation.
...
framework-cert matter — NOT a knb / self-hosted deploy** — and are listed only so
nothing falls through.

> **Reconciliation (2026-06-25):** 4 of the 5 have since reached
> `status: "done"` in committed state.json (verified per packet); only
> **BUG-077-002** (spec 077 / `BUG-002`) remains `in_progress`. The self-hosted
...
scope for this self-hosted handoff**.
...
| Artifact producer | local-operator: `./smackerel.sh build --target self-hosted` (GitHub CI is `disabled_manually`) |
