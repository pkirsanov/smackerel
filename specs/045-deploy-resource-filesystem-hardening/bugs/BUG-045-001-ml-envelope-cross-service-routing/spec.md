...
- **Severity:** P1 — HIGH. Three concrete observable impacts on `main` at HEAD `de49b2f9`:
  ...
  2. **Spec 052 live canary blocked** — documented at `specs/052-bundle-secret-injection-contract/state.json` (concerns `C-A12`, `C-B5`, `C-B6` — all severity:low, all followUpOwner:operator) with verbatim error message:
     ...
     Spec 052 closed `done_with_concerns` because the operator cannot perform the L2/L3 live-stack canary on the dev box (gemma4:26b needs 18.4 GiB; dev box has 11.7 GiB) and the deploy-adapter (knb / self-hosted) path is gated by the same envelope mismatch.
...
| Documentation of the per-service envelope contract | `docs/Operations.md` (no "Model Envelope Sizing" section) | New section explaining the dev/self-hosted/production model-selection trade-off |
...
| Severity | P1 — HIGH (blocks main-branch CI, blocks spec 052 self-hosted canary, will block any new operator running `./smackerel.sh up` on default config) |
...
9. **`docs/Operations.md` has a new "Model Envelope Sizing" section** documenting per-service envelope contracts and the dev / self-hosted / production model-selection trade-off.
...
8. `docs/Operations.md` has NO section documenting the per-service envelope contract or the dev/self-hosted/production model-selection trade-off.
...
- **AC-9 (`docs/Operations.md` "Model Envelope Sizing" section):** A new section in `docs/Operations.md` documents:
  ...
  - The dev / self-hosted / production model-selection trade-off (dev: 8G ollama envelope, small model; self-hosted: 16-32G ollama envelope, gemma4:26b possible; production: tuned per workload).
...
- **Increasing the default ollama envelope above 8G** — out of scope. The default `deploy_resources.ollama.memory = "8G"` is intentional for dev-box-fit; the fix is to change the default model to fit that envelope, not to grow the envelope. self-hosted and production operators may grow the envelope in their overlay.
...
- **Spec 052 self-hosted canary blocked** — concerns `C-A12`, `C-B5`, `C-B6` were filed at severity:low but the SUM of three independent blockers on the same root cause is a P1 condition: the spec 052 L2/L3 defense-in-depth live-stack canary cannot be exercised until this is fixed.
