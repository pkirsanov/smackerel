...
```
/ask tide schedule for 06/11 in ocean shores, wa
```

The bot replied with TWO messages:

1. `. Saved: "/ask tide schedule for 06/11 in ocean shores, wa" (idea)`
...
2. Send `/ask tide schedule for 06/11 in ocean shores, wa` to the bot.
...
| **B** | captured idea title = `"/ask tide schedule for 06/11 in ocean shores, wa"` (prefix included) | captured idea title = `"tide schedule for 06/11 in ocean shores, wa"` (slash-command prefix stripped) |
