...
> self-hosted bot — they are checked here against the in-repo code+test validation
...
- [ ] LIVE: on the self-hosted bot, `/ask tide schedule for 06/11 in wa-town-A, wa` returns a sourced answer instead of "I don't have a sourced answer for that." — **pending the bubbles.devops redeploy** of the fixed SHA + regenerated self-hosted config bundle.
...
- [ ] LIVE: on the self-hosted bot, a captured idea title never contains `/ask` — **pending the bubbles.devops redeploy**.
