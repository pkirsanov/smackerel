...
| 2026-08-29 | `scenario-manifest.json` `linkedTests` used a `path::TestName` string form that the traceability guard reads as a literal filename, so all 6 linked-test references failed to resolve even though both files existed and were executing | Audit phase of this packet; `traceability-guard.sh` reported `RESULT: FAILED (6 failures)` | FIXED IN THIS PACKET, not deferred. Converted all five scenarios to the structured `{file, testId, category}` form used by specs 066, 067 and 068, which is the only form that yields a clean path to the guard. The framework guard itself is framework-managed and was correctly left untouched. `SCN-064-003-02` additionally carried a bare LINE reference (`openknowledge_routing_test.go:125`) naming a location rather than a test; it now names the containing test. Verified: guard `RESULT: PASSED (0 warnings)`, exit 0, and `scenario-state-resolve.sh` still parses all five with `id` + `scenarioId` intact | This report, "Validate And Audit Phases" section; manifest at `specs/064-open-ended-knowledge-agent/bugs/BUG-064-003-router-warmup-exceeds-fixed-deadline/scenario-manifest.json`; reference convention at `specs/066-legacy-keyword-surface-retirement/scenario-manifest.json` |
