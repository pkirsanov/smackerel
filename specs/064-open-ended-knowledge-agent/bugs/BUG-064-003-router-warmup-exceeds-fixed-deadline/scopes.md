...
| T6 | Stress | `stress` | `tests/stress/bug064003_router_warmup_sla_stress_test.go` | This scope declares latency budgets (`warmup_target_latency_ms`, `warmup_budget_ms`, `build_per_call_budget_ms`), so it owes a stress run proving the warm-up budget is a real upper bound under concurrent load rather than only on an idle sidecar. 4 tests: SST budgets present + ordered (against the real lane env), budget bounds 25 sequential never-warm waits, budget holds across 32 concurrent waiters, and a warm embedder still qualifies on its first probe instead of burning the term | `./smackerel.sh test stress --go-run BUG064003` | Yes |
...
- [x] Stress coverage for the declared latency SLAs - T6 is authored, passing, and proven adversarial. It closes exactly the gap this item named. The earlier figures (`464ms` qualifying warm latency, `2s` per-call budget, `2m38s` derived build budget) were measured on an IDLE sidecar and said nothing about concurrent load; `tests/stress/bug064003_router_warmup_sla_stress_test.go` now asserts the budget is an upper bound under load. Measured: 25 sequential never-warm waits against a 300ms budget worst-cased at `301.794713ms`, and 32 CONCURRENT waiters worst-cased at `301.346458ms` - the deadline holds to ~2ms under contention. The positive counterpart matters as much: 32 concurrent warm embedders qualified in `1.430401ms`, so a "fix" that simply waited the full budget every time would fail. The SST half runs against the real lane env and confirmed `target=1s budget=1m0s per-call=2s embed-timeout=30s` with the documented ordering invariant intact. Adversarial proof, not assumed: stretching the deadline to `WarmupBudget * 4` drove the lane to exit 1 with `warm-up overran its declared budget - this is BUG-064-003 reappearing. budget=300ms observed=1.200s`, both bound tests failing while the SST-shape and warm-path tests correctly still passed; the mutation was reverted under a `trap` and verified with `restored=0`. Evidence: [report.md](report.md) `### T6 Stress Evidence`.
