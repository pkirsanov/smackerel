...
|---|-------|----------|-------------------|-----------|--------|
| 1 | Foundation: config SST + migration + PKCE helper + encrypted store | `internal/auth`, `internal/connector/twitter`, `internal/config`, `config/smackerel.yaml`, `scripts/commands/config.sh`, `cmd/core/connectors.go`, `internal/db/migrations` | unit ×5, migration smoke ×1 | Fail-loud SST, S256 PKCE, AES-256-GCM store, migration applies | Done (9/9 DoD green; migration live-apply PASS 2026-06-09 — `TestTwitterOAuthMigration_AppliesCleanly` GREEN vs live Postgres, report.md A-E8) |
...
**Status:** Done (Foundation implemented + unit-tested 2026-06-08; 9/9 DoD green — the final migration live DB-apply under `./smackerel.sh test integration` is now CLOSED GREEN: `bubbles.validate` ran `./smackerel.sh test integration --go-run 'TwitterOAuthMigration'` on 2026-06-09 against the full live stack (all 8 containers Healthy) and `TestTwitterOAuthMigration_AppliesCleanly` PASSED against live Postgres — `--- PASS: TestTwitterOAuthMigration_AppliesCleanly (0.15s)`, `ok .../tests/integration 0.385s`, `PASS: go-integration`, `INTEGRATION_WRAPPER_EXIT=0`; see report.md → A-E8 and the updated Scope A Migration-Apply Disposition. The prior sandbox memory-saturation that blocked the integration stack was resolved by freeing headroom, retiring the honest not-run UD. Scope is certified Done; the packet stays NON-terminal in this PHASE-1 planning-truth pass — `certifiedAt` is set by PHASE 2 after the commit.)
**Priority:** P1
**Depends On:** none
...
| — | migration `056_twitter_oauth_pkce.sql` applies cleanly + both tables + `expires_at` index present (+ idempotent re-apply) | integration | `tests/integration/twitter_oauth_migration_test.go::TestTwitterOAuthMigration_AppliesCleanly` (`//go:build integration`, needs `DATABASE_URL`) — **PASS 2026-06-09** via `./smackerel.sh test integration --go-run 'TwitterOAuthMigration'` against live Postgres (`--- PASS ... (0.15s)`, `ok .../tests/integration 0.385s`, `INTEGRATION_WRAPPER_EXIT=0`); report.md A-E8 (+ unit-level embed/parse proof A-E6) |
...
- [x] Migration `internal/db/migrations/056_twitter_oauth_pkce.sql` creates `twitter_oauth_states` (+`code_verifier`, 15-min TTL, expires_at index) and `twitter_oauth_tokens` (composite PK, encrypted token columns) and applies cleanly under `./smackerel.sh test integration` — **Claim Source: executed (in current session, 2026-06-09).** `bubbles.validate` ran `./smackerel.sh test integration --go-run 'TwitterOAuthMigration'` against the full live stack (all 8 containers Healthy: nats, postgres, ollama, searxng, stub-providers, jaeger, smackerel-core, smackerel-ml) and `TestTwitterOAuthMigration_AppliesCleanly` PASSED against live Postgres: `--- PASS: TestTwitterOAuthMigration_AppliesCleanly (0.15s)`, `ok github.com/smackerel/smackerel/tests/integration 0.385s`, `PASS: go-integration`, `INTEGRATION_WRAPPER_EXIT=0` (the disposable stack torn down cleanly on exit). The test reads the real `056_twitter_oauth_pkce.sql`, applies it from scratch, asserts BOTH `twitter_oauth_states` + `twitter_oauth_tokens` tables and the `idx_twitter_oauth_states_expires_at` index, then re-applies for idempotency. The prior honest not-run Uncertainty Declaration (sandbox memory-saturation/OOM blocked the integration stack) is now RETIRED — memory headroom was freed and the live apply is GREEN. Evidence: report.md A-E8 (+ unit-level embed/parse proof A-E6)
