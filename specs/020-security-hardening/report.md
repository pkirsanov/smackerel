...
| Secret handling — bundle | CLEAN | Production-class target (`self-hosted`) emits `__SECRET_PLACEHOLDER__<KEY>__` markers in `app.env`/`nats.conf`, so the deterministic `chmod 0644` bundle copy carries NO real secrets; adapter substitutes at apply time (spec 052). |
