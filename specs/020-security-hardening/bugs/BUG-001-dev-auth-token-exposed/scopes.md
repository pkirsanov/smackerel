...
|---|------|----------|-----------|--------|
| 1 | Reject default token + change YAML default | `internal/config/config.go`, `config/smackerel.yaml`, `internal/config/validate_test.go` | Unit: placeholder rejection, prefix rejection, case-insensitive | [ ] Not started |
