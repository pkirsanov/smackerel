...
2. **SCOPE-02 — SST plumbing (forecast_days + 3 unit keys), fail-loud, end-to-end.** `smackerel.yaml` → `config.sh` → `assistant.go` (loaders + closed-vocab/range validation) → `cmd/core` wiring threads the values into the provider constructor. After this scope the live home-lab path renders the rich forecast from real SST config.
