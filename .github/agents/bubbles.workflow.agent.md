...
- **⚠️ TOOL-AVAILABILITY ESCALATION (NON-NEGOTIABLE):** This agent's frontmatter MUST expose the `agent` tool alias. If the active runtime still lacks `runSubagent`, emit a `blocked` RESULT-ENVELOPE with the missing tool and intended child invocation. Do not emulate specialists inline, and do not ask the user to switch agents as a substitute for an attempted dispatch.
