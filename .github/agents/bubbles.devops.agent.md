...
- Reference state gate: **G079 (Build-Once Deploy-Many Integrity)** — see `agents/bubbles_shared/state-gates.md` Section "Build-Once Deploy-Many Integrity Gate (G079)".
...
4. **Determine if Build-Once Deploy-Many applies.** If the project ships images to multiple environments, every change MUST keep the G079 invariants intact (digest pinning, no CI/deploy fusion, cosign verification, immutable bundles, pointer-swap rollback).
