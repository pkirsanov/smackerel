...
- **The two chains above are the closure path for a `required` finding** (IMP-038 SCOPE-4 / GF-3) — one inside the work boundary that the Goal Contract cannot be satisfied while open. A `blocking-external` finding BLOCKS the parent pending an operator-approved expansion or external repair; an `independent` finding is discharged by filing its `G095` artifact plus routing, under a separate scoped packet, and does NOT run these chains inline. Classification is defined in [operating-baseline.md → Goal Impact](operating-baseline.md).
- Filing plus routing closes an `independent` finding; it never closes a `required` one. A `required` finding routed away is an unclosed finding wearing a disposition, and the runner MUST reject it.
