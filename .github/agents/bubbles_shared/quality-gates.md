...
## Gate Family Reference (G082–G137)

> **Range:** the canonical gate set runs G001–G137 (G096 is burned; G101 is the
...
- **G101** `release_delivery_reconciliation_gate` — every `delivery=required` feature in `docs/releases/<phase>/features.md` (machine-bound via `<!-- bubbles:feature id=… spec=… delivery=required -->` plus a `<!-- bubbles:reconciled-packet … -->` header) MUST map to a DELIVERY-CAPABLE TERMINAL + validate-certified spec; a promised-but-unspecced, non-terminal, blocked, or implement-self-certified required feature is a finding. Delivery-capable means terminal for the resolved mode AND owned by a mode that actually delivers — a planning/docs/review ceiling (`specs_hardened`, `docs_updated`, `validated`) and `delivered_prototype` are NOT delivery, because planning maturity is not product delivery (BUG-032 D4). WARN-grandfathered without the header; BLOCKING with the header or `--require-coverage` (the goal/sprint release-phase convergence path); a malformed reconciled packet fails loud; framework source EXEMPT. Compile-time twin: `scenario-compile-lint.sh` requires a scenario's `rootOutcome.targetReleasePacket` to cover every required feature with a delivery node (`release-delivery-reconciliation-guard.sh`).
...
- **G128** `session_cap_enforcement_gate` — BLOCKING; evaluates all seven `sessionBudget` dimensions for one explicit host-issued session ID. Turns, convergence, tool-result bytes, and usage-adapter tokens require exact matching identity; mismatched and legacy rows remain retained and excluded. `maxToolCalls` is `UNMEASURABLE:no-exact-producer`, not inferred from the legacy scalar or receipt count. Default-off needs no identity; an active budget without one exact ID is `INPUT-ERROR` exit 2. Equality, strict-greater-than breach, and the measured-only 70% soft boundary are preserved. Check 40 forwards `BUBBLES_SESSION_ID` and distinguishes breach exit 1 from input exit 2 (`session-cap-guard.sh`).
...
**Autonomy posture (G135)** — the posture surface is only as good as its next edit:
- **G135** `autonomy_posture_gate` — BLOCKING; keeps the IMP-039 autonomy posture surface internally consistent. That spec shipped a resolvable dial, a corrected enum, an opt-in `unattended`, a never-suppressible floor, a decision ledger and posture persistence — but AUT-2 was precisely a declaration describing behavior the implementation did not have, and nothing stopped that recurring. G135 checks the four places drift actually enters. (1) The never-suppressible floor still exists AND still enumerates its six items, so the section cannot be hollowed out while its heading survives — the subtler regression, because a present heading reads as an intact policy. (2) The `executionOptions.optionalWorkflowTags.autonomy` enum and the resolver's `VALID_AUTONOMY` agree exactly, in BOTH directions: a posture declared but not implemented is the AUT-2 shape, and a posture implemented but not declared is undocumented behavior. (3) `unattended` with an unbounded session budget is still refused with exit 3 AND still names `E039-UNATTENDED-UNBOUNDED`, because an unnamed refusal is not actionable; and `unattended` with a bounded budget still resolves, so the rule cannot be satisfied by refusing everything. (4) The resolver still refuses every bypass flag, so the posture can never begin waiving verification. Exits 0 (consistent), 1 (findings, one per line), or 2 (usage). There is NO `--force`/`--skip`/`--ignore`: a posture that cannot satisfy the floor is CHANGED, never exempted. Hermetic selftest `autonomy-posture-guard-selftest.sh` runs 15 cases, each pairing the green fixture with a drifted variant. Enforced by `autonomy-posture-guard.sh`.

**User acceptance (G136)** — a terminal status is a claim that no user rejected the delivered behavior:
- **G136** `human_acceptance_terminal_gate` — BLOCKING; refuses a target-`done` transition while any item in `## Checklist` is unchecked. Acceptance is opt-out: the checklist ships checked, and unchecking records a user rejection. The gate proves only that no user recorded an objection. It does not prove that a human reviewed the file. The shared reader also validates `## Automation Readiness` and any authored `## Human Acceptance Record`. Its six default-active shape and checklist codes remain declared. `PD12-NO-RECORD` is conditional-active and appears only when a supported registry sets `requiredAtTerminal: true`. The shipped `false` value leaves it dormant. `PD12-AUTHORITY-UNAVAILABLE` is bootstrap-active. Both public verdicts emit it and fail closed when the authority is missing, unreadable, malformed, or incomplete. A ceiling-bound mode is not claiming acceptance, and an open checklist remains correct there. A `[ ]` under `## Notes` is ignored. The guard never edits the file. After a fix, only the user re-checks a rejected item. No `--force`/`--skip`/`--ignore`. Selftests live in `acceptance-authority-selftest.sh`, `state-transition-guard-selftest.sh`, and `tests/regression/test_35_human_acceptance_terminal.sh`. Enforced by `guards/tail-delegated-gates.sh` Check 43.

**Release ladder schema (G137)** — a delivery assertion is only meaningful over a well-formed ladder:
- **G137** `release_ladder_schema_gate` — BLOCKING for a repo that DECLARES a release ladder; AUTO-EXEMPT otherwise. Sibling of G101 and deliberately NOT folded into it. G101 asks "is every promised required feature delivered?"; G137 asks the prior question "is the ladder itself well-formed, ordered, and stably identified?" A repo can be structurally clean and honestly red (ordered ladder, unfinished work) or delivered and unprovable (finished specs, headers that disagree about what phase they are); collapsing the two gates would let either failure mask the other. DIVISION OF DUTY: the PRODUCT declares its ladder via `<!-- bubbles:release-ladder schemaVersion=1 phases=a,b,c -->` in `docs/releases/README.md`, its packet headers, its `dependsOn`, and its per-feature bindings; the FRAMEWORK enforces only that those declarations are internally consistent. The framework NEVER invents a ladder — a repo with no declaration resolves EXEMPT rather than failing, because most repos have no release ladder and inventing one for them would be a false finding (the Bubbles source checkout itself is EXEMPT). CHECKS: exactly one packet header per declared phase; `schemaVersion` refused rather than best-effort parsed; the header's declared phase equals its directory; `dependsOn` equals the FULL transitive predecessor set in ladder order so a phase can never silently skip a prerequisite; `id`/`spec`/`delivery` mandatory; `delivery` a closed vocabulary; a `deferred-to` target resolving to a known LATER phase, never itself and never backward (a backward defer is an unreachable promise); `assurance` only `implemented|planned` and only on a `required` binding; `delivery=carried` declaring `carried-regression`; and one capability id binding one spec across every phase, since id/spec drift is exactly what makes "the same capability" unprovable from phase to phase. FAIL-LOUD on malformed: a declaration with no `phases=` list, a duplicated phase, or an unsupported `schemaVersion` exits 1 rather than degrading to a silent no-op. No `--skip`/`--force`/`--ignore`; the only knob is `--ladder`, which cannot relax any check because a bogus ladder simply fails on the missing phase directories. Selftest: 17 cases — 1 CONTROL plus 13 adversarial, because a schema guard that only ever sees well-formed input proves nothing. Enforced by `bubbles/scripts/release-ladder-schema-guard.sh`.

**Release packet completeness (G138)** — blocking for a repo that ships `docs/releases/`; auto-exempt otherwise:
...
**Action risk classification integrity (G139)** — blocking; requires every classified cli action to carry a risk class the gate can actually interpret:
...
**Phase name enum integrity (G140)** — blocking; constrains every phase name a packet or an agent definition may write to the phases `bubbles/workflows:
