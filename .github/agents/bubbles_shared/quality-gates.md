...
- **build-once-deploy-many violation (Gate G074, advisory in framework / blocking in opted-in product repos):** deployment manifests pinned by mutable tag instead of `sha256:<digest>`, CI workflows that fuse build with deploy, adapter `apply.sh` that builds locally or skips signature verification (see state-gates.md and bubbles-deployment-target-adapter skill)
