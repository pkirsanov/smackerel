...
**Human acceptance (G136)** — a terminal status is a claim that a human accepted the behavior:
- **G136** `human_acceptance_terminal_gate` — BLOCKING; refuses a terminal transition while any acceptance item in `uservalidation.md` is still unchecked. The hole was narrow and load-bearing: `artifact-lint.sh` requires the checklist to carry at least ONE checked `[x]` and never rejects an unchecked one, so one checked plus five unchecked passes lint and the spec reaches a terminal status with five behaviors no human ever accepted. Lint is the WRONG place to repair it, and that choice is the design: lint also runs during PLANNING, where a checked-by-default template is legitimate — the template records what WILL be accepted, it does not claim a human already ran it — so tightening lint would either break planning or force it to fabricate acceptance up front. The terminal transition is the moment the claim stops being provisional. Runs ONLY when the resolved target status is `done`; a ceiling-bound mode (`validate-only`, `docs-only`, `spec-scope-hardening`) is not claiming human acceptance and an open checklist is correct for it. Parses only the `## Checklist` section using the SAME awk reader `artifact-lint.sh` uses, so the two cannot silently desync, and a `[ ]` under `## Notes` is ignored on purpose. THE GUARD NEVER EDITS THE FILE — it prints each unaccepted item and stops, because checking a box on the author's behalf would fabricate exactly the human acceptance the gate exists to require, the one change that would make the gate worse than not having it. No `--force`/`--skip`/`--ignore`. Selftest: two cases in `state-transition-guard-selftest.sh` — the BUG-029 shape itself and an adversarial partner proving the section parser does not over-reach — plus persistent regression `tests/regression/test_35_human_acceptance_terminal.sh`. Enforced by `guards/tail-delegated-gates.sh` (Check 43).
