...
- A broad mechanical contract change (renaming / moving / replacing a widely-consumed route, symbol, contract, or schema) whose callers CANNOT each land as an ordinary independently-green vertical slice uses the **wide-refactor exception**: sequence it `expand → migrate → contract` per the *Wide-Refactor Sequencing* contract in [scope-workflow.md](scope-workflow.md). Every `migrate` scope `dependsOn` the `expand` scope, and the `contract` scope `dependsOn` **all** `migrate` scopes AND a G043 consumer-trace proof that zero callers of the old form remain. This RETAINS — never relaxes — G043 (consumer trace), G044 (regression), G067 (shared-infrastructure blast radius), and G069 (collateral containment), and it uses NO integration-branch escape hatch (trunk / release-train policy stays authoritative). Mechanically enforced by `bubbles/scripts/expand-migrate-contract-guard.sh`.
