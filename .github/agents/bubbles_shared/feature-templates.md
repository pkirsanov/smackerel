...
**`executionHistory[]` entry:** `agent` MUST be a registered id from `bubbles/agent-capabilities.yaml`, or one of the non-agent actors `manual`, `operator`, `human`. It is an enum, not free text. Qualifiers go in named sibling fields: `expansionReason` for a parent-expansion cause, `sweepRound` for a sweep counter, `scope` for the scope worked, `mode` for the workflow mode. Recording several agents in one entry is forbidden; write one entry per agent. Enforced by `bubbles/scripts/agent-id-enum-lint.sh`, which ratchets against a PER-REPO baseline at `<repo>/.specify/agent-id-enum-lint.baseline` so historical records stay readable while new ones cannot regress. The baseline is per-repo because downstream the lint installs under `.github/bubbles/scripts/`, which `downstream-framework-write-guard.sh` forbids the consuming repo from editing. This field is the control plane's primary key: when it carried free text it reached 163 distinct values across six repos with 60 appearing exactly once, and no query could group the runs that exposed a framework-wide dispatch failure.
**`gateVintageOverrides[]`:** Optional. Each entry is `{ "gate": "Gxxx", "owner": "<name>", "reason": "<why>" }` and deliberately re-applies a gate that is NEWER than the spec. Gates carry `since` / `sinceDate` in `bubbles/registry/gates.yaml`, derived from git by `bubbles/scripts/gate-vintage-annotate.sh`, and `bubbles/scripts/gate-vintage-guard.sh` reports which failing gates did not exist when the spec was opened. Grandfathering is NOT automatic and no guard silently excuses a gate: auto-amnesty would be a mechanism for hiding real defects. An override is the exception that re-applies one gate with a named owner. Overriding every gate is a catalogue sweep by another name, which is the pattern that put reopen language into about 176 specs.
**No-op state writes are forbidden.** If a run ends with `statusBefore` equal to `statusAfter` and no scope changed state, it MUST NOT commit a state mutation. Across six repos this file was touched in 306 to 563 separate commits per repo per 60 days, and 71-95% of recorded transitions moved nothing. A run that inspected and found nothing to change is a legitimate outcome; recording it as a write is what made the control plane look busy while the product did not move. Append an `executionHistory` entry only when the run changed something a later reader needs.
