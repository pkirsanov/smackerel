...
8. **Impact/trace config honored when present** — if `.github/bubbles-project.yaml` or `bubbles-project.yaml` defines `testImpact`, validation must account for the generated impact plan and honor full-suite triggers. If it defines `traceContracts`, validation must run `trace-contract-guard.sh` against actual trace/log evidence for relevant workflows or record a concrete missing-evidence gap. **MUST-when-wired:** when `traceContracts.observability.posture: wired` and an instrumented scope (a Test Plan row declaring `observabilityWorkflow`) targets a workflow with an `slo:` link, validation must also run `observability-slo-guard.sh` (G100) against the captured `.specify/runtime/observability/<workflow>.slo.json` evidence; emitting + capturing that trace/SLO evidence is required, not optional. Both guards stay clean no-ops when posture is opted-out / undeclared or the scope is not instrumented.
