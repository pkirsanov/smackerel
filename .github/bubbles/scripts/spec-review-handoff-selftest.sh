...
assert_contains "spec-review names preferred single-mode runner" "$SPEC_REVIEW_AGENT" 'preferredAgent: bubbles\.workflow'
...
assert_block_contains "spec-review Phase 5 says routing is mandatory" "$SPEC_REVIEW_AGENT" 'Phase 5' 'Routing is MANDATORY|MUST route'
assert_contains "spec-review forbids nested runner invocation" "$SPEC_REVIEW_AGENT" 'MUST NOT invoke one as a subagent'
...
assert_contains "workflow auto-escalation directly executes improve-existing" "$WORKFLOWS_FILE" 'active authorized runner executes improve-existing directly'
...
assert_contains "orchestration core requires direct improve-existing execution" "$ORCHESTRATION_CORE" 'active authorized runner MUST execute `mode=improve-existing`|improve-existing.*active authorized runner'
