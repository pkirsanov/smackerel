...
make_stub generate-gate-enforcement.sh gate-enforcement.stale
...
expected="$(printf '%s\n' generate-gate-enforcement.sh generate-framework-stats.sh generate-cheatsheet.sh generate-capability-ledger-docs.sh generate-gate-coverage-map.sh generate-release-manifest.sh)"
if [[ "$(cat "$order_log")" == "$expected" ]]; then
  pass "generators invoked in dependency order (gate enforcement -> stats -> cheatsheet -> ledger -> gate-map -> manifest LAST)"
else
