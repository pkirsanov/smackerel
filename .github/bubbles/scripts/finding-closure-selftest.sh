...
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-fix-cycle-protocol.md" 'MUST execute the mapped child workflow mode instead of pre-running the trigger or hand-building a bespoke fix cycle' "Shared trigger protocol forbids bespoke parent-side fix cycles"
...
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-fix-cycle-protocol.md" 'Parent workflows MUST wait for the mapped finding-owned workflow mode to reach a terminal `## RESULT-ENVELOPE`' "Shared trigger protocol requires terminal mapped-mode result before parent returns"
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-fix-cycle-protocol.md" 'Mapped child workflow modes own docs/finalize/certification for touched specs' "Shared trigger protocol assigns closeout ownership to mapped modes"
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-fix-cycle-protocol.md" 'parent-expanded-child-mode' "Shared trigger protocol records parent-expanded execution model"
...
check_pattern "$AGENTS_DIR/bubbles.workflow.agent.md" 'Batching round selections without executing child workflow modes is FORBIDDEN' "Workflow agent forbids batch-then-summarize round execution"
check_pattern "$AGENTS_DIR/bubbles.workflow.agent.md" 'No report-only completion.*Producing a table of findings without executing mapped child workflow modes' "Workflow agent forbids report-only sweep completion"
...
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-execution-loops.md" 'No report-only completion.*Producing a table of findings without executing mapped child workflow modes' "Execution loops forbid report-only sweep output"
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-execution-loops.md" 'Execute the mapped child workflow mode' "Execution loops require mapped mode execution"
check_pattern "$AGENTS_DIR/bubbles_shared/workflow-execution-loops.md" 'parent-expanded-child-mode' "Execution loops define parent-expanded child mode fallback"
