...
command_effective_risk_class() {
  ...
  case "$command_name" in
    ...
    hooks|framework-proposal|autofix|upgrade)
      ...
      ;;
    audit-done|audit)
      if [[ "$command_args" == *'--fix'* || "$command_args" == *'--reopen-failing'* ]]; then
        printf '%s' 'owned_mutation'
      else
        printf '%s' "$default_risk"
      fi
...
cmd_doctor() {
  ...
  # Every non-blocking warning below increments advisory_count, so the summary
  # line can never report fewer advisories than the operator can see.
  local passed=0 failed=0 healed=0 advisory_count=0
...
try:
  ...
  local doctor_git_dir=''
  doctor_git_dir="$(git -C "$REPO_ROOT" rev-parse --absolute-git-dir 2>/dev/null || true)"
  if [[ -z "$doctor_git_dir" ]]; then
    echo -e "  ${CYAN}ℹ️${NC}  Not a git repository — hook health not applicable"
  else
    local doctor_hooks_dir="$doctor_git_dir/hooks"
    local hook_name='' hook_path=''
    for hook_name in pre-commit pre-push; do
      hook_path="$doctor_hooks_dir/$hook_name"
      if [[ ! -f "$hook_path" ]]; then
        if is_framework_repo; then
          echo -e "  ${YELLOW}⚠️${NC}  $hook_name hook not installed — run: bash bubbles/scripts/cli.sh hooks install --all"
          advisory_count=$((advisory_count + 1))
        else
          echo -e "  ${CYAN}ℹ️${NC}  $hook_name hook not installed (expected — Bubbles-managed hooks are framework-source-only)"
        fi
      elif ! grep -q 'Generated by.*bubbles' "$hook_path" 2>/dev/null; then
        echo -e "  ${CYAN}ℹ️${NC}  $hook_name hook installed but not Bubbles-managed (left untouched)"
      elif [[ ! -x "$hook_path" ]]; then
        echo -e "  ${YELLOW}⚠️${NC}  $hook_name hook is Bubbles-managed but not executable — run: chmod +x \"$hook_path\""
        advisory_count=$((advisory_count + 1))
      else
        echo -e "  ${GREEN}✅${NC} $hook_name hook installed (Bubbles-managed)"
      fi
    done

    # `hooks install` regenerates pre-push from scratch and can wipe the guard
    # appended by install-bubbles-hooks.sh, silently dropping framework-validate.
    if is_framework_repo && [[ -f "$doctor_hooks_dir/pre-push" ]]; then
      if grep -Fq 'bubbles framework-validate guard' "$doctor_hooks_dir/pre-push" 2>/dev/null; then
        echo -e "  ${GREEN}✅${NC} pre-push carries the framework-validate + release-check guard"
      else
        echo -e "  ${YELLOW}⚠️${NC}  pre-push is missing the framework-validate guard — run: bash bubbles/scripts/install-bubbles-hooks.sh"
        advisory_count=$((advisory_count + 1))
      fi
    fi
  fi
  ...
  # Check 15: Project scan config presence.
  #
  # Generating the config is a WRITE. Plain `doctor` is declared read_only in
  # bubbles/action-risk-registry.yaml (only --heal is owned_mutation), so the
  # generator runs under --heal only; otherwise this reports and instructs.
  if is_framework_repo; then
    ...
    if [[ ! -f "$setup_script" ]]; then
      echo -e "  ${YELLOW}⚠️${NC}  Project scan config missing and project-scan-setup.sh is unavailable (generic defaults will be used)"
      advisory_count=$((advisory_count + 1))
    elif [[ "$heal" == "true" ]]; then
      ...
      echo -e "  ${YELLOW}⚠️${NC}  Project scan config missing (no 'scans:' block in .github/bubbles-project.yaml) — run 'doctor --heal' to generate it; generic defaults are used until then"
      advisory_count=$((advisory_count + 1))
    fi
  else
...
" 2>/dev/null || echo "unavailable 0")"
    ...
    case "$corpus_state" in
      pass)
        ...
        ;;
      FAIL)
        # A red result must move the exit code; otherwise a golden-task
        # regression ships behind a green doctor.
...
# Generated by: bubbles.sh hooks install
set -uo pipefail
push_records="$(cat || true)"
echo "🫧 Bubbles pre-push: validating changed specs and changed done specs..."
failed=0
bash bubbles/scripts/agnosticity-lint.sh || failed=1

changed_spec_dirs=()

append_changed_spec_dir() {
  ...
  candidate="specs/$spec_name"
  [[ -f "$candidate/state.json" ]] || return 0

  for existing in "${changed_spec_dirs[@]}"; do
    [[ "$existing" == "$candidate" ]] && return 0
  done

  changed_spec_dirs+=("$candidate")
}
...
collect_changed_paths() {
  ...
  while IFS= read -r changed_path; do
    ...
    append_changed_spec_dir "$changed_path"
  done < <("$@")
}
...
if [[ -n "$push_records" ]]; then
  while read -r local_ref local_sha remote_ref remote_sha; do
    [[ -n "${local_ref:-}" && -n "${local_sha:-}" ]] || continue
    if zero_sha "$local_sha"; then
      continue
    fi
    if [[ -n "${remote_sha:-}" ]] && ! zero_sha "$remote_sha"; then
      collect_changed_paths git diff --name-only "$remote_sha..$local_sha" -- 'specs/**'
    else
      collect_changed_paths git diff-tree --no-commit-id --name-only -r "$local_sha" -- 'specs/**'
    fi
  done <<< "$push_records"
fi

collect_changed_paths git diff --name-only --cached -- 'specs/**'
collect_changed_paths git diff --name-only -- 'specs/**'

if [[ "${#changed_spec_dirs[@]}" -gt 0 ]]; then
  if ! bash bubbles/scripts/done-spec-audit.sh --profile changed "${changed_spec_dirs[@]}"; then
    failed=1
  fi
else
  echo "🫧 Bubbles pre-push: no changed spec directories detected."
fi

[[ $failed -ne 0 ]] && { echo "❌ Pre-push blocked."; exit 1; }
echo "✅ Changed spec validation complete."
PPHOOK
