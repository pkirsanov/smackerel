...
Auto-generated by Bubbles bootstrap. Customize for your project.
...
|--------|---------|---------|
| Build all | `./smackerel.sh build` | 30 min |
| Test all | `./smackerel.sh test` | 30 min |
| Lint | `./smackerel.sh lint` | 10 min |
| Format | `./smackerel.sh format` | 5 min |
| Start dev | `./smackerel.sh dev` | 5 min |
| Stop all | `./smackerel.sh stop` | 2 min |
| Status | `./smackerel.sh status` | 30 sec |
...
|-----------|----------|---------|-----------|
| Unit | `unit` | `./smackerel.sh test unit` | Always |
| Integration | `integration` | `./smackerel.sh test integration` | Always |
| E2E API | `e2e-api` | `./smackerel.sh test e2e` | When API changes |
| E2E UI | `e2e-ui` | `./smackerel.sh test e2e` | When UI changes |
| Stress | `stress` | `./smackerel.sh test stress` | When perf-sensitive |

> **TODO:** Update test commands and add project-specific test types.
...
- Project-specific instructions should define the canonical scan command for interception patterns and require gap-filling if a mocked reclassification removes the last real live-stack test.
...
- Always use `./smackerel.sh` — no direct tool invocation
...
```
Source code:     [TODO: update paths]
Tests:           [TODO: update paths]
Specs:           specs/
Config:          [TODO: update paths]
```
...
> **TODO:** Fill in if your project serves UI from Docker containers.
...
| Frontend image | `[TODO]` |
| Static root | `[TODO]` |
| Stop command | `./smackerel.sh stop` |
| Build command | `./smackerel.sh build` |
| Start command | `./smackerel.sh start` |
| Bundler | `[TODO: Vite/webpack/etc]` |
