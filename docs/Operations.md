...
| `SmackerelSynthesisFailing` | critical | `smackerel_synthesis_state == 4` | The latest synthesis attempt failed to persist. An older verified output may still exist; it does NOT clear this, because a current failure is a current fact. Read `failure_class` from `synthesis_run_attempts` — the class is recorded, the candidate content deliberately is not. |
| `SmackerelSynthesisStale` | warning | `smackerel_synthesis_state == 2` | The newest read-back-verified output has aged past its freshness budget. Either the scheduler is not running, or recent windows were claimed and abandoned. Check `smackerel_synthesis_last_verified_output_unixtime` and `lease_expires_at` on `synthesis_runs`. |
| `SmackerelSynthesisNeverRun` | warning | `smackerel_synthesis_state == 0` | No attempt or persisted output exists. Expected on a new install for the first day; after that the daily job is not reaching the persistence layer. Never-run is its own state — not a failure, and not healthy. |
| `SmackerelSynthesisStateUnreadable` | critical | `smackerel_synthesis_state == 5` | The durable synthesis state could not be read. Distinct from a failed run: the run may have been fine and the database unreachable. Health cannot claim up while this holds. |
