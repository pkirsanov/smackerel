...
|----|---------|-------|------------------------------------|
| RTE-M1 | [`specs/039-recommendations-engine`](../../../specs/039-recommendations-engine/) is `done` and genuinely validate-certified, but its `state.json` records `certification.certifiedCompletedPhases` as a **mixed array** of strings and objects. Gate G101 reads that array with `.[]? \| ascii_downcase`, which aborts at the first object; in 039's case every string element before that point excludes `validate`, so the guard cannot see the certification. The binding is therefore `carried`, not `required` — the capability is delivered, the record shape is the defect. Verified at HEAD `8d971420`: first object element is at index 12; `validate` appears only after it. | `bubbles.plan` (owns every spec's `state.json`) | `bubbles.releases` does not edit any spec's `state.json`. Flipping the binding to `required` without fixing the record would turn this gate red for a data-shape reason and teach readers to distrust it. |
| RTE-M2 | [`specs/038-cloud-drives-integration`](../../../specs/038-cloud-drives-integration/) and [`specs/040-cloud-photo-libraries`](../../../specs/040-cloud-photo-libraries/) carry the **same** mixed-array shape as 039. They currently pass only because `validate` happens to appear at index 3, before the first object at index 12. Their `required` bindings are correct today but rest on element ordering, not on a guaranteed shape. Fixing all three together is the durable repair. | `bubbles.plan` | Same ownership boundary as RTE-M1. |
...
| RTE-M5 | The [`_ops/OPS-001`](../../../specs/_ops/OPS-001-spec-banner-sweep/) banner sweep was a point-in-time remediation and drift has **re-accumulated**. Measured at `HEAD` `2a54d2fe` over every `specs/*/state.json` with `status == "done"` that has a sibling `spec.md` (blockquote-aware banner regex `^\s*(?:>\s*)?\*\*Status:\*\*\s*(.+)$`): **99** such specs, **84** carrying a canonical Done banner, **2** carrying no banner at all (`070-web-username-password-login`, `090-observability-slo-dogfood`), **13** carrying a stale banner (`057`, `059`, `060`, `078`, `083`, `085`, `091`, `093`, `094`, `097`, `099`, `100`, `103`) — **15 drifted**. Note for whoever re-measures: a naive regex that does not allow the `> ` blockquote prefix inflates this to 41 false positives. | `bubbles.plan` (owns every spec's `state.json`), with execution by an implementation agent | The sweep edits `spec.md` files, which `bubbles.releases` does not own. Re-running it would edit 15 already-certified `spec.md` files and so collide with Gate G088 (`post_certification_spec_edit_gate`) — the exact hazard [`_ops/OPS-002`](../../../specs/_ops/OPS-002-g088-certifiedat-backfill/) exists to manage, and which OPS-002 deliberately refused to resolve by bulk recertification. A blind re-sweep would trade a cosmetic drift for a portfolio-wide certification-integrity problem. |
