...
| R6 | **Gate G101 not wired to CI for this phase.** The `next` packet is new; until its guard invocation joins the pre-push/CI position that `mvp` already occupies, enforcement depends on someone remembering to run it. | Medium | OPS-N3 in [`actions.md`](actions.md). |
