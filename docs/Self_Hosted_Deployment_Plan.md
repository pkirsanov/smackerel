# Self-Hosted Deployment Plan — Migrated

> **Status:** Migrated. This document no longer hosts target-specific self-hosted
> readiness content.
>
> **Last reviewed:** 2026-05-13 (BUG-001 — `specs/032-documentation-freshness/bugs/BUG-001-self-hosted-readiness-plan-stale/`).

## What changed

The previous version of this document blended generic Smackerel product
deployment guidance with target-specific self-hosted operator instructions
...
|-------------|-----------|
| self-hosted adapter readiness checklist (apply / verify / rollback / bootstrap / host singleton / operator-host details / Caddy / tailnet exposure / host monitoring / backup timer paths) | Operator-private deploy-adapter overlay readiness artifact |
...
| Obsolete OPS-SELFHOSTED-1xx planning rows that were never created as artifacts | Removed. No replacement; the work either lives under a real Smackerel spec already, or is target-adapter work in the deploy-adapter overlay. |

## Why this repo no longer hosts a self-hosted plan
...
- Operator deploying Smackerel to self-hosted: read the operator-private
