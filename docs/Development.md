...
- Database migrations through `063_auth_token_granted_scopes.sql`; migrations 002-017 remain consolidated into `001_initial_schema.sql`
...
| `internal/graphreadstate/` | BUG-080-001 SCOPE-04 single typed activation/read-state projection — `Project` is the ONLY decoder that reduces an explicit `graphapi.Activation` plus one raw per-family `FamilyObservation` and an opt-in `Policy` (`AllowEmpty`, `Optional`) to exactly one closed `graphsynthetic.AggregateResult`, so Wiki Browse, Graph availability, and readiness structurally cannot disagree about what a read meant. It declares NO read-state vocabulary of its own (its only constant is the value-safe refusal code `F080-PROJECTION-INVALID`) and delegates every reduction through the thin exported `internal/graphsynthetic` seam (`NewFamilyRow`, `ClassifyHTTPOutcome`, `Aggregate`) that wraps the SAME reducer the SCOPE-03 synthetic runs, so no parallel vocabulary can drift out of step. The gate order is the contract — transport, then HTTP status, then row count — so a non-200 outcome such as a route-missing 404 never reaches the emptiness branch and cannot be projected as a permitted true-empty, and an explicit `ActivationDisabled` short-circuits before any observation is consulted so a disabled capability is never advertised as available or ready. The canonical family list is derived from `graphapi.RequiredGraphFamilies()` on every path (never hardcoded), and an unobserved, duplicated, non-canonical, or self-contradictory family returns a typed value-safe error and NO result rather than being coerced into an empty read; projected rows carry no HTTP status and no row count, leaving consumers nothing to re-infer state from. Guarantees proven by `TestGraphActivationProjectionUsesClosedExclusiveStates` in `web/pwa/tests/graph_activation_state_test.go`. |
