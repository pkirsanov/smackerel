...
- Database migrations through `067_synthesis_causal_event_truth.sql`; migrations 002-017 remain consolidated into `001_initial_schema.sql`
...
| `internal/proactive/` | Spec 107 SCOPE-01 single-controller proactive card-projection foundation — `ProactiveCardModel` (projects a card only for a `permit`/`escalated` verdict from the single spec-078 surfacing controller), the ephemeral process-local `NudgeRef` registry (sole anti-leak boundary: an opaque ULID-shaped wire ref resolves back to a `content_key` that never leaves the process), the single `NudgeAck` path (act/snooze/dismiss resolve their ref and call one `Acknowledge(content_key)` on the shared suppression window), the closed `HonestState` vocabulary, the `BudgetMeter` read, and the additive `a:n:<ref>:<a\|s\|d>` nudge callback wire form shared by the Telegram callback_data and the WhatsApp reply-id. Adds no second budget/store/cache and edits no spec-078 owner type. |
...
| `internal/observability/` | Spec 101 shared-observability instrumentation (SCOPE-01) — fail-loud reader/validator of the knb spec-014 scope-03 canonical 3-var OTLP instrumentation contract (`OTLP_TRACES_ENDPOINT`, `OTLP_LOGS_ENDPOINT`, `METRICS_SCRAPE_LABEL_PRODUCT`) the knb adapter injects into the generated `app.env` when the shared-observability posture is active. Config-contract boot gate ONLY — deliberately does not fork a second span exporter (spans keep flowing through `internal/assistant/tracing`, `/metrics` through `internal/metrics`); every value is REQUIRED with NO default and NO fallback (smackerel NO-DEFAULTS SST — aborts startup on any empty value). |
| `internal/acceptance/` | BUG-102-001 journey-contract acceptance layer — the closed `E102-JOURNEY-*` failure-code registry, the value-safe evidence sanitizer (`evidence.go`, which rejects rather than redacts an unsafe observation), and the production-inert, test-only fault-profile registry for disposable `env=test*` validate/e2e stacks (`Resolve` returns `ErrFaultInertInProduction` under a production posture, and `AssertNoFaultControlInProductionSurface` refuses any production route, config, request, or UI descriptor carrying a fault selector, trigger, or profile control). |
| `internal/experience/` | Spec 106 renderer-neutral experience-contract layer — the cross-surface consumer catalog and inventory (`catalog.go`, `catalog.gen.json`, `consumer_inventory.go`), the navigation / renderer / state / mutation presentation projections, and the authenticated-request auth-outcome presentation adapter (`auth_adapter.go`: 401 clears every protected presentation target before re-authentication, 403 retains the session and shows access-denied without a login loop). Presentation contract ONLY — it never issues, reads, verifies, or parses an auth token, cookie, or middleware result. |
| `internal/graphsynthetic/` | Spec 080 + BUG-080-001 SCOPE-03 product-owned Knowledge Graph read synthetic — a READ-ONLY, fixed-order observation over the canonical eight-family `internal/api/graphapi` route manifest using production HTTP behavior and a real scoped session, plus the CLOSED value-safe result contract (`GraphFamilyResult`, `AggregateResult`) that authenticated health, strict readiness, and capability status derive from. Configuration is fail-loud with NO defaults, and every result is validated against closed state / code / family vocabularies before it is published, logged, traced, or emitted as a metric label, so a node id, label, query value, cursor body, credential, or target host structurally cannot escape (SCN-080-001-07). Consumed in-product by `internal/api.GraphReadiness` through the `Observer` seam. |
...
| 061 | `061_model_provider_connections.sql` | Spec 096 SCOPE-02 operator-global encrypted-at-rest credential vault (runtime plane) — `model_provider_connections` table keyed 1:1 to the SST `llm.connections[]` registry `connection_id`; stores the AES-256-GCM `secret_ciphertext` + per-record 96-bit `secret_nonce` + `secret_key_version` master-key epoch + non-secret last-4 `secret_redaction` hint, the `enabled` runtime toggle, and `last_tested_at`/`last_test_outcome`/`last_test_detail`; the master key (`LLM_PROVIDER_SECRET_MASTER_KEY`) is env-held in the Go core and NEVER in the DB (a leak yields only ciphertext + nonce + key-version). CHECK constraints enforce ollama-carries-no-secret, the three crypto columns travel together (all-NULL or all-NONNULL), and an enabled hosted row must carry a credential. Reversible managed-secret class (never one-way hashed); no `actor_user_id` (operator-global); app-written timestamps (no DB-side default — G028) |
| 062 | `062_model_usage_ledger.sql` | Spec 096 SCOPE-05 append-only monthly USD spend ledger — `model_usage_ledger` table (`actor_user_id` per-user SPEND dimension, `connection_id` provider-kind grouping/provenance, `model` effective provider-qualified id, `tokens`, `usd_cost NUMERIC(12,6)`, app-written `created_at` monthly-window key) that makes the open-knowledge agent's per-user + global USD budgets load-bearing for paid providers: the budget pre-flight reads `SUM(usd_cost)` for the current month (filtered by `actor_user_id` for the per-user ceiling, unfiltered for the global ceiling) to refuse a paid `/ask` BEFORE any billable provider call, and the agent appends the realized cost after a successful billable turn. Rows are INSERT-only (never UPDATE/DELETE); a `created_at` index and a composite `(actor_user_id, created_at)` index serve the month-to-date SUMs; non-negative `usd_cost`/`tokens` CHECKs; no DB-side default (G028) |
| 063 | `063_auth_token_granted_scopes.sql` | Spec 108 design §10 (F-108-UX-ROSTER-01) additive nullable `auth_tokens.granted_scopes text[]` — records the scope set at issuance so a principal's grants are readable server-side without possessing the wire token; written by `IssueAndPersistToken` from the same value that becomes the PASETO `scope` claim (no DB-side default — NO-DEFAULTS / G028). Three DISTINCT states: `NULL` = UNKNOWN (issued before grant recording existed; produced by THIS MIGRATION ONLY, never by the write path), `'{}'` = RECORDED AS NONE (issued with no scope claim, including every admin-API mint), non-empty = the exact claim carried in the token. NULL and `'{}'` MUST NOT be conflated — NULL is "nobody recorded", `'{}'` is "recorded as no grants"; conflating them silently converts UNKNOWN into provably-no-grants, i.e. fabricated authority state. No backfill (existing rows stay NULL; unknown fails closed and the remedy is deliberate rotation); `ADD COLUMN IF NOT EXISTS` self-idempotent |
| 064 | `064_synthesis_durable_persistence.sql` | BUG-004-004 SCOPE-01 durable synthesis foundation — five tables (`synthesis_runs`, `synthesis_outputs`, `synthesis_output_insights`, `synthesis_citations`, `synthesis_attempts`) that make a synthesis result a fact in the database rather than a value a process was holding. `synthesis_runs.logical_key` is UNIQUE and carries the length-prefixed hash of (cadence, principal, window start, window end, policy version), so re-triggering the same window resolves to the SAME durable identity instead of minting a second run. Insights and citations are written inside the same serializable transaction as their output and the transaction is gated on a post-commit read-back, so a partially-written aggregate can never become readable. `synthesis_attempts` records every terminal outcome including failures, which is what lets health distinguish "nothing ever ran" from "the last run failed". App-written timestamps throughout (no DB-side default — G028) |
| 065 | `065_synthesis_output_kind.sql` | BUG-004-004 SCOPE-02 honest emptiness — adds `synthesis_outputs.output_kind` (`full`/`quiet`/`partial`, matching the `SynthesisOutputKind` vocabulary already in `synthesis_health.go`), `evaluated_artifact_count`, and the `synthesis_output_source_classes` table recording which source classes were included and which were omitted. A quiet window is a SUCCESSFUL run that found nothing worth saying, and storing that as a durable kind is what stops it being read back as never-run or as failure. `partial` names its omissions rather than presenting an incomplete answer as a whole one |
| 066 | `066_synthesis_run_lifecycle.sql` | BUG-004-004 SCOPE-03 coordination and lifecycle — adds `synthesis_runs.lifecycle_state` (DEFAULT `'current'`), `attempt_count` (DEFAULT `1`), `lease_holder`, and `lease_expires_at`. The two defaults are deliberate and are NOT hidden runtime configuration: they exist so the migration can add NOT NULL columns to rows that already exist, and every write path sets both explicitly. The lease pair lets exactly one holder claim a window, with expired leases reclaimable so a crashed holder cannot block the cadence forever |
| 067 | `067_synthesis_causal_event_truth.sql` | BUG-004-004 SCOPE-03A causal event truth — links numbered attempts to runs and outputs, adds immutable run-and-attempt-linked `synthesis_run_events`, projects principal/cadence/window lifecycle identity onto outputs, enforces one current output per window, adds causal-history indexes and foreign keys, and rejects event `UPDATE` or `DELETE` |
