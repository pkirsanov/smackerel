...
| **4** | **The digest cannot select, because nothing selectable is passed to it.** SQL returns `ORDER BY created_at DESC LIMIT 20`; the LLM prompt then receives those artifacts as **titles and types only** — no summary, no score, no content. Meanwhile `relevance_score` exists and is indexed, and both inputs to its §11.3 formula are extracted on every artifact and **discarded**. | [generator.go:344](../internal/digest/generator.go#L344), [nats_client.py:1032](../ml/app/nats_client.py#L1032) |
...
| Bill reminder from detected due date (§16.8) | ⚠️ Bills alert from a separate table; the extracted `temporal_relevance` due-date field is **never persisted or read**. |
...
graph TD
    ...
    X["quality signals extracted every call<br/>then DISCARDED<br/>(source_quality, temporal_relevance)"] --> I
    H --> I["digest has no ranking to use, and its prompt<br/>receives only titles<br/>→ falls back to recency"]
...
| **D15** | **Every artifact is compressed to roughly 100–150 words before it becomes searchable by meaning.** The universal prompt asks for a "2-4 sentence summary" plus ≤5 key ideas, and *that is the vector index*. A 10,000-word paper and a 200-word note therefore receive **identical retrieval surface area**. This is the quantified ceiling on "vague in, precise out" and explains why the lexical fallback over `content_raw` carries more weight than the vector path. | [`UNIVERSAL_PROCESSING_PROMPT`](../ml/app/processor.py#L26) + [embedder.py:210](../ml/app/embedder.py#L210) |
...
| **D14** | **The digest LLM is given nothing to select with.** Its prompt renders overnight artifacts as `f"- {title} ({type})"` — titles and types only, no summary, no score, no content — then instructs it to write an "overnight summary". The selection failure is therefore **not** "the model isn't good enough": the information required to choose was never passed. | [nats_client.py:1032-1072](../ml/app/nats_client.py#L1032) |
| **D16** | **Three signals are extracted on every LLM call and never consumed.** `source_quality` and `key_ideas` have **zero** ranking/filtering reads; `temporal_relevance` is fully dead — a schema column, a prompt field, a Go struct field, **no writer and no reader** (§16.8's promised bill reminders come from a separate alert table instead). `source_quality` and `temporal_relevance` are precisely the `base_quality_score` and recency inputs §11.3's relevance formula needs. **Relevance ranking is a wiring problem, not a data problem.** | consumption audit across `internal/`; [processor.py:56](../ml/app/processor.py#L56); [001_initial_schema.sql:38](../internal/db/migrations/001_initial_schema.sql#L38) |
...
| **Change** | (a) **Close the C8 violation** — read the model from config, fail loud if absent; delete the workaround comment at `main.py:500`. (b) Move to a current-generation model (dimension change ⇒ `vector(N)` migration + scripted background re-embed, not downtime). (c) **Chunk over raw content**: `artifact_chunks(artifact_id, ordinal, text, embedding)` with its own HNSW index; retrieval scores `max(chunk_score)` per artifact. This is the fix for **D15** — it ends the regime where a 10,000-word paper and a 200-word note have the same retrieval surface area. (d) Keep the summary vector as a second signal and the lexical path ([search.go:894](../internal/api/search.go#L894)) as a third — a real hybrid. Fixes **D2**, **D3**, **D15**. |
...
| | |
|---|---|
| **Why now** | Step 6 supplies the interaction signal §11.3 needs; Step 3 supplies trustworthy counts; Step 5 supplies a trustworthy `connection_count`. **And the remaining inputs already exist** — per **D16**, `source_quality` and `temporal_relevance` are extracted on every artifact and thrown away. This step is mostly wiring, not new computation. |
| **Change** | (a) **Implement §11.3 at ingestion** — compute `relevance_score` from base quality + topic momentum + interaction count + connection count + recency, with the `explicit_boost` multiplier, **consuming the already-extracted `source_quality` and `temporal_relevance` rather than adding new extraction**; persist `temporal_relevance` (currently written nowhere). (b) **Rewrite digest selection** — replace `ORDER BY created_at DESC LIMIT 20` with relevance-ranked selection targeting *the few that matter*. (c) **Pass the score and the summary into the digest prompt**, not just titles — without this the LLM still has nothing to select on (**D14**). (d) **Delete the "Knowledge Health" section from the user digest** — synthesis backlog is an operator metric belonging on `/status`. Fixes **D8**, **D14**, **D16**. |
...
| Quality signals extracted every call, then discarded | `source_quality` and `temporal_relevance` feeding the ranking they were extracted for |
...
- **The signal-consumption audit (D16) is grep-based**, counting ranking/filtering reads per field across `internal/`. `temporal_relevance` was additionally hand-verified across Go, SQL and Python (schema column and prompt field present; no writer, no reader). `source_quality` and `key_ideas` were counted, not exhaustively hand-traced.
