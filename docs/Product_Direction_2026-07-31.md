...
| # | Finding | Evidence |
|---|---|---|
| **1** | **The front door understands five things.** The assistant — Product Principle P2's "intelligent front door", first item in the nav — has exactly **5 user-facing intents** (`search my notes`, `check weather`, `remind me`, `find recipes`, `answer open question`) against **~20 built capabilities**. Ask "what am I spending on subscriptions?" and nothing can route it. | [scenarios.yaml](../config/assistant/scenarios.yaml) |
...
| **Capability exposure** | ❌ **Missing** | No single registry says "this capability exists, here is its intent, here is its tool, here is its MCP projection." Capabilities are reachable three unrelated ways — a slash command, a dedicated page, or (for 5 of them) an assistant intent — with no shared source of truth. This is why ~15 capabilities are effectively invisible. |
...
| **2** | **Reachability** — anything the system can do, you can ask for | every registered capability has an intent; intent-routing accuracy gated |
...
| | |
|---|---|
| **Why now** | Fifteen working capabilities are invisible (**D17**). This is the highest usability return on the ladder, and it is the **foundation Step 10 consumes** — one registry, two surfaces. |
| **Change** | (a) **Introduce the missing capability seam.** One registry entry per capability declares: `id`, natural-language intent, the domain service that backs it, `requires_provenance`, side-effect class, and its MCP projection. **This is the single source of truth for reachability** — the assistant, the slash commands, and (in Step 10) MCP all derive from it, instead of the three unrelated inventories that exist today. (b) **Add intents for what is already built**, using the documented 4-step recipe (prompt contract → manifest row → tool registration → wiring import), prioritised by §16 promise: subscriptions, people/cooling, trips, expertise, expenses, lists, learning paths. (c) **Extend the Step-1 eval corpus** with the new intents so routing accuracy is gated as it grows. (d) Retire slash commands that become redundant, keeping them as aliases. |
| **Value** | *"What am I spending on subscriptions?"*, *"who haven't I talked to lately?"*, *"what does my Lisbon trip look like?"* start working. **Nothing new is built — what exists becomes usable.** It also removes the structural cause of "saved as an idea": most questions currently fall below the floor because no intent exists to catch them. |
| **Verify** | Extended eval corpus: routing accuracy ≥ the existing `routing_accuracy_min: 0.85` **with the new intents included**, and a capability-coverage assertion that every registry entry has an intent. |
...
| **A3** | **A capability is not done until it is askable** | Registry coverage is asserted in CI: every registered capability has an intent and an eval-corpus case. This is what prevents the next fifteen invisible features. |
