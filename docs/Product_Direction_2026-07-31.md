...
- **Lenses used:** product, UX, runtime, stability, DevOps, simplification, trust, validation, audit, engineering, documentation, and spec freshness.
...
1. an authenticated caller can redirect configured photo credentials to a caller-selected host ([D24](#trust--security--compliance));
2. assistant tools take identity from model arguments instead of authenticated context and can search the global corpus without `corpus:read` ([D25](#trust--security--compliance));
3. connector handoff is not lossless: cursors can advance after publish failures, while `source_ref` is not persisted as stable replay identity ([D19](#runtime--real-user-execution), [D10](#engineering--code-signals));
4. ML work can outlive its JetStream lease by at least five times, without a heartbeat or shared inference limit ([D21](#runtime--real-user-execution));
5. several proactive jobs record or log delivery after an ignored send error ([D22](#runtime--real-user-execution)); and
6. the promised unconditional exit path exports only processed artifact rows and paginates them unsafely ([D23](#trust--security--compliance)).
...
**Verdict:** preserve the ingestion and domain foundations, replace implicit handoffs with durable contracts, make identity server-derived, prove delivery before committing state, then finish the context-server product that the repository already points toward.
...
| The repository already contains quality-gate patterns | The assistant eval corpus under [`tests/eval/assistant/`](../tests/eval/assistant/) and EXPLAIN assertions under [`tests/integration/agent/`](../tests/integration/agent/) are reusable for retrieval. |
...
| Authenticated principal | Photo credentials can cross audiences (D24); assistant tools trust model/body identity (D25). |
...
|---|---|
| Ask anything in natural language | Five declared user-facing intents cover only a fraction of built capabilities (D17). Retrieval also lacks an authenticated corpus grant boundary (D25). |
...
That edge is not defensible while credentials can cross audiences (D24), corpus reads bypass the corpus grant (D25), and exit is incomplete (D12/D23). Safety work is product work here.

### 2.7 Missing or incomplete capabilities

| Capability | Weight | Current status |
|---|---:|---|
| Authenticated-principal tool boundary | Critical | Missing; model/body identity crosses retrieval and notification tools (D25). |
...
graph LR
    ...
    S5 --> S6["6 Coherent PWA"]
    S6 --> S7["7 Measured HNSW retrieval"]
    S7 --> S8["8 Full-content retrieval"]
    S8 --> S9["9 Meaningful graph and feedback"]
    S9 --> S10["10 Relevance and synthesis"]
...
The operator asks, "what am I spending on subscriptions?" The capability registry routes the request. The authenticated principal, not a model-supplied ID, authorizes corpus access. Artifact sensitivity and provider credential audience are approved before any external egress. The result cites the winning chunk and retrieval strategy. The same governed capability is available through MCP, with an authorization-filtered tool list and no unapproved raw-content egress.
...
The operator can export a complete versioned corpus, import it elsewhere, or delete it entirely. Release documentation reports what the runtime ledger proves. The product can answer, with current numbers, whether it finds what the user is looking for and whether it delivered what it claims.
...
| **A4 - Runtime capability ledger** | Generated release/doc projection with drift gate | Implemented, configured, activated, live-verified, degraded, and disabled are separate fields. Release claims derive from this ledger. The spec 095 V7 mismatch is the regression fixture. |
