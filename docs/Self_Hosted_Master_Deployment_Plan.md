# Self-Hosted Master Deployment Plan — MIGRATED
...
>
> The cross-product self-hosted Master Plan was operator-coupled
...
>
> The companion file
> [`docs/Self_Hosted_Deployment_Plan.md`](Self_Hosted_Deployment_Plan.md) was
...
The self-hosted cross-product coordination plan now lives in the operator-private
deploy-adapter overlay repo, alongside the self-hosted adapter
...
[`.github/copilot-instructions.md`](../.github/copilot-instructions.md),
self-hosted adapter content is owned by the deploy-adapter overlay, not by this
product repo.

| What you are looking for | Where it lives now |
|--------------------------|--------------------|
| The self-hosted cross-product coordination plan (hardware, OS, networking, reverse-proxy contract, backup destinations, port allocation, rollout schedule, Uptime-Kuma monitor catalog, Tailscale ↔ Docker bridge mitigations) | Operator-private deploy-adapter overlay — self-hosted cross-product Master Plan |
| The self-hosted adapter implementation for Smackerel (`apply.sh`, `params.yaml`, `manifest.yaml`, etc.) | Operator-private deploy-adapter overlay, self-hosted adapter readiness artifact |
...
| The Smackerel-side per-product self-hosted plan (also migrated to a pointer stub) | This repo — [`docs/Self_Hosted_Deployment_Plan.md`](Self_Hosted_Deployment_Plan.md) (60-line migration-pointer stub) |
...
preserved in the deploy-adapter overlay's self-hosted adapter documentation. If you operate Smackerel on a target other than
self-hosted, this file is intentionally not relevant to you — see
