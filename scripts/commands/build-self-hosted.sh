#!/usr/bin/env bash
# scripts/commands/build-self-hosted.sh
#
# Spec 017 scope 03: `./smackerel.sh build --target self-hosted`
#
...
#
# Trust model: local-operator (single-operator self-hosted). The operator's
...
#   SMACKEREL_HARDWARE_TIER hardware profile selected by the deployment owner
#
# Outputs:
...
#   ghcr.io/pkirsanov/smackerel-config-bundles:self-hosted-<sourceSha> (pushed, signed)
...
bhl_require_env() {
  ...
  [[ -n "$value" ]] \
    || bhl_fail F017-BUILD-01 "$var_name env var required for build --target self-hosted"
}

# Required env.
bhl_require_env OPERATOR_COSIGN_KEY
bhl_require_env OPERATOR_COSIGN_PUBKEY
[[ -f "$OPERATOR_COSIGN_KEY" ]] \
  || bhl_fail F017-BUILD-01 "OPERATOR_COSIGN_KEY does not name a readable file"
[[ -f "$OPERATOR_COSIGN_PUBKEY" ]] \
  || bhl_fail F017-BUILD-01 "OPERATOR_COSIGN_PUBKEY does not name a readable file"
bhl_require_env COSIGN_PASSWORD
bhl_require_env SMACKEREL_HARDWARE_TIER
...
BUNDLE_ENV='self-hosted'
BUNDLE_TAG="${BUNDLE_ENV}-${SOURCE_SHA}"
BUNDLE_REF="${BUNDLE_REGISTRY}:${BUNDLE_TAG}"
# Hardware capability is a deployment-owner input. This producer records and
# consumes the chosen profile without mapping a concrete target to a tier.
BUNDLE_HARDWARE_TIER="$SMACKEREL_HARDWARE_TIER"
...
echo "=================================================================="
echo "smackerel build --target self-hosted"
echo "  sourceSha:   $SOURCE_SHA"
echo "  builtAt:     $BUILT_AT"
echo "  builtBy:     $BUILT_BY"
echo "  builtDirty:  $BUILT_DIRTY"
echo "  manifest:    $MANIFEST"
echo "  signing key: supplied"
echo "  hardware:    $BUNDLE_HARDWARE_TIER"
echo "=================================================================="
...
CORE_LOCAL_TAG="${CORE_REGISTRY}:self-hosted-${SHORT_SHA}"
ML_LOCAL_TAG="${ML_REGISTRY}:self-hosted-${SHORT_SHA}"
...
# .smackerel.local.env (sourced by smackerel.sh) cannot clobber the explicit
# deployment-owner SMACKEREL_HARDWARE_TIER input.
echo
...
echo
echo "=================================================================="
echo "build --target self-hosted COMPLETE"
echo "  manifest:      $MANIFEST"
echo "  manifest sig:  ${MANIFEST}.sig"
echo
echo "Next: hand the signed manifest to the external deployment owner."
echo "=================================================================="
