...
DRIVE_PROVIDER_GOOGLE_HTTP_RESPONSE_HEADER_TIMEOUT_SECONDS="$(required_value drive.providers.google.http_response_header_timeout_seconds)"
...
# Proactive & Correlated Experience foundation (spec 107 SCOPE-01 SST-owned; no
# generator defaults). NudgeRef registry TTL; the Go loader additionally
...
INTELLIGENCE_RELATIONSHIP_COOLING_CONFIDENCE_FLOOR="$(required_value intelligence.relationship_cooling.confidence_floor)"
INTELLIGENCE_RELATIONSHIP_COOLING_DEDUP_WINDOW_DAYS="$(required_value intelligence.relationship_cooling.dedup_window_days)"
INTELLIGENCE_ALERT_TIMING_LOOKAHEAD_DAYS="$(required_value intelligence.alert_timing.lookahead_days)"
INTELLIGENCE_ALERT_TIMING_MAX_CANDIDATES="$(required_value intelligence.alert_timing.max_candidates)"
INTELLIGENCE_ALERT_TIMING_CONFIDENCE_FLOOR="$(required_value intelligence.alert_timing.confidence_floor)"
INTELLIGENCE_RESURFACE_MIN_DORMANCY_DAYS="$(required_value intelligence.resurface.min_dormancy_days)"
INTELLIGENCE_RESURFACE_MAX_CANDIDATES="$(required_value intelligence.resurface.max_candidates)"
INTELLIGENCE_RESURFACE_CONFIDENCE_FLOOR="$(required_value intelligence.resurface.confidence_floor)"
INTELLIGENCE_EXPERTISE_MAX_TOPICS="$(required_value intelligence.expertise.max_topics)"
INTELLIGENCE_EXPERTISE_MATURITY_DAYS="$(required_value intelligence.expertise.maturity_days)"
INTELLIGENCE_EXPERTISE_BLIND_SPOT_MIN_MENTIONS="$(required_value intelligence.expertise.blind_spot_min_mentions)"
INTELLIGENCE_EXPERTISE_BLIND_SPOT_MAX_CAPTURES="$(required_value intelligence.expertise.blind_spot_max_captures)"
INTELLIGENCE_EXPERTISE_BLIND_SPOT_LIMIT="$(required_value intelligence.expertise.blind_spot_limit)"
INTELLIGENCE_SEASONAL_MIN_DATA_DAYS="$(required_value intelligence.seasonal.min_data_days)"
INTELLIGENCE_SEASONAL_TOPIC_MIN_CAPTURES="$(required_value intelligence.seasonal.topic_min_captures)"
INTELLIGENCE_SEASONAL_TOPIC_CANDIDATE_LIMIT="$(required_value intelligence.seasonal.topic_candidate_limit)"
INTELLIGENCE_SEASONAL_MAX_OBSERVATIONS="$(required_value intelligence.seasonal.max_observations)"
DIGEST_HOSPITALITY_GUEST_CANDIDATE_LIMIT="$(required_value digest.hospitality.guest_candidate_limit)"
DIGEST_HOSPITALITY_PROPERTY_CANDIDATE_LIMIT="$(required_value digest.hospitality.property_candidate_limit)"
...
RECOMMENDATIONS_WATCHES_DEFAULT_PRICE_DROP_THRESHOLD_PCT="$(required_value recommendations.watches.default_price_drop_threshold_pct)"
...
# Spec 094 — rich-conditions + N-day forecast knobs (REQUIRED, fail-loud via
# required_value; no fallback default). forecast_days 1..16; units are
# closed-vocabulary open-meteo params validated in internal/config/assistant.go.
ASSISTANT_SKILLS_WEATHER_FORECAST_DAYS="$(required_value assistant.skills.weather.forecast_days)"
ASSISTANT_SKILLS_WEATHER_TEMPERATURE_UNIT="$(required_value assistant.skills.weather.temperature_unit)"
ASSISTANT_SKILLS_WEATHER_WIND_SPEED_UNIT="$(required_value assistant.skills.weather.wind_speed_unit)"
ASSISTANT_SKILLS_WEATHER_PRECIPITATION_UNIT="$(required_value assistant.skills.weather.precipitation_unit)"
...
# LoadRetrieval) fails loud [F095-SST-MISSING] at startup if any value is
# missing, out of range, or (for contracts) not parseable JSON. contracts is a
# single-line compact JSON object (artifact type -> shape list) emitted via
# required_value (the flattener round-trips the raw line) — mirroring the
# ML_MODEL_MEMORY_PROFILES_JSON SST-JSON precedent.
RETRIEVAL_ROUTING_ENABLED="$(required_value retrieval.routing.enabled)"
RETRIEVAL_ROUTING_INTENT_CONFIDENCE_THRESHOLD="$(required_value retrieval.routing.intent_confidence_threshold)"
RETRIEVAL_ROUTING_STRATEGY_WHOLE_DOCUMENT_ENABLED="$(required_value retrieval.routing.strategies.whole_document_enabled)"
RETRIEVAL_ROUTING_STRATEGY_STRUCTURED_AGGREGATE_ENABLED="$(required_value retrieval.routing.strategies.structured_aggregate_enabled)"
RETRIEVAL_ROUTING_STRATEGY_VAGUE_RECALL_ENABLED="$(required_value retrieval.routing.strategies.vague_recall_enabled)"
RETRIEVAL_ROUTING_CONTRACTS="$(required_value retrieval.routing.contracts)"
RETRIEVAL_EVERGREEN_ENABLED="$(required_value retrieval.evergreen.enabled)"
RETRIEVAL_EVERGREEN_JUDGMENT_SOURCE="$(required_value retrieval.evergreen.judgment_source)"
RETRIEVAL_EVERGREEN_CONFIDENCE_FLOOR="$(required_value retrieval.evergreen.confidence_floor)"
RETRIEVAL_EVERGREEN_PER_TICK_BUDGET="$(required_value retrieval.evergreen.per_tick_budget)"
RETRIEVAL_EVERGREEN_DEDUP_WINDOW_DAYS="$(required_value retrieval.evergreen.dedup_window_days)"
RETRIEVAL_EVERGREEN_POOLS_SYNTHESIS_EXCLUDES_LOW_EVERGREEN="$(required_value retrieval.evergreen.pools.synthesis_excludes_low_evergreen)"
RETRIEVAL_EVERGREEN_POOLS_DIGEST_EXCLUDES_LOW_EVERGREEN="$(required_value retrieval.evergreen.pools.digest_excludes_low_evergreen)"
...
DRIVE_PROVIDER_GOOGLE_HTTP_RESPONSE_HEADER_TIMEOUT_SECONDS=${DRIVE_PROVIDER_GOOGLE_HTTP_RESPONSE_HEADER_TIMEOUT_SECONDS}
...
INTELLIGENCE_ALERT_TIMING_LOOKAHEAD_DAYS=${INTELLIGENCE_ALERT_TIMING_LOOKAHEAD_DAYS}
INTELLIGENCE_ALERT_TIMING_MAX_CANDIDATES=${INTELLIGENCE_ALERT_TIMING_MAX_CANDIDATES}
INTELLIGENCE_ALERT_TIMING_CONFIDENCE_FLOOR=${INTELLIGENCE_ALERT_TIMING_CONFIDENCE_FLOOR}
INTELLIGENCE_RESURFACE_MIN_DORMANCY_DAYS=${INTELLIGENCE_RESURFACE_MIN_DORMANCY_DAYS}
INTELLIGENCE_RESURFACE_MAX_CANDIDATES=${INTELLIGENCE_RESURFACE_MAX_CANDIDATES}
INTELLIGENCE_RESURFACE_CONFIDENCE_FLOOR=${INTELLIGENCE_RESURFACE_CONFIDENCE_FLOOR}
INTELLIGENCE_EXPERTISE_MAX_TOPICS=${INTELLIGENCE_EXPERTISE_MAX_TOPICS}
INTELLIGENCE_EXPERTISE_MATURITY_DAYS=${INTELLIGENCE_EXPERTISE_MATURITY_DAYS}
INTELLIGENCE_EXPERTISE_BLIND_SPOT_MIN_MENTIONS=${INTELLIGENCE_EXPERTISE_BLIND_SPOT_MIN_MENTIONS}
INTELLIGENCE_EXPERTISE_BLIND_SPOT_MAX_CAPTURES=${INTELLIGENCE_EXPERTISE_BLIND_SPOT_MAX_CAPTURES}
INTELLIGENCE_EXPERTISE_BLIND_SPOT_LIMIT=${INTELLIGENCE_EXPERTISE_BLIND_SPOT_LIMIT}
INTELLIGENCE_SEASONAL_MIN_DATA_DAYS=${INTELLIGENCE_SEASONAL_MIN_DATA_DAYS}
INTELLIGENCE_SEASONAL_TOPIC_MIN_CAPTURES=${INTELLIGENCE_SEASONAL_TOPIC_MIN_CAPTURES}
INTELLIGENCE_SEASONAL_TOPIC_CANDIDATE_LIMIT=${INTELLIGENCE_SEASONAL_TOPIC_CANDIDATE_LIMIT}
INTELLIGENCE_SEASONAL_MAX_OBSERVATIONS=${INTELLIGENCE_SEASONAL_MAX_OBSERVATIONS}
DIGEST_HOSPITALITY_GUEST_CANDIDATE_LIMIT=${DIGEST_HOSPITALITY_GUEST_CANDIDATE_LIMIT}
DIGEST_HOSPITALITY_PROPERTY_CANDIDATE_LIMIT=${DIGEST_HOSPITALITY_PROPERTY_CANDIDATE_LIMIT}
...
RECOMMENDATIONS_WATCHES_DEFAULT_PRICE_DROP_THRESHOLD_PCT=${RECOMMENDATIONS_WATCHES_DEFAULT_PRICE_DROP_THRESHOLD_PCT}
...
ASSISTANT_SKILLS_WEATHER_FORECAST_DAYS=${ASSISTANT_SKILLS_WEATHER_FORECAST_DAYS}
ASSISTANT_SKILLS_WEATHER_TEMPERATURE_UNIT=${ASSISTANT_SKILLS_WEATHER_TEMPERATURE_UNIT}
ASSISTANT_SKILLS_WEATHER_WIND_SPEED_UNIT=${ASSISTANT_SKILLS_WEATHER_WIND_SPEED_UNIT}
ASSISTANT_SKILLS_WEATHER_PRECIPITATION_UNIT=${ASSISTANT_SKILLS_WEATHER_PRECIPITATION_UNIT}
...
RETRIEVAL_ROUTING_STRATEGY_STRUCTURED_AGGREGATE_ENABLED=${RETRIEVAL_ROUTING_STRATEGY_STRUCTURED_AGGREGATE_ENABLED}
RETRIEVAL_ROUTING_STRATEGY_VAGUE_RECALL_ENABLED=${RETRIEVAL_ROUTING_STRATEGY_VAGUE_RECALL_ENABLED}
RETRIEVAL_ROUTING_CONTRACTS=${RETRIEVAL_ROUTING_CONTRACTS}
RETRIEVAL_EVERGREEN_ENABLED=${RETRIEVAL_EVERGREEN_ENABLED}
RETRIEVAL_EVERGREEN_JUDGMENT_SOURCE=${RETRIEVAL_EVERGREEN_JUDGMENT_SOURCE}
RETRIEVAL_EVERGREEN_CONFIDENCE_FLOOR=${RETRIEVAL_EVERGREEN_CONFIDENCE_FLOOR}
RETRIEVAL_EVERGREEN_PER_TICK_BUDGET=${RETRIEVAL_EVERGREEN_PER_TICK_BUDGET}
RETRIEVAL_EVERGREEN_DEDUP_WINDOW_DAYS=${RETRIEVAL_EVERGREEN_DEDUP_WINDOW_DAYS}
RETRIEVAL_EVERGREEN_POOLS_SYNTHESIS_EXCLUDES_LOW_EVERGREEN=${RETRIEVAL_EVERGREEN_POOLS_SYNTHESIS_EXCLUDES_LOW_EVERGREEN}
RETRIEVAL_EVERGREEN_POOLS_DIGEST_EXCLUDES_LOW_EVERGREEN=${RETRIEVAL_EVERGREEN_POOLS_DIGEST_EXCLUDES_LOW_EVERGREEN}
