#!/usr/bin/env bash
# tests/integration/build_target_homelab_trivy_test.sh
#
# Spec 017 scope 03 / Scenario (adversarial): "Trivy CRITICAL/HIGH gate blocks a bad image".
#
# Verifies the Trivy gate wiring in build-home-lab.sh by running trivy
...
SCRIPT="$REPO_ROOT/scripts/commands/build-home-lab.sh"
...
if ! grep -q 'trivy image.*--severity CRITICAL,HIGH.*--exit-code 1' "$SCRIPT"; then
  echo "FAIL: build-home-lab.sh does not invoke trivy with the expected --severity CRITICAL,HIGH --exit-code 1 wiring"
  exit 1
fi
echo "OK: build-home-lab.sh invokes trivy with --severity CRITICAL,HIGH --exit-code 1"
