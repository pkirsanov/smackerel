...
// self-hosted single-source model selection — SST contract proving the self-hosted
// bundle does NOT hardcode any operator/hardware-specific model.
//
// As of the single-source change, self-hosted's real model selection AND the
...
// every per-env self-hosted model override from config/smackerel.yaml's
// environments.self-hosted block, so the self-hosted bundle inherits the SAME generic
// commodity base that dev resolves.
//
// This test enforces that contract at the SST -> env-file boundary: it
// generates BOTH the self-hosted and dev env files from the live SST into isolated
// temp dirs, then asserts that for EACH of the 17 adapter-owned model env vars
// the self-hosted value EQUALS the dev value. Equality proves self-hosted carries no
// self-hosted-specific override; any divergence proves a re-hardcoded model.
//
...
// ollama_model) to gemma3:4b via environments.dev.*. self-hosted, having dropped
// all overrides, takes those four from the tier matrix
// (models.tiers.<tier>.interactive.model). Only the accel tier's interactive
// model is gemma3:4b, so accel makes self-hosted's tier-resolved interactive
// models match dev's explicit gemma3:4b — isolating the variable under test
// (self-hosted's per-env overrides) from the orthogonal hardware-tier dimension.
...
//
//   - If someone re-adds ANY self-hosted per-env model override to
//     config/smackerel.yaml (e.g.
//     environments.self-hosted.agent_provider_default_model: gpt-oss:20b, or
//     .ollama_memory_limit: "48G"), the self-hosted env emits the operator value
//     while dev keeps the commodity base, so selfHosted[k] != dev[k] and the
//     matching assertion FAILs naming the re-hardcoded key + both values.
//
//   - The operator-model-absence backstop additionally FAILs if a self-hosted
...
//
// Generation harness mirrors scripts/commands/config_self_hosted_runtime_env_test.sh:
// it supplies a non-dev-default POSTGRES_PASSWORD so the production-class
// self-hosted generate clears the spec 051 FR-051-005 dev-default-password guard
// (config.sh resolution path 1). config-validate is skipped for
// production-class targets (placeholder mode), so self-hosted generation succeeds
...
// selfHostedModelEnvKeys is the exact set of model env vars the deploy adapter
// now owns and injects for self-hosted. The self-hosted bundle MUST resolve each to
// the same generic commodity value dev resolves (no self-hosted-specific override
// remains in config/smackerel.yaml).
var selfHostedModelEnvKeys = []string{
	"OLLAMA_MEMORY_LIMIT",
	"LLM_MODEL",
	"OLLAMA_MODEL",
	"OLLAMA_VISION_MODEL",
...
// repoRootForSelfHostedNoHardcode climbs from CWD looking for
// config/smackerel.yaml. Independent of the `go test` working dir.
func repoRootForSelfHostedNoHardcode(t *testing.T) string {
	t.Helper()
	wd, err := os.Getwd()
	if err != nil {
...
// generateEnvForSelfHostedNoHardcode invokes scripts/commands/config.sh for a
...
// `config.sh --env self-hosted` is production-class and rejects the dev-default
...
// unaffected by it.
func generateEnvForSelfHostedNoHardcode(t *testing.T, root, targetEnv, outDir string) map[string]string {
	t.Helper()
	...
	cmd.Env = append(os.Environ(),
		...
		// Production-class self-hosted generation rejects the dev-default Postgres
	...
	return selfHostedParseEnvFile(string(envBytes))
}

// selfHostedParseEnvFile parses KEY=VALUE lines (one per line) into a map. Blank
// lines and comment lines (leading '#') are skipped; only the first '=' splits
// the pair so values containing '=' survive intact.
func selfHostedParseEnvFile(text string) map[string]string {
...
// TestSelfHostedDoesNotHardcodeModels asserts the single-source contract: the
// self-hosted bundle carries NO operator/hardware-specific model override, so for
// every one of the 17 adapter-owned model env vars the resolved self-hosted value
// equals the resolved dev (commodity-base) value. See the file header for the
// full rationale and adversarial properties.
func TestSelfHostedDoesNotHardcodeModels(t *testing.T) {
	root := repoRootForSelfHostedNoHardcode(t)

	selfHosted := generateEnvForSelfHostedNoHardcode(t, root, "self-hosted", t.TempDir())
	dev := generateEnvForSelfHostedNoHardcode(t, root, "dev", t.TempDir())

	// Primary contract: self-hosted == dev for every adapter-owned model field.
	// A mismatch means a self-hosted-specific override was re-hardcoded into
	// config/smackerel.yaml (the regression this gate forbids).
	for _, k := range selfHostedModelEnvKeys {
		hlVal, hlOK := selfHosted[k]
		devVal, devOK := dev[k]
		if !hlOK {
			t.Errorf("self-hosted.env is missing model key %q (every adapter-owned model field MUST still be emitted from the commodity base, fail-loud)", k)
			continue
		}
		if !devOK {
		...
		if hlVal != devVal {
			t.Errorf("self-hosted re-hardcodes model %q: self-hosted=%q but dev (commodity base)=%q — self-hosted MUST inherit the commodity base; operator/hardware-specific model selection is owned by the deploy adapter's params.yaml model_selection block and injected at apply, NOT pinned in config/smackerel.yaml", k, hlVal, devVal)
		}
	}
	...
	for k, want := range wantCommodity {
		if got := selfHosted[k]; got != want {
			t.Errorf("self-hosted commodity base %q = %q, want %q (the in-repo bundle MUST ship the generic commodity value; the operator value is adapter-injected at apply)", k, got, want)
		}
	}

	// Adversarial backstop: the in-repo self-hosted bundle MUST NOT carry the
	...
	for _, k := range selfHostedModelEnvKeys {
		...
		for _, operatorValue := range []string{"gpt-oss:20b", "gemma4:26b", "48G"} {
			if strings.Contains(v, operatorValue) {
				t.Errorf("self-hosted.env %q=%q carries operator-specific value %q — the in-repo bundle MUST ship the commodity base; gpt-oss:20b / gemma4:26b / 48G are adapter-injected at apply only", k, v, operatorValue)
			}
		}
	}

	t.Logf("self-hosted emits the commodity base (== dev) for all %d adapter-owned model fields (e.g. AGENT_PROVIDER_DEFAULT_MODEL=%q, ASSISTANT_OPEN_KNOWLEDGE_SYNTHESIS_MODEL_ID=%q, OLLAMA_MEMORY_LIMIT=%q); operator selection (gpt-oss:20b / gemma4:26b / 48G) is injected by the deploy adapter at apply, not pinned in config/smackerel.yaml",
		len(selfHostedModelEnvKeys), selfHosted["AGENT_PROVIDER_DEFAULT_MODEL"], selfHosted["ASSISTANT_OPEN_KNOWLEDGE_SYNTHESIS_MODEL_ID"], selfHosted["OLLAMA_MEMORY_LIMIT"])
}
