...
STATUS=$(curl -s --max-time 15 -o /dev/null -w '%{http_code}' "$CORE_URL/artifact?id=detail-e2e-001")
e2e_assert_eq "$STATUS" "200" "Artifact detail page returns 200"

BODY=$(curl -sf --max-time 15 "$CORE_URL/artifact?id=detail-e2e-001" 2>/dev/null || true)
...
STATUS=$(curl -s --max-time 15 -o /dev/null -w '%{http_code}' "$CORE_URL/artifact?id=nonexistent-999")
