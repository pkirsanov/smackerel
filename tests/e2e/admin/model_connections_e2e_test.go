...
// endpoints with REAL operator credentials, on the self-hosted bubbles.devops
...
// real credentials, which requires real secrets and is therefore a self-hosted
// concern (test-isolation + env-pollution policy forbid real provider secrets
// in dev/CI).
//
// To enable on the self-hosted dispatch, seed the live core URL, the operator
...
func liveAdminPrereqs(t *testing.T) (coreURL, operatorToken string) {
	...
	if coreURL == "" || operatorToken == "" {
		t.Skip("DEFERRED (SCOPE-06 C7, self-hosted bubbles.devops dispatch): set " +
...
// test → enable flow against a REAL Anthropic connection (self-hosted).
func TestAdmin_WireTestEnableAnthropic_Spec096(t *testing.T) {
	...
	t.Fatal("SPEC096_ADMIN_LIVE_* set but the live Anthropic wire/test/enable " +
		"assertion is implemented on the self-hosted bubbles.devops dispatch (real " +
...
// each independently testable/enable-able/disable-able (self-hosted).
...
// never a false success and never an Ollama substitute (self-hosted).
